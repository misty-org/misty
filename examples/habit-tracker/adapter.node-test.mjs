...
import { appID, connectionID, manifest } from "./manifest.mjs";
import { createHabitClient } from "./client.mjs";
...
test("the sample signs the exact public install document with a persistent publisher", async (t) => {
  ...
  const signScript = fileURLToPath(new URL("sign-manifest.mjs", import.meta.url));
...
test("the standalone HTTP adapter authenticates before accepting an effect", async (t) => {
  ...
  const child = spawn(process.execPath, [fileURLToPath(new URL("server.mjs", import.meta.url))], { cwd: directory, env: { PORT: "0", HABITS_BACKEND_TOKEN: token, HABITS_DATABASE: join(directory, "habits.sqlite") }, stdio: ["ignore", "pipe", "pipe"] });
  t.after(async () => {
