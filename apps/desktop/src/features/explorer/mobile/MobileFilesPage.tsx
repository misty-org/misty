...
export function MobileFilesPage() {
  ...
  const downloadEntries = async (nextEntries: FileEntry[]) => {
    ...
    let debugId: string | null = null;
    try {
      const destinationDirectory = await chooseMobileDownloadDirectory(homeDir);
  ...
  return (
    <section className="mobile-page mobile-files-page">
      ...
      <div className="mobile-files-tabs" role="tablist" aria-label="Files sections">
        ...
        <button
          type="button"
          className={activeFilesTab === "remote" ? "active" : ""}
          role="tab"
      ...
      <div className="mobile-files-sortbar">
        <div className="mobile-files-history-actions" aria-label="Folder navigation">
          <button
            type="button"
        ...
        <button
          type="button"
          className="mobile-files-sort-picker"
          aria-label="Sort files"
          aria-haspopup="dialog"
          onClick={() => setSortSheetOpen(true)}
        >
          <span>{mobileSortColumnLabel(sort.column)}</span>
          <span className="mobile-files-sort-direction">
            <ArrowUp className={sort.direction === "desc" ? "desc" : ""} size={28} strokeWidth={2.4} />
          </span>
        </button>
        <button
          type="button"
          className="mobile-files-more-button"
      ...
      {notice ? <div className="mobile-success">{notice}</div> : null}
      {actionDebug ? (
      ...
      <div className={`mobile-file-list ${viewMode === "grid" ? "grid-view" : ""}`} aria-busy={loading}>
        ...
        {entries.map((entry) => {
          ...
          <div
            key={entry.id}
            className={`mobile-file-row${selectionMode ? " selecting" : ""}${selected ? " selected" : ""}`}
          >
            {selectionMode ? (
              <button
                type="button"
                className={`mobile-file-select-button${selected ? " selected" : ""}`}
            ...
            <button
              type="button"
              className="mobile-file-row-main"
      ...
      <button
        type="button"
        className="mobile-files-floating-add"
        ...
        >
          ...
          >
            ...
            <dl className="mobile-detail-list">
              <div>
                <dt>Location</dt>
                <dd>{mobileLocationLabel(detailEntry.path, rootPath, mountRoot, remotes)}</dd>
              </div>
              <div>
                <dt>Size</dt>
                <dd>{formatBytes(detailEntry.sizeBytes)}</dd>
              </div>
              <div>
                <dt>Modified</dt>
                <dd>{formatDate(detailEntry.modifiedMs)}</dd>
              </div>
              <div>
                <dt>Type</dt>
                <dd>{detailEntry.mimeType || detailEntry.extension || detailEntry.kind}</dd>
              </div>
            </dl>
            <section className="mobile-detail-tags" aria-label="File tags">
              <span>Tags</span>
              {detailTags.length > 0 ? (
                <div className="mobile-detail-tag-list">
                  {detailTags.map((tag) => (
                    <button
                      key={tag}
                      type="button"
              ...
              <form
                className="mobile-detail-tag-form"
                onSubmit={(event) => {
              ...
              >
                ...
                />
                <button type="submit" disabled={actionBusy || !tagDraft.trim()}>
                  Add
                </button>
              </form>
            </section>
            {detailEntry.kind !== "folder" ? (
              <button type="button" className="mobile-primary-action" disabled={opening} onClick={() => void openFile(detailEntry)}>
...
function MobileWorkspaceDialog(props: {
  ...
  const title = props.state.kind === "create"
    ...
    : props.state.kind === "rename"
      ...
      : "Delete Workspace";
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onCancel}>
      <section
        className="mobile-detail-sheet mobile-file-action-sheet"
        role="dialog"
        aria-modal="true"
        aria-label={title}
        onClick={(event) => event.stopPropagation()}
      >
        <header>
          ...
          <button type="button" className="mobile-icon-button" aria-label="Close" onClick={props.onCancel}>
            <X size={20} />
          </button>
        </header>
        {deleting ? (
        ...
        <div className="mobile-action-stack">
          <button
            type="button"
            className={deleting ? "mobile-secondary-action danger" : "mobile-primary-action"}
            disabled={deleting ? props.workspaceCount <= 1 : !props.value.trim()}
            onClick={props.onConfirm}
          >
            {deleting ? "Delete" : "Save"}
          </button>
          <button type="button" className="mobile-secondary-action" onClick={props.onCancel}>
            Cancel
          </button>
        </div>
      </section>
  ...
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      ...
      >
        ...
        <div className="mobile-file-action-list" role="menu">
...
function MobileClaudeSheet(props: {
  ...
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      <section
        className="mobile-detail-sheet mobile-claude-sheet"
        role="dialog"
        aria-modal="true"
        aria-label="Claude"
        onClick={(event) => event.stopPropagation()}
      >
        <header>
          ...
          <button type="button" className="mobile-icon-button" aria-label="Close Claude" onClick={props.onClose}>
            <X size={20} />
          </button>
        </header>
        <div className="mobile-claude-status">
          <dl>
            <div>
              <dt>Status</dt>
            ...
            <div>
              <dt>Folder</dt>
              <dd>{props.workingDirectory || "No active folder"}</dd>
            </div>
            <div>
              <dt>Selection</dt>
              <dd>{props.selectedPath ?? "None"}</dd>
            </div>
          </dl>
          {error ? <p>{error}</p> : null}
        </div>
        <div ref={logRef} className="mobile-claude-log" aria-live="polite">
          {messages.length === 0 ? (
            <p className="mobile-claude-empty">Ask Claude about the current folder or selected file.</p>
          ) : messages.map((message) => (
            <article key={message.id} className={`mobile-claude-message ${message.role}`}>
        ...
        <form
          className="mobile-claude-composer"
          onSubmit={(event) => {
            event.preventDefault();
            submitPrompt();
          }}
        >
          ...
          />
          <div>
            <button type="button" className="mobile-secondary-action" disabled={messages.length === 0 || running} onClick={clearConversation}>
              Clear
            </button>
            {running ? (
              <button type="button" className="mobile-secondary-action danger" onClick={abortPrompt}>Stop</button>
            ) : (
              <button type="submit" className="mobile-primary-action" disabled={!installed || !prompt.trim()}>Send</button>
            )}
          </div>
        </form>
      </section>
...
function MobileFilesSelectionBar(props: {
  ...
  return (
    <section className="mobile-files-selection-bar" aria-label="Selected file actions">
      <header>
        <strong>{props.selectedCount === 1 ? "1 selected" : `${props.selectedCount} selected`}</strong>
        <div>
          <button type="button" disabled={!props.canSelectAll || props.busy} onClick={props.onSelectAll}>
            {props.allSelected ? "Clear" : "All"}
          </button>
          <button type="button" disabled={props.busy} onClick={props.onDone}>
            Done
          </button>
        </div>
      </header>
      <div>
        <button type="button" disabled={!hasActionableSelection || props.busy} onClick={props.onCopy}>
          <Copy size={17} strokeWidth={2} /> Copy
        </button>
        <button type="button" disabled={!hasActionableSelection || props.busy} onClick={props.onCut}>
          <Scissors size={17} strokeWidth={2} /> Cut
        </button>
        <button type="button" disabled={!hasActionableSelection || props.busy} onClick={props.onRename}>
          <Pencil size={17} strokeWidth={2} /> Rename
        </button>
        <button type="button" disabled={!props.canDownload || props.busy} onClick={props.onDownload}>
          <Download size={17} strokeWidth={2} /> Download
        </button>
        <button type="button" className="danger" disabled={!hasActionableSelection || props.busy} onClick={props.onDelete}>
    ...
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      ...
      >
        ...
        <div className="mobile-file-action-list" role="menu">
          ...
          />
          <div className="mobile-file-action-separator" role="separator" />
          <MobileFileActionButton
...
function MobileFilesSortSheet(props: {
  ...
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      <section
        className="mobile-detail-sheet mobile-file-action-sheet mobile-files-sort-sheet"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="mobile-files-sort-options" role="listbox" aria-label="Sort column">
          {mobileSortColumns.map((column) => (
            <button
              key={column}
              type="button"
              className={props.sort.column === column ? "selected" : ""}
            ...
            >
              ...
              {props.sort.column === column ? (
                <small>{props.sort.direction === "asc" ? "Ascending" : "Descending"}</small>
              ) : null}
            </button>
          ))}
        </div>

        <div className="mobile-files-segmented" aria-label="Sort direction">
          <button
            type="button"
            className={props.sort.direction === "asc" ? "selected" : ""}
          ...
          <button
            type="button"
            className={props.sort.direction === "desc" ? "selected" : ""}
        ...
        <div className="mobile-files-segmented" aria-label="View mode">
          <button
            type="button"
            className={props.viewMode === "list" ? "selected" : ""}
            onClick={() => props.onViewMode("list")}
          >
            List
          </button>
          <button
            type="button"
            className={props.viewMode === "grid" ? "selected" : ""}
            onClick={() => props.onViewMode("grid")}
          >
            Grid
          </button>
        ...
        <label className="mobile-files-toggle-row">
          <span>
            <strong>Show hidden files</strong>
            <small>{props.hiddenCount > 0 ? `${props.hiddenCount} hidden in this folder` : "Match desktop hidden file browsing"}</small>
          </span>
          <input
            type="checkbox"
...
function MobileFileSyncSheet(props: {
  ...
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      <section
        className="mobile-detail-sheet mobile-file-sync-sheet"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {props.pairError ? <div className="mobile-error">{props.pairError}</div> : null}
        {props.session?.error ? <div className="mobile-error">{props.session.error}</div> : null}
        {props.session?.message ? <div className="mobile-success">{props.session.message}</div> : null}

        <label className="mobile-file-sync-picker">
          <span>Pair</span>
          <select
        ...
        {props.pairs.length === 0 && !props.loadingPairs ? (
          <p className="mobile-file-sync-empty">No saved sync pairs yet. Create a pair from desktop compare, then run it here.</p>
        ) : null}

        {props.session ? (
          <>
            <div className="mobile-file-sync-endpoints">
            ...
            <div className="mobile-file-sync-actions">
              <button type="button" disabled={props.session.comparing || props.session.applying} onClick={props.onSwap}>
                Swap
              </button>
              <button type="button" disabled={props.session.comparing || props.session.applying} onClick={props.onCompare}>
                {props.session.comparing ? "Comparing..." : "Compare"}
              </button>
              <button type="button" className="primary" disabled={props.session.applying || plannedCount === 0} onClick={props.onApply}>
                {props.session.applying ? "Applying..." : `Apply${plannedCount ? ` ${plannedCount}` : ""}`}
              </button>
            </div>

            <div className="mobile-file-sync-summary" aria-label="Compare summary">
              <span>Left only <strong>{counts.left_only}</strong></span>
              <span>Right only <strong>{counts.right_only}</strong></span>
              <span>Different <strong>{counts.different}</strong></span>
              <span>Conflict <strong>{counts.conflict}</strong></span>
            </div>

            {props.session.rows.length > 0 ? (
              <div className="mobile-file-sync-results">
                {props.session.rows.map((row) => (
                  <article key={row.relativePath} className={`mobile-file-sync-row ${row.disposition}`}>
                    <div>
                      <strong>{row.relativePath || "/"}</strong>
                      <small>{row.kind} · {row.disposition.replace(/_/g, " ")}</small>
                    </div>
                    <span>
                      <small>Left</small>
                      <em title={mobileCompareSideTitle(row.left)}>{mobileCompareSideSummary(row.left)}</em>
                    </span>
                    <span>
                      <small>Right</small>
                      <em title={mobileCompareSideTitle(row.right)}>{mobileCompareSideSummary(row.right)}</em>
                    </span>
                    <label>
                      <small>Action</small>
                      <select
              ...
              <p className="mobile-file-sync-empty">Run compare to review planned sync actions.</p>
            )}
          </>
        ) : null}
      </section>
...
function MobileFileSyncEndpoint(props: { label: string; endpoint: FileSyncEndpoint }) {
  return (
    <div>
      ...
      <strong>{mobileFileSyncEndpointLabel(props.endpoint)}</strong>
    </div>
  );
}
  ...
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      ...
      >
        ...
        <div className="mobile-action-stack">
          <button type="button" className="mobile-primary-action" disabled={props.busy} onClick={props.onSubmit}>
            {props.busy ? "Creating..." : "Create"}
          </button>
          <button type="button" className="mobile-secondary-action" disabled={props.busy} onClick={props.onClose}>
            Cancel
          </button>
        </div>
      </section>
  ...
  return (
    <section className="mx-0 my-3 rounded-[18px] border border-white/10 bg-black/35 p-4 text-left text-xs text-white/70">
      <div className="mb-3 flex items-start justify-between gap-3">
        ...
        <button type="button" className="mobile-icon-button" aria-label="Hide action debug" onClick={props.onClose}>
          <X size={18} />
        </button>
      </div>
      {props.debug.error ? <div className="mobile-error">{props.debug.error}</div> : null}
...
function MobileBatchRenameSheet(props: {
  ...
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      <section
        className="mobile-detail-sheet mobile-file-action-sheet mobile-batch-rename-sheet"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <div>
            <span>Rename</span>
            <h2>{props.items.length} selected</h2>
          </div>
          <button type="button" className="mobile-icon-button" aria-label="Close" onClick={props.onClose}>
            <X size={20} />
          </button>
        </header>
        <div className="mobile-batch-rename-list">
          {props.items.map((item) => (
            <label key={item.entry.id} className={`mobile-batch-rename-row${item.error ? " invalid" : ""}`}>
              <span>
                <strong>{item.entry.name}</strong>
                {item.error ? <small>{item.error}</small> : <small>{mobileLocationShortLabel(item.entry.path)}</small>}
              </span>
              <span className="mobile-batch-rename-input">
                <input
                  value={item.value}
                  disabled={props.busy}
                  onChange={(event) => props.onValue(item.entry.id, event.target.value)}
                />
                {item.lockedExtension ? <em>{item.lockedExtension}</em> : null}
              </span>
            </label>
          ))}
        </div>
  ...
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      ...
      >
        ...
        <div className="mobile-action-stack">
          <button type="button" className="mobile-secondary-action danger" disabled={props.busy} onClick={props.onConfirm}>
...
function MobilePreviewSheet(props: {
  ...
  return (
    <div className="mobile-sheet-backdrop" role="presentation" onClick={props.onClose}>
      <section
        className="mobile-detail-sheet mobile-preview-sheet"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <div>
            <span>Preview</span>
            <h2>{props.preview.entry.name}</h2>
          </div>
          <button type="button" className="mobile-icon-button" aria-label="Close" onClick={props.onClose}>
            <X size={20} />
          </button>
        ...
        <div className="mobile-preview-body">
          {props.preview.loading ? <p>Loading preview...</p> : null}
          {props.preview.error ? <p>{props.preview.error}</p> : null}
          ...
          {!props.preview.loading && !props.preview.error && props.preview.url && props.preview.mimeType === "application/pdf" ? (
            <object data={props.preview.url} type={props.preview.mimeType} aria-label={`Preview of ${props.preview.entry.name}`} />
          ) : null}
          {!props.preview.loading
            && !props.preview.error
            && props.preview.url
            && props.preview.mimeType !== "application/pdf" ? (
              <img src={props.preview.url} alt={`Preview of ${props.preview.entry.name}`} />
            ) : null}
        </div>
      </section>
    </div>
...
function MobileFileActionButton(props: {
  ...
  return (
    <button
      type="button"
      role="menuitem"
      className={`mobile-file-action-button${props.danger ? " danger" : ""}`}
...
async function chooseMobileDownloadDirectory(homeDir: string): Promise<string | null> {
  const defaultPath = joinMobilePath(homeDir || smokeHome, "Downloads");
  const selection = await open({
