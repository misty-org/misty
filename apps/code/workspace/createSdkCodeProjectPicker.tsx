...
export function createSdkCodeProjectPicker(
  ...
  function Menu({ onSelect }: { onSelect(root: string): void }) {
    const state = store();
    useEffect(() => {
      if (!closed) void refresh();
      return cancelPending;
    ...
    return (
      <div className="w-full space-y-2 text-left">
        ...
        <div
          className="misty-transient-scrollbar max-h-56 space-y-1 overflow-y-auto pr-1"
          aria-busy={state.loading || state.busy}
        >
          {state.loading && !state.saved.length ? (
            <p className="py-3 text-center text-xs text-cream-muted" role="status">
              Loading saved projects…
            </p>
          ) : null}
          {!state.loading && !state.saved.length ? (
            <p className="py-3 text-center text-xs text-cream-muted">No recent projects yet.</p>
          ) : null}
          {state.saved.map((saved) => (
            <div
              key={saved.bookmarkId}
              className="group flex items-center justify-between rounded-lg px-2.5 py-1.5 transition hover:bg-charcoal-hover"
            >
              <button
                type="button"
                className="flex min-w-0 flex-1 items-center gap-2.5 text-left text-sm text-cream hover:text-cream-bright disabled:opacity-50"
                disabled={state.busy}
                onClick={() => void openSaved(saved, onSelect)}
              >
                <FolderOpen
                  size={16}
                  className="shrink-0 text-cream-muted group-hover:text-cream-bright"
                />
                <span className="min-w-0 truncate font-medium">{saved.name}</span>
                {!saved.writable ? (
                  <span className="rounded bg-charcoal-border px-1.5 py-0.5 text-[10px] text-cream-muted">
                    Read-only
                  </span>
                ) : null}
              </button>
              <Button
                variant="ghost"
                size="icon"
                className="size-7 opacity-0 transition group-hover:opacity-100 hover:text-cream-bright"
                disabled={state.busy}
                aria-label={`Forget ${saved.name}`}
  ...
  return { Picker, Menu, store, refresh, choose, openSaved, forget, cancelPending, close };
}
