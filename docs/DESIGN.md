...
Desktop navigation and workspace tabs can each occupy the left, top, right, or bottom edge, but must always occupy different edges. The layout customizer visibly disables an edge already used by the other control. Classic places navigation left and tabs top; Top bar places navigation top and tabs left; Bottom dock places navigation bottom and tabs left; Right rail places navigation right and tabs bottom. The editor lives in Settings → Layout; the shell has no persistent Layout button. Each virtual window owns its arrangement. Users can save, restore, and delete named presets locally on the device and reuse them across windows.

The native window chrome retains its 38px top band in every arrangement. The titlebar matches the black workspace surface, including beneath the macOS traffic lights; it does not reveal desktop wallpaper. Top-docked tabs stay in that band. Side tabs keep New tab fixed above the independently scrolling tab list as the list grows. Docking changes reposition the existing workspace without remounting its content. The Settings editor identifies the current virtual window and applies changes only to that window. These rules apply to the desktop shell; the Apple mobile shell retains its own layout. See [desktop docking implementation](docs/implementation/desktop-docking.md) for persistence and review scope.
...
Tools are available through workspace navigation without installation or catalog review. Misty releases are the only software update presented to users. Native folder and connection permissions stay attached to the operation that needs them.

### Browser site information and permissions

On macOS, the information button beside a website's address opens a compact charcoal popover. Lead with the current origin and an explicit connection state supplied by the native browser, pairing a lock or warning icon with explanatory text. Camera and Microphone use labeled native Ask / Allow / Block selectors, with muted supporting copy and a Reset permissions action. Choices stay on this device per origin and browser profile; macOS permissions still apply. Temporary sessions explain that choices are not saved and disable the selectors.

Settings → Privacy lists saved choices for open browser profiles in the existing settings row pattern: origin first, camera and microphone decisions beneath, and Reset aligned to the right. Preserve the established cream text, compact type hierarchy, subtle borders, and neutral control states. Keep loading and error feedback explicit, and describe blocking or resetting as stopping affected capture in the browser profile. This pattern covers camera, microphone, and connection information only.

### Desktop Control

The permanent Control pill follows the back/forward arrows and remains available with navigation collapsed or the nonmodal device-sleep screen open. It uses a sliders icon, green for healthy sync, a red warning icon for failures, and neutral styling for an unconfirmed connection. The compact charcoal popover retains status, Local saving, Website data, Retry now, and Settings.

Device rows show account-enrolled browser sync devices with Connected, Offline, or Unknown presence and an Active label. Switching stays pending until signed activation is confirmed. Full sync is available on connected, capable devices: off keeps an independent workspace connected; on rejoins the shared workspace. Show explicit “Turning Full sync on/off…” feedback while confirmation is pending. Upload and download rates describe this device’s sync WebSocket payload bytes per second, excluding website traffic and HTTP/TLS connection overhead.
