...
Implementation lives in `src/features/app-shell/dockingLayout.ts` and `src/app/layouts/DesktopLayout/`, principally `index.tsx`, `dockingGeometry.ts`, `docking.css`, `GlobalNavigator.tsx`, `WorkspaceCanvas.tsx`, and `WorkspaceLayoutTabs.tsx`. The editor is `src/features/settings/sections/LayoutSection.tsx`; `src/features/workspace/useWindowDockingLayout.ts` supplies the same active-window choice to Settings and the shell.
