...
| Rust/Tauri embedded storage feature check | Local build host | Pass | Run `npm run service:archive`, then `cargo check --manifest-path src-tauri/Cargo.toml --features embedded-storage-go`. |
