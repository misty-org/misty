...
import { configureApiSession } from "../src/api/client/session";
import { inboxFixture, mailCalls } from "./sdk-inbox-host-fixture";
...
configureApiSession({
  ...
  readGeneration: () => 0,
  readToken: async () => "disposable-inbox-cache-secret",
});
const params = new URLSearchParams(location.search);
const nonce = params.get("nonce")!;
const catalog = await fetch(params.get("catalog")!).then((response) => response.json());
const app: OfficialApp = catalog.apps.find((app: OfficialApp) => app.id === "inbox");
if (app?.desktop.runtime !== "downloaded") throw new Error("Inbox candidate is unavailable");
...
const count = () =>
  [...document.querySelectorAll('[data-misty-component-app="inbox"]')].filter((root) =>
    root.querySelector("[data-email-thread]"),
  ).length;
const tabs = () => dockTabs(useWorkspaceStore.getState().layout.root);
const inboxTabs = () => tabs().filter((tab) => tab.groupKey === "app:inbox");
...
function button(name: string, parent: ParentNode = document) {
  return [...parent.querySelectorAll<HTMLElement>('button, [role="button"]')].find(
    (button) =>
      button.textContent?.trim() === name ||
      button.getAttribute("aria-label") === name ||
      (name === "Downloaded Inbox SDK" && button.textContent?.includes(name)),
  );
}
let stage = "Discover install",
  ...
  message = "";
try {
  await until(() => !!button("Add Inbox"), "Discover did not offer Inbox installation");
  button("Add Inbox")!.click();
  ...
  await until(
    () => !!button("Inbox", document.querySelector('nav[aria-label="Primary"]')!),
    "Installed Inbox did not appear in navbar",
  );
  button("Cancel", document.querySelector('[role="dialog"]')!)!.click();
  button("Inbox", document.querySelector('nav[aria-label="Primary"]')!)!.click();
  await until(
    () => !!document.querySelector('nav[aria-label="Primary"] a[href^="/apps/inbox"]'),
    "Inbox destinations did not expand",
  );
  stage = "navbar open";
  document
    .querySelector<HTMLAnchorElement>('nav[aria-label="Primary"] a[href^="/apps/inbox"]')!
    .click();
  await until(() => !!button("Downloaded Inbox SDK"), "Downloaded Inbox did not load its mailbox");
  button("Downloaded Inbox SDK")!.click();
  await until(
    () => !!document.querySelector('[data-inbox-message-view][data-state="open"]'),
    "Message detail did not open",
  );
  await until(
    () => mailCalls.some((call) => call.method === "mail.threads.get"),
    "Detail skipped SDK RPC",
  );
  stage = "save draft";
  button("Expand to full window")!.click();
  await until(() => !!button("Save draft"), "Reply composer did not open");
  button("Save draft")!.click();
  await until(
    () => mailCalls.some((call) => call.method === "mail.drafts.create"),
    "Draft did not save through SDK RPC",
  );
  await until(() => !button("Saving…"), "Draft save did not finish");
  button("Save draft")!.click();
  await until(
    () => mailCalls.some((call) => call.method === "mail.drafts.update"),
    "Draft update did not use its opaque provider ID",
  );
  document.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true }));
  stage = "split panel";
  const original = inboxTabs()[0];
  ...
  const second = useWorkspaceStore
    .getState()
    .openSurface({ ...workspaceSurfaceFromRoute("/apps/inbox?space=sdk-space")!, forceNew: true });
  if (!useWorkspaceStore.getState().splitPane(pane.id, "right", second.id))
    throw new Error("Host refused second panel");
  await until(
    () => count() === 2 && inboxTabs().length === 2,
    "Two downloaded Inbox panels did not render",
  );
  stage = "close panels";
  useWorkspaceStore.getState().closeTab(second.id);
  useWorkspaceStore.getState().focusTab(original.id);
  await until(() => count() === 1, "Closing peer removed the wrong Inbox view");
  useWorkspaceStore.getState().closeTab(original.id);
  useWorkspaceStore.getState().focusTab(discover.id);
  await until(() => count() === 0, "Closing Inbox retained its component");
  stage = "Discover remove";
  await until(() => !!button("View Inbox details"), "Discover did not return");
  button("View Inbox details")!.click();
  ...
  if (document.querySelector('nav[aria-label="Primary"] a[href^="/apps/inbox"]'))
    throw new Error("Removed Inbox remains in navbar");
  if (
  ...
  success = true;
  message =
    "PASS: actual Discover install, verified native package, navbar appearance/open, normal OfficialAppRuntimePage/DownloadedAppSurface, message reading and draft create/update through SDK RPC, two Inbox workspace panels, panel/tab cleanup, Discover confirmed removal and navbar/package cleanup. Account and installation API replies are fixtures; no real account was changed.";
} catch (error) {
