function(target, input, origin) {
 try {
  ...
  if(!element?.isConnected || element.ownerDocument!==record.document || !record.isCurrent() || record.readFingerprint(element)!==record.fingerprint)return {ok:false,errorCode:"browser_snapshot_stale",error:"The file input changed. Inspect again."};
  const view=element.ownerDocument.defaultView;
  if(!(element instanceof view.HTMLInputElement)||element.type!=="file"||element.disabled)throw new Error("An enabled file input is required.");
  ...
  const transfer=new view.DataTransfer();transfer.items.add(new view.File([bytes],file.name,{type:file.mimeType}));
