...
pub(super) struct RemoteUploadRequest<'a> {
    pub(super) local_path: &'a Path,
    pub(super) remote_name: &'a str,
    pub(super) remote_directory: &'a str,
    pub(super) item_name: &'a str,
    pub(super) transfer_id: Option<u64>,
    pub(super) progress: Option<TransferProgress>,
...
impl ExplorerService {
    pub(super) async fn upload_local_item(
        ...
        if !is_directory {
            return self
                .upload_remote_file(RemoteUploadRequest {
                    local_path: source,
                    remote_name: &destination.remote_name,
                    remote_directory: &destination.remote_path,
        ...
        self.upload_remote_directory(RemoteUploadRequest {
            local_path: source,
            remote_name: &destination.remote_name,
            remote_directory: &destination.remote_path,
            item_name: source_name,
    ...
    pub(super) async fn upload_remote_file(
        &self,
        request: RemoteUploadRequest<'_>,
    ) -> ApiResult<()> {
        let RemoteUploadRequest {
            local_path,
            remote_name,
            remote_directory,
            item_name: file_name,
