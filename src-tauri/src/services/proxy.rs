...
impl ProxyResponse {
    ...
    pub async fn text(self) -> ApiResult<String> {
        String::from_utf8(self.body).map_err(|error| {
            ApiError::Message(format!("Proxy response was not valid UTF-8: {error}"))
        })
    }
...
impl ProxyService {
    pub fn new(environment: AppEnvironmentService) -> Self {
        Self::new_with_proxy_runtime(environment, None, None)
    }
    ...
    pub async fn get(&self, path: &str) -> ApiResult<ProxyResponse> {
    ...
    pub async fn delete_with_query<Q>(&self, path: &str, query: &Q) -> ApiResult<ProxyResponse>
    where
        Q: Serialize + ?Sized,
    {
    ...
    pub async fn delete(&self, path: &str) -> ApiResult<ProxyResponse> {
        self.embedded_delete(path).await
    }

    async fn invoke_embedded(&self, method: &str, params: Value) -> ApiResult<ProxyResponse> {
        let runtime = self.inner.proxy_runtime.as_ref().ok_or_else(|| {
            ApiError::Unavailable("Embedded proxy runtime is not configured.".to_string())
        })?;
        let data = runtime
            .invoke_embedded(method, params)
            .map_err(ApiError::Message)?;
        ProxyResponse::ok_json(data)
    }

    async fn embedded_get(&self, path: &str) -> ApiResult<ProxyResponse> {
        match path {
            "/api/remote/health" => {
            ...
            _ => Err(ApiError::Unavailable(format!(
                "Embedded proxy transport does not support GET {path}"
            ))),
        }
    }

    async fn embedded_get_with_query<Q>(&self, path: &str, query: &Q) -> ApiResult<ProxyResponse>
    where
        Q: Serialize + ?Sized,
    {
        ...
        match path {
            ...
            _ => Err(ApiError::Unavailable(format!(
                "Embedded proxy transport does not support GET {path} with query"
            ))),
        }
    }

    async fn embedded_post_json(&self, path: &str, body: &Value) -> ApiResult<ProxyResponse> {
        let method = match path {
            ...
            _ => {
                return Err(ApiError::Unavailable(format!(
                    "Embedded proxy transport does not support POST {path}"
                )))
            }
        };
    ...
    async fn embedded_delete_with_query<Q>(&self, path: &str, query: &Q) -> ApiResult<ProxyResponse>
    where
        Q: Serialize + ?Sized,
    {
        let params = query_params_value(query)?;
        match path {
            ...
            _ => Err(ApiError::Unavailable(format!(
                "Embedded proxy transport does not support DELETE {path} with query"
            ))),
        }
    }

    async fn embedded_delete(&self, path: &str) -> ApiResult<ProxyResponse> {
        ...
        Err(ApiError::Unavailable(format!(
            "Embedded proxy transport does not support DELETE {path}"
        )))
    }

    async fn embedded_remote_job_get(&self, path: &str) -> ApiResult<ProxyResponse> {
