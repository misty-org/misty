...
pub(super) fn encode_png(image: arboard::ImageData<'_>) -> Result<Value, String> {
    ...
    if width == 0
        ...
        || height > 4096
        || pixels > MAX_PIXELS
        || image.bytes.len() as u64 != pixels * 4
    {
    ...
    Ok(
        serde_json::json!({"mimeType":"image/png","data":base64::engine::general_purpose::STANDARD.encode(png)}),
    )
}
...
pub(super) fn decode_png(params: &Value) -> Result<arboard::ImageData<'static>, String> {
    ...
    let dimensions = image::ImageReader::with_format(Cursor::new(&bytes), image::ImageFormat::Png)
        .into_dimensions()
        .map_err(|_| "Invalid clipboard PNG.")?;
    if dimensions.0 == 0
        || dimensions.1 == 0
        || dimensions.0 > 4096
        || dimensions.1 > 4096
...
mod tests {
    ...
    fn clipboard_png_decodes_bounded_rgba_and_rejects_unsupported_payloads() {
        ...
        assert!(encode_png(arboard::ImageData {
            width: 2,
            height: 1,
            bytes: Cow::Owned(vec![1])
        })
        .is_err());
        assert!(encode_png(arboard::ImageData {
            width: usize::MAX,
        ...
        assert!(
            decode_png(&json!({"mimeType":"image/png","data":"AAAA","path":"/tmp/image"})).is_err()
        );
