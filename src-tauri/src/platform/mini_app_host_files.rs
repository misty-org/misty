...
fn directory_path(directory: &cap_std::fs::Dir) -> Result<std::path::PathBuf, String> {
    ...
    let current =
        fs::symlink_metadata(&path).map_err(|_| "The chosen folder moved or was removed.")?;
    if !path.is_absolute()
        || !current.is_dir()
...
pub async fn mini_app_host_file(
    ...
    match operation.as_str() {
        "startTransfer" => {
            ...
            let request: crate::domain::explorer::PasteItemsRequest = serde_json::from_value(
                params
                    .get("request")
                    .cloned()
                    .ok_or("Missing transfer request.")?,
            )
            .map_err(|_| "Invalid host transfer request.")?;
            if request.sources.len() != 1 {
                return Err("Host transfers require one owned source.".into());
            }
            let slot = p
                .transfer_slots
                .clone()
            ...
            let status = Arc::new(Mutex::new(
                json!({"status":"running","bytes":0,"files":0,"message":"Transferring connected file…","result":null}),
            ));
            ...
            p.transfers.insert(
                job_id.clone(),
                super::file_transfers::TransferJob {
                    state: status.clone(),
                    cancel: cancel.clone(),
                },
            );
            let explorer = app
                .state::<crate::app::runtime::MistyRuntime>()
                .explorer
                .clone();
            let peers = app
                .state::<crate::app::runtime::MistyRuntime>()
            ...
            tokio::spawn(async move {
                ...
                let monitor = tokio::spawn(async move {
                    let _ = permission.changed().await;
                ...
                let operation = async {
                    if request.sources[0].path.starts_with("misty://device/") {
                        if matches!(
                            ...
                            return Err("Connected devices are read-only.".to_owned());
                        }
                        let prepared = peers
                            .materialize_tree(&request.sources[0].path)
                            .await
                            .map_err(|e| e.to_string())?;
                        request.sources[0].path =
                            prepared.local_path.to_string_lossy().into_owned();
                    }
                    let removed = matches!(
                        request.operation,
                        crate::domain::explorer::ClipboardOperation::Move
                    );
                    let kind = if request.sources[0].is_directory {
                        "directory"
                    } else {
                        "file"
                    };
                    let result = explorer
                        .paste_items_with_cancellation(request, cancel.clone())
                        .await
                        .map_err(|e| e.to_string())?;
                    let path = result
                        .affected_paths
                        .first()
                        .ok_or("Transfer finished without a destination.")?;
                    ...
                    Ok::<_, String>(
                        json!({"entry":token,"name":name,"kind":kind,"sourceRemoved":removed}),
                    )
                }
                .await;
                monitor.abort();
                if let Ok(mut current) = status.lock() {
                    match operation {
                        Ok(result) => {
                            ...
                            current["files"] = json!(1);
                            current["message"] = json!("Done");
                            current["result"] = result;
                        }
                        Err(error) => {
                            current["status"] = json!(if cancel.load(Ordering::Acquire) {
                                "cancelled"
                            } else {
                                "failed"
                            });
                            current["message"] =
                                json!(error.chars().take(2048).collect::<String>());
                        }
                    }
                }
            });
            ...
            #[cfg(target_os = "macos")]
            {
                if let Some(folder) = p.folders.get(handle) {
                    if folder.released.load(std::sync::atomic::Ordering::Acquire)
                        || write && !folder.writable
                    {
                        return Err("This folder is read-only or has been released.".into());
                    }
                    return Ok(
                        json!({"path":directory_path(&folder.directory)?, "kind":"directory", "writable":folder.writable}),
                    );
                }
                let file = p
                    .files
                    .get(handle)
                    .ok_or("This file belongs to another App or has closed.")?;
                if write && !file.writable {
                    return Err("This file is read-only.".into());
                }
                Ok(
                    json!({"path":super::file_open::granted_path(&file.file)?, "kind":"file", "writable":file.writable}),
                )
            }
            #[cfg(not(target_os = "macos"))]
            {
                let _ = handle;
                Err("Native file services currently require macOS.".into())
            }
        }
        "adoptDirectory" => {
            let write = params
            ...
            if p.folders.len() >= 64 {
                return Err("Close a folder before opening another.".into());
            }
            let directory = cap_std::fs::Dir::open_ambient_dir(path, cap_std::ambient_authority())
                .map_err(|_| "This source folder is unavailable.")?;
            let name = params
                .get("name")
                .and_then(Value::as_str)
                .unwrap_or("Folder")
        ...
        "adoptPrepared" => {
            ...
            let path = params
                ...
                .and_then(Value::as_str)
                .filter(|path| std::path::Path::new(path).is_absolute())
                .ok_or("Invalid prepared file.")?;
            if p.files.len() >= 256 {
                return Err("Too many open files in this App.".into());
            }
            let metadata =
                fs::symlink_metadata(path).map_err(|_| "The prepared file is unavailable.")?;
            if !metadata.is_file() {
                return Err("Prepared sources must be regular files.".into());
            }
            #[cfg(unix)]
            let file = {
            ...
            let bytes = file
                .metadata()
                .map_err(|_| "The prepared file is unavailable.")?
                .len();
            let handle = uuid::Uuid::new_v4().to_string();
            p.files.insert(
                handle.clone(),
...
mod tests {
    ...
    fn resolves_retained_folder_identity_after_rename_and_rejects_unlinked_folders() {
        ...
        fs::create_dir(&path).unwrap();
        assert_eq!(
            directory_path(&directory).unwrap(),
