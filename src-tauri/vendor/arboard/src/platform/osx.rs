...
impl<'clipboard> Get<'clipboard> {
	...
	pub(crate) fn image(self) -> Result<ImageData<'static>, Error> {
		...
		// Keep TIFF handling in AppKit instead of statically linking a TIFF
		// decoder solely for the system pasteboard's native representation.
...
#[cfg(feature = "image-data")]
fn decode_pasteboard_image(data: &objc2_foundation::NSData) -> Result<image::DynamicImage, Error> {
	use objc2_app_kit::{NSBitmapImageFileType, NSBitmapImageRep};
	let bitmap = NSBitmapImageRep::imageRepWithData(data).ok_or(Error::ConversionFailure)?;
	// The empty dictionary has no incorrectly typed property values.
	let png = unsafe {
		bitmap.representationUsingType_properties(NSBitmapImageFileType::PNG, &NSDictionary::new())
	}
	.ok_or(Error::ConversionFailure)?;
	image::load_from_memory_with_format(&png.to_vec(), image::ImageFormat::Png)
...
mod native_image_tests {
	use super::*;
	#[test]
	fn native_tiff_roundtrip_preserves_pixels_without_touching_the_clipboard() {
		autoreleasepool(|_| {
			let pixels = vec![255, 0, 0, 255, 0, 128, 255, 128, 0, 0, 0, 0, 255, 255, 255, 255];
			let image = image_from_pixels(pixels.clone(), 2, 2);
			let tiff = image.TIFFRepresentation().expect("native TIFF");
			let decoded = decode_pasteboard_image(&tiff).expect("native decode").into_rgba8();
			assert_eq!(decoded.dimensions(), (2, 2));
			assert_eq!(decoded.into_raw(), pixels);
			let invalid = objc2_foundation::NSData::with_bytes(b"corrupt TIFF");
			assert!(decode_pasteboard_image(&invalid).is_err());
		});
	}
}
