...
export function DrawingPreview(props: { drawing: SpaceDrawing; user: AuthUser }) {
      ...
      try {
        ...
        const hydratedFiles = await Promise.allSettled(
          Array.from(session.files.values()).map(async (reference) => {
            const file = await hydrateDrawingBinaryFile(
  ...
  }, [props.drawing.id, props.drawing.space_id, room.session, room.synced]);
  ...
  const exportPng = () => {
    ...
    downloadUrl(previewUrl, drawingExportFilename(props.drawing.title, "png"));
  };
