...
import { useExplorerStore, type ExplorerInlineEditState } from "../../../stores/useExplorerStore";
import { ExplorerNotifications, ExplorerRenameStatus } from "../desktop/ExplorerDesktopStatus";
...
describe("Explorer rename feedback", () => {
  ...
  it("keeps validation text out of the inline editor", async () => {
    await act(async () => root.render(
      <InlineNameEditor edit={invalidRename} onChange={() => undefined} onCommit={() => undefined} onCancel={() => undefined} />,
    ));
  ...
  it("pushes invalid rename feedback into the top notification stack", async () => {
    ...
    await act(async () => root.render(
      <ExplorerNotifications notifications={notification ? [notification] : []} onDismiss={() => undefined} />,
    ));
