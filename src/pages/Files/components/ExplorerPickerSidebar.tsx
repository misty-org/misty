...
export function ExplorerPickerSidebar(props: ExplorerPickerSidebarProps) {
  ...
  const quickAccess = useMemo(() => quickAccessItems.map((item) => ({
    ...item,
    path: item.suffix ? joinPath(props.homePath, item.suffix) : props.homePath,
  })).filter((item) => !quickAccessPathHidden(item.path, hiddenQuickAccessPaths)), [hiddenQuickAccessPaths, props.homePath]);
  const visiblePinnedPaths = useMemo(
    () => dedupePinnedPathsForQuickAccess(props.pinnedPaths, quickAccess.map((item) => item.path)),
  ...
  return (
    <aside className={sidebarStyles.root} aria-label="File locations">
      ...
      <section className={sidebarStyles.section}>
        <SidebarSectionHeader title="Quick access" collapsed={collapsedSections.quickAccess} onToggle={() => toggleSection("quickAccess")} />
      ...
      <section className={sidebarStyles.section}>
        <SidebarSectionHeader title="Remote" collapsed={collapsedSections.remote} onToggle={() => toggleSection("remote")} />
        {!collapsedSections.remote ? (
          props.remoteLoading && props.remotes.length === 0 ? <div className={sidebarStyles.muted}>Loading remote...</div>
            : props.remotes.length === 0 ? <div className={sidebarStyles.muted}>No remotes connected</div>
              : (
                <div className={sidebarStyles.list}>
      ...
      <section className={sidebarStyles.section}>
        ...
        {!collapsedSections.devices ? (
          deviceEntries.length === 0 ? <div className={sidebarStyles.muted}>{props.devicesLoading ? "Loading devices..." : "No devices connected"}</div>
            : (
              <div className={sidebarStyles.list}>
