...
vi.mock("../../../shared/tauri", () => ({ safeTauriAssetUrl: (path: string) => `asset://${path}` }));
vi.mock("@tauri-apps/api/image", () => ({ Image: { fromBytes: vi.fn(async () => ({ close: closeNativeImage })) } }));
vi.mock("@tauri-apps/plugin-clipboard-manager", () => ({ writeImage }));

import { GlobalPreviewDialog, globalPreviewKindForSource } from "./GlobalPreview";
...
describe("GlobalPreviewDialog image editor", () => {
  ...
  let root: Root;
  const context = {
    save: vi.fn(), restore: vi.fn(), clearRect: vi.fn(), translate: vi.fn(), rotate: vi.fn(),
    scale: vi.fn(), drawImage: vi.fn(), filter: "", getImageData: vi.fn(() => ({ data: new Uint8ClampedArray([20, 30, 40, 255]) })),
    beginPath: vi.fn(), moveTo: vi.fn(), lineTo: vi.fn(), stroke: vi.fn(), strokeRect: vi.fn(), fillText: vi.fn(),
  };

  beforeEach(() => {
    (globalThis as typeof globalThis & { IS_REACT_ACT_ENVIRONMENT?: boolean }).IS_REACT_ACT_ENVIRONMENT = true;
    savePreview.mockResolvedValue({ affectedPaths: ["/Pictures/photo copy.png"], parentPath: "/Pictures" });
    ...
    vi.spyOn(HTMLCanvasElement.prototype, "toBlob").mockImplementation((callback) => callback(new Blob([new Uint8Array([1, 2, 3])], { type: "image/png" })));
    class EditorImage {
      naturalWidth = 4;
      naturalHeight = 2;
      decoding = "async";
  ...
  it("applies an image transform and supports Save as Copy and Save", async () => {
    await act(async () => {
      root.render(<GlobalPreviewDialog source={{ path: "/Pictures/photo.png", name: "photo.png", extension: "png" }} onClose={() => undefined} />);
    });
    await act(async () => { await Promise.resolve(); });
    const rotate = container.querySelector<HTMLButtonElement>('button[aria-label="Rotate clockwise"]');
    ...
    const saveCopy = [...container.querySelectorAll<HTMLButtonElement>('[role="menuitem"]')].find((button) => button.textContent?.includes("Save as copy"));
    await act(async () => { saveCopy?.click(); await new Promise((resolve) => setTimeout(resolve, 0)); });
    expect(savePreview).toHaveBeenCalledWith(expect.objectContaining({ path: "/Pictures/photo.png", bytes: [1, 2, 3], saveAsCopy: true }));

    await act(async () => saveMenu?.click());
    const save = [...container.querySelectorAll<HTMLButtonElement>('[role="menuitem"]')].find((button) => button.textContent === "Save");
    savePreview.mockResolvedValueOnce({ affectedPaths: ["/Pictures/photo.png"], parentPath: "/Pictures" });
    await act(async () => { save?.click(); await new Promise((resolve) => setTimeout(resolve, 0)); });
    expect(savePreview).toHaveBeenCalledWith(expect.objectContaining({ path: "/Pictures/photo.png", saveAsCopy: false }));
  });
  ...
  it("exposes the reference tool rail and commits text annotations", async () => {
    ...
    await act(async () => container.querySelector<HTMLButtonElement>('button[aria-label="Text"]')?.click());
    const canvas = container.querySelector("canvas");
    await act(async () => canvas?.dispatchEvent(new MouseEvent("pointerdown", { bubbles: true, clientX: 1, clientY: 1 })));
    await vi.waitFor(() => expect(context.fillText).toHaveBeenCalledWith("Text", expect.any(Number), expect.any(Number)));
  });

  it.each(["gif", "svg"])('opens .%s images directly in the editor', async (extension) => {
    await act(async () => {
      root.render(<GlobalPreviewDialog source={{ path: `/Pictures/photo.${extension}`, name: `photo.${extension}`, extension }} onClose={() => undefined} />);
    });
  ...
  it("shows editable tags as individual capsules", async () => {
    ...
    await act(async () => {
      root.render(<GlobalPreviewDialog source={{ path: "/Pictures/photo.png", name: "photo.png", extension: "png", tags: ["workflow", "visual"] }} onClose={() => undefined} onSaveMetadata={saveMetadata} />);
    });
    await act(async () => { await Promise.resolve(); });
    expect(container.querySelector('button[title="Edit workflow"]')).not.toBeNull();
    const remove = container.querySelector<HTMLButtonElement>('button[aria-label="Delete tag visual"]');
    await act(async () => remove?.click());
    const saveTags = [...container.querySelectorAll<HTMLButtonElement>("button")].find((button) => button.textContent?.includes("Save tags"));
    await act(async () => { saveTags?.click(); await Promise.resolve(); });
  ...
  it("copies the rendered edit to the system clipboard from the Save menu", async () => {
    ...
    const copy = [...container.querySelectorAll<HTMLButtonElement>('[role="menuitem"]')].find((button) => button.textContent?.includes("Copy to clipboard"));
    await act(async () => { copy?.click(); await new Promise((resolve) => setTimeout(resolve, 0)); });
  ...
  it("groups history, zoom, rotation, and reset in the top editor tray", async () => {
    ...
    const tray = container.querySelector("[data-editor-history-tray]");
    for (const label of ["Undo", "Redo", "Zoom out", "Zoom in", "Rotate clockwise", "Reset editor"]) {
      expect(tray?.querySelector(`button[aria-label="${label}"]`)).not.toBeNull();
    }
    expect(tray?.textContent).toContain("100%");
    expect(container.querySelector("footer")?.querySelector('button[aria-label="Zoom in"]')).toBeNull();
  });
});
