...
import type { DirectoryListing, DirectorySizeRecord, FileEntry } from "../../../api/types";
    ...
    return (
      <aside className={inspectorStyles.root}>
        <div className={inspectorStyles.emptyState}>
          <span className={inspectorStyles.emptyStateIcon} aria-hidden="true"><FileSearch size={22} /></span>
          <p className={inspectorStyles.emptyStateText}>Select a file to preview it and view its details.</p>
        </div>
      </aside>
    );
  }
  ...
  return (
    <aside className={inspectorStyles.root}>
      ...
      {!multiple && displayEntry && metadataError ? (
        <section className={inspectorStyles.detailsCard} aria-label="Stat metadata"><Detail label="Stat" value={metadataError} /></section>
      ) : null}
      {!multiple && displayEntry ? (
        <section className={inspectorStyles.editorCard} aria-label="Misty metadata">
          ...
          <label className={inspectorStyles.editorLabel}>
            ...
            <Textarea
              className={inspectorStyles.editorTextarea}
              value={commentsDraft}
          ...
          <div className={inspectorStyles.editorActions}>
            <Button size="sm" type="button" disabled={!metadataDirty} onClick={() => props.onSaveMetadata(displayEntry, parseTagDraft(tagsDraft), commentsDraft)}>
              Save Metadata
            </Button>
          </div>
        </section>
...
function sizeDetailValue(entry: FileEntry | null, directorySize: DirectorySizeRecord | undefined): ReactNode {
  if (!entry) return "-";
...
function DirectorySizeDots() {
  return (
    <span className={inspectorStyles.dots} aria-label="Calculating folder size">
      {[0, 1, 2].map((index) => (
        <span key={index} className={inspectorStyles.dot} style={{ animationDelay: `${index * 110}ms` }} />
      ))}
    </span>
  );
}
