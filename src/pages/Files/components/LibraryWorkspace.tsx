import { File, Film, FolderSearch, Images, Loader2, Plus, Search, Sparkles, Tag, X } from "lucide-react";
...
export function LibraryWorkspace(props: { paneId: string; workingDirectory: string; onOpenResult?: (result: SearchResult) => void | Promise<void> }) {
  const { loaded, library, error, load, setAssetTags } = useSmartLibraryStore(useShallow((state) => ({
    loaded: state.loaded,
  ...
  const openLibrarySettings = () => {
    useSettingsStore.getState().setActiveSection("search");
    navigate("/settings");
  };
  ...
  return (
    <section className="grid h-full min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] overflow-hidden bg-[var(--misty-surface)] text-[var(--misty-text)]">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--misty-border)] px-6 py-5">
        <div><h1 className="m-0 text-2xl font-bold tracking-[-0.03em]">Library</h1><p className="m-0 mt-1 text-sm text-[var(--misty-text-muted)]">AI-organized files, tags, saved views, and filters.</p></div>
        <button className="inline-flex h-10 items-center gap-2 rounded-xl bg-[var(--misty-primary)] px-4 text-sm font-bold text-[var(--misty-primary-foreground,#111)]" type="button" onClick={openLibrarySettings}><Sparkles size={16} />{pendingAnalysisCount > 0 ? `${pendingAnalysisCount.toLocaleString()} ready to analyze` : "Library settings"}</button>
      </header>
      ...
      <div className="min-h-0 overflow-auto p-6">
        {tab !== "media" && (!loaded ? <LibraryEmpty title="Loading your library…" text="Opening the private on-device catalog." /> : !library ? <LibraryEmpty title="No Library yet" text="Choose one folder and analyze a representative sample to enable tags and semantic organization." action={<button className="rounded-xl bg-white px-4 py-2 text-sm font-bold text-black" onClick={openLibrarySettings}>Set up Library</button>} /> : null)}
        {tab === "media" ? <MediaLibraryPanel /> : null}
        {library && tab === "library" ? <>
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><strong>{analyzed.length} analyzed files</strong><span className="ml-2 text-sm text-[var(--misty-text-muted)]">{library.displayName}</span></div><label className="flex h-10 min-w-[320px] items-center gap-2 rounded-xl border border-[var(--misty-border)] bg-[var(--misty-surface-2)] px-3"><Search size={16} /><input className="min-w-0 flex-1 border-0 bg-transparent outline-none" value={query} placeholder="Search subjects, descriptions, or tags" onChange={(event) => setQuery(event.target.value)} />{semanticSearching ? <Loader2 className="animate-spin text-[var(--misty-text-muted)]" size={15} /> : null}</label></div>
        ...
        {library && tab === "tags" ? <>
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h2 className="m-0 text-xl font-bold">Tags</h2><p className="m-0 mt-1 text-sm text-[var(--misty-text-muted)]">Mika adds tags during analysis. Open a file to review, remove, or add one.</p></div><label className="flex h-10 min-w-[260px] items-center gap-2 rounded-xl border border-[var(--misty-border)] bg-[var(--misty-surface-2)] px-3"><Search size={15} /><input className="min-w-0 flex-1 border-0 bg-transparent text-sm outline-none" value={tagQuery} placeholder="Search tags" aria-label="Search tags" onChange={(event) => setTagQuery(event.target.value)} />{tagQuery ? <button type="button" aria-label="Clear tag search" className="opacity-60 hover:opacity-100" onClick={() => setTagQuery("")}><X size={14} /></button> : null}</label></div>
...
function MediaLibraryPanel(){
  const {loaded,loading,indexingAssetId,progress,error,snapshot,scan,indexAsset,indexEligible}=useMediaSearchStore(useShallow((state)=>({loaded:state.loaded,loading:state.loading,indexingAssetId:state.indexingAssetId,progress:state.progress,error:state.error,snapshot:state.snapshot,scan:state.scan,indexAsset:state.indexAsset,indexEligible:state.indexEligible})));
  ...
  return <div className="grid gap-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="m-0 text-xl font-bold">Media Search</h2><p className="m-0 mt-1 max-w-2xl text-sm leading-6 text-[var(--misty-text-muted)]">Search spoken words and visual scenes at exact timestamps. Misty indexes only {snapshot?.rootPath??"~/Movies"}; original video files never leave your Mac.</p></div><div className="flex gap-2"><button type="button" disabled={loading||!!indexingAssetId} onClick={()=>void scan()} className="h-9 rounded-lg border border-[var(--misty-border)] px-3 text-sm font-semibold disabled:opacity-50">{loading?"Scanning…":"Scan Movies"}</button>{eligible.length?<button type="button" disabled={!!indexingAssetId} onClick={()=>void indexEligible()} className="h-9 rounded-lg bg-white px-3 text-sm font-bold text-black disabled:opacity-50">Index {eligible.length} {eligible.length===1?"file":"files"}</button>:null}</div></div>
    <div className="rounded-xl border border-[var(--misty-border)] bg-[var(--misty-surface-2)] p-4 text-sm"><strong>Private local preparation</strong><p className="m-0 mt-1 text-[var(--misty-text-muted)]">Audio is sent in 30-second compressed chunks with up to four 512px keyframes. Each file is limited to 120 minutes; there is no total library-minute limit.</p></div>
    ...
    {snapshot?.assets.length?<div className="grid gap-2">{snapshot.assets.map((asset)=>{const busy=indexingAssetId===asset.assetId;const current=asset.indexedFingerprint===asset.fingerprint;return <div key={asset.assetId} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-xl border border-[var(--misty-border)] p-3"><span className="grid size-11 place-items-center rounded-lg bg-black/20"><Film size={20}/></span><span className="min-w-0"><strong className="block truncate">{asset.name}</strong><small className="block text-[var(--misty-text-muted)]">{formatMediaDuration(asset.durationMs)} · {asset.mediaType} · {asset.status==="unsupported"?(asset.failureCode==="duration_limit_exceeded"?"Over 120-minute limit":"Unsupported"):busy?`Indexing ${Math.round(progress*100)}%`:current?"Searchable":"Ready to index"}</small>{busy?<span className="mt-2 block h-1.5 overflow-hidden rounded-full bg-white/10"><span className="block h-full bg-[var(--misty-primary)]" style={{width:`${Math.max(3,progress*100)}%`}}/></span>:null}</span><button type="button" disabled={busy||asset.status==="unsupported"||current||!!indexingAssetId} onClick={()=>void indexAsset(asset)} className="h-8 rounded-lg border border-[var(--misty-border)] px-3 text-xs font-bold disabled:opacity-40">{current?"Indexed":busy?"Indexing…":"Index"}</button></div>})}</div>:loaded&&!loading?<LibraryEmpty title="No media found" text="Add an audio or video file to ~/Movies, then scan again."/>:null}
  </div>;
}
...
function joinPath(root: string, relative: string) { return `${root.replace(/[\\/]+$/, "")}/${relative.replace(/^[\\/]+/, "")}`; }
