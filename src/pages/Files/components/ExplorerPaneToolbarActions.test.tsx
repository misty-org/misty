...
import { ExplorerPaneToolbarActions } from "./ExplorerPaneToolbarActions";
...
describe("ExplorerPaneToolbarActions", () => {
  ...
  it("exposes the active view and keeps view switching wired", () => {
    ...
    expect(toolbar?.getAttribute("aria-label")).toBe("View and file actions");
  ...
  it("anchors file actions in the accessible dropdown menu", () => {
    ...
    const moreButton = container.querySelector<HTMLButtonElement>('[aria-label="More file actions"]');
    act(() => {
      moreButton?.dispatchEvent(new MouseEvent("pointerdown", {
        bubbles: true,
        button: 0,
      }));
    });

    const copyPathItem = [...document.body.querySelectorAll<HTMLElement>('[role="menuitem"]')]
      .find((item) => item.textContent?.includes("Copy Current Path"));
