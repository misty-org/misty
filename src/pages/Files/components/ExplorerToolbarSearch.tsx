...
import {
  explorerCommandPaletteEntries,
  type ExplorerLocationResult,
...
export function ExplorerToolbarSearch(props: ExplorerToolbarSearchProps) {
  ...
  const locationFilter = commandMode || !searchMode
    ? ""
    : props.commandQuery.trim().toLowerCase();
  ...
  useEffect(() => {
    ...
    const semanticTimer = semanticFinished ? null : window.setTimeout(() => {
      void querySemanticExplorerSearch(
        query,
        { scope: "everything", currentPath: props.path, limit: 8 },
      ).then((results) => {
        if (canceled) return;
...
function LocationResults(props: {
  ...
  return (
    <>
      ...
      {props.locations.length > 0 && props.indexedResults.length > 0
        ? <CommandSeparator />
        : null}
