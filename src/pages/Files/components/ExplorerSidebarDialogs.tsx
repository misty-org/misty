...
import type { SavedSearchRule } from "../../../api/types";
import type {
      ...
      <AlertDialog open onOpenChange={(open) => { if (!open) props.onCancel(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Delete <strong className="font-medium text-foreground">{props.state.title}</strong>? This removes the saved layout, not any files.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={props.onConfirm}>
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        >
          ...
          <div className={fieldClass}>
            <Label className={fieldLabelClass} htmlFor="workspace-name">Name</Label>
            <Input
              id="workspace-name"
              autoFocus
              value={props.value}
          ...
          <DialogFooter>
            <Button variant="outline" type="button" onClick={props.onCancel}>Cancel</Button>
            <Button type="submit" disabled={!props.value.trim()}>Save</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
      ...
      <DialogContent className="max-h-[min(720px,calc(100vh-48px))] overflow-y-auto sm:max-w-2xl">
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>Build a reusable Explorer view from a query and structured rules.</DialogDescription>
          </DialogHeader>

          <div className="grid gap-3 sm:grid-cols-2">
            <div className={fieldClass}>
              <Label className={fieldLabelClass} htmlFor="smart-folder-name">Name</Label>
              <Input
            ...
            <div className={fieldClass}>
              <Label className={fieldLabelClass} htmlFor="smart-folder-match">Match</Label>
              <Select
                value={draft.matchMode}
                onValueChange={(value) => setDraft((current) => ({ ...current, matchMode: value === "any" ? "any" : "all" }))}
              >
                <SelectTrigger id="smart-folder-match"><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          <div className={fieldClass}>
            <Label className={fieldLabelClass} htmlFor="smart-folder-query">Query string</Label>
            <Input
          ...
          <div className="grid gap-2 rounded-lg bg-muted/40 p-3">
            ...
            {draft.rules.map((rule, index) => (
              <div className="grid gap-2 border-t border-border/70 pt-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)_minmax(0,1.2fr)_32px]" key={`rule:${index}`}>
                <Select value={rule.field} onValueChange={(value) => updateRule(index, { field: value })}>
                  ...
                  <SelectContent>
                    {smartFolderFields.map((field) => <SelectItem key={field.value} value={field.value}>{field.label}</SelectItem>)}
                  </SelectContent>
                </Select>
                <Select value={rule.operator} onValueChange={(value) => updateRule(index, { operator: value })}>
                  ...
                  <SelectContent>
                    {smartFolderOperators.map((operator) => <SelectItem key={operator.value} value={operator.value}>{operator.label}</SelectItem>)}
                  </SelectContent>
                </Select>
                <Input
                  value={rule.value}
                ...
                />
                <Button variant="ghost" size="icon" type="button" aria-label={`Remove rule ${index + 1}`} onClick={() => removeRule(index)}>
                  <X size={15} />
                </Button>
              </div>
            ))}
          ...
          {props.error ? <p className="m-0 text-sm text-destructive" role="alert">{props.error}</p> : null}
          <DialogFooter className="sm:justify-between">
            <div>
              {editing ? (
                <Button variant="destructive" type="button" onClick={() => void props.onDelete(draft.id)}>Delete</Button>
              ) : null}
            </div>
    ...
    <Dialog open onOpenChange={(open) => { if (!open) props.onCancel(); }}>
      <DialogContent className="sm:max-w-sm">
        ...
        >
          ...
          <div className={fieldClass}>
            <Label className={fieldLabelClass} htmlFor={inputId}>{props.label}</Label>
            <Input
              id={inputId}
              autoFocus
              value={props.value}
...
function smartFolderValuePlaceholder(field: string): string {
  switch (field) {
    case "path": return "/Users/name/Documents or remote:";
    case "kind": return "file or folder";
    case "extension": return "pdf";
    case "size": return "10MB";
    case "modified": return "2026-06-01";
    case "hidden": return "true or false";
    case "tag": return "work";
