...
const browserAudioMimeTypes: Record<string, string> = {
  ...
  ogg: "audio/ogg", oga: "audio/ogg", opus: "audio/ogg", weba: "audio/webm", aif: "audio/aiff", aiff: "audio/aiff",
};

const archivePreviewExtensions = new Set([
  "zip", "tar", "tgz", "tar.gz", "tbz", "tbz2", "tar.bz2", "txz", "tar.xz", "7z", "rar",
]);

const nativeImageThumbnailExtensions = new Set([
  "png", "jpg", "jpeg", "gif", "bmp", "webp", "tga", "hdr", "pic", "pbm", "pgm", "pnm", "ppm", "psd",
]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fileMetadataSnapshot(entry.path)
        .then((snapshot) => { if (active) setMetadata(snapshot); })
        .catch((error) => { if (active) setMetadataError(errorText(error)); });
...
export function useFolderPreview(entry: FileEntry | null, listing: DirectoryListing | null): {
  entries: FileEntry[];
  loading: boolean;
  error: string | null;
} {
  ...
  useEffect(() => {
    ...
    void explorerListDirectory({ path: entry.path, showHidden: false })
      .then((next) => { if (active) setEntries(folderPreviewEntries(next.entries)); })
      .catch((previewError) => { if (active) setError(errorText(previewError)); })
...
export function useFilePreview(entry: FileEntry | null, enabled = true): {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const settle = (request: Promise<LoadedInspectorPreview>) => {
        void request
          .then((loadedPreview) => { if (active) setPreview(loadedPreview); })
          .catch((error) => { if (active) setPreviewError(previewErrorTextForDisplay(error)); })
...
export function PreviewImage(props: { className: string; src: string; alt: string }) {
  return <img className={props.className} src={props.src} alt={props.alt} draggable={false} loading="lazy" decoding="async" />;
}
...
export function AudioPreview(props: { preview: LoadedInspectorPreview; title: string }) {
  return (
    <div className={inspectorStyles.audioPreview} aria-label={`Audio preview of ${props.title}`}>
      <div className={inspectorStyles.audioIcon} aria-hidden="true"><Music size={25} /></div>
    ...
    <div className={inspectorStyles.folderPreview} aria-label="Archive contents preview">
      ...
      <div className={inspectorStyles.folderPreviewList}>
        {entries.map((entry, index) => {
          ...
          return (
            <div className={inspectorStyles.folderPreviewItem} key={`${entry.path}-${index}`} title={entry.path}>
              ...
              <span className={inspectorStyles.folderPreviewSize}>{entry.isDir ? "" : formatArchiveEntrySize(entry)}</span>
            </div>
          );
        })}
      </div>
...
export function FolderContentsPreview(props: {
  ...
  if (props.entries.length === 0) {
    return <span className={inspectorStyles.previewStatus}>{props.error ?? "Folder is empty"}</span>;
  }
  return (
    <div className={inspectorStyles.folderPreview} aria-label="Directory contents preview">
      <div className={inspectorStyles.folderPreviewList}>
        {props.entries.map((entry) => (
          ...
          >
            <div className={inspectorStyles.folderPreviewThumb}><FileIcon entry={entry} size={21} variant="table" /></div>
            <span className={inspectorStyles.folderPreviewName} title={entry.name}>{entry.name}</span>
            <span className={inspectorStyles.folderPreviewSize}>{entry.kind === "folder" ? "" : formatBytes(entry.sizeBytes)}</span>
          </Button>
        ))}
      </div>
    </div>
...
async function loadNativeImagePreview(entry: FileEntry): Promise<LoadedInspectorPreview> {
  ...
  return { kind: "image", text: null, url: safeTauriAssetUrl(payload.path), mimeType: payload.mimeType };
}
...
function archivePreviewSupported(entry: FileEntry): boolean {
  return entry.kind !== "folder" && entry.kind !== "symlink" && archivePreviewExtensions.has(normalizedArchiveExtension(entry));
}
