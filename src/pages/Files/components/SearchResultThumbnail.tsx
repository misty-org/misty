...
export function SearchResultThumbnail(props: {
  ...
  useEffect(() => {
    ...
    if (!isPreviewableLocalImage(props.result) || failedThumbnails.has(cacheKey) || thumbnailCache.has(cacheKey)) return;
    let active = true;
  ...
  return (
    <span className={props.className} aria-hidden="true">
      {url ? <img className={props.imageClassName} src={url} alt="" /> : <FileIcon entry={entry} size={props.iconSize ?? 22} />}
    </span>
  );
}
...
function isPreviewableLocalImage(result: SearchResult): boolean {
  ...
  return entry.location.kind === "local"
    && entry.kind === "file"
    && ((entry.mimeType ?? "").toLocaleLowerCase().startsWith("image/") || imageExtensions.has(extension));
}
