...
import { edgeScrollDelta, pathContainsPath, physicalToClientPoint, selectDropCandidate } from "./geometry";
import { groupItemsByOperation, invalidTransferReason, operationForInternalDrop, storageIdForPath } from "./operations";
import type { ExplorerDragItem, ExplorerDragPayload } from "./types";
...
describe("Explorer drag payload and operation rules", () => {
  it("preserves a selected multi-item payload and excludes deleted entries", () => {
    ...
    expect(dragItemsForEntry(first, [first, second, deleted], new Set(["a", "b", "c"]))
      .map((item) => item.entryId)).toEqual(["a", "b"]);
    expect(dragItemsForEntry(second, [first, second], new Set(["a"])).map((item) => item.entryId)).toEqual(["b"]);
  ...
  it("derives stable storage identities and honors copy modifiers", () => {
    ...
    expect(storageIdForPath("/media/card/a.jpg", null, [{ id: "card", mountPath: "/media/card" }])).toBe("device:card");
  ...
  it("groups mixed selections and rejects self, parent no-op, and descendant drops", () => {
    ...
    expect(invalidTransferReason([dragItem("/Users/me/Folder", true)], "/Users/me/Folder/Child")).toMatch(/itself/);
  ...
  it("keeps invalid nested targets invalid instead of falling through", () => {
    const payload: ExplorerDragPayload = { sessionId: "drag", origin: "internal", items: [dragItem("/tmp/a.txt", false)] };
    expect(transferDropAcceptance(payload, "/tmp/file.txt", { folder: false })).toMatchObject({ valid: false });
    expect(transferDropAcceptance(payload, "/tmp/readonly", { writable: false })).toMatchObject({ valid: false });
  });
});

function dragItem(path: string, isDirectory: boolean, storageId = "local:default"): ExplorerDragItem {
