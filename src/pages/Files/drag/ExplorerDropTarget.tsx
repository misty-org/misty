...
export function ExplorerDropTarget(props: {
  ...
  const zone = useMemo<ExplorerDropZoneSpec>(() => createExplorerDropTargetSpec({
    id: props.id,
    path: props.path,
    kind,
    paneId: props.paneId,
  ...
  }), [kind, props.id, props.onSpringLoad, props.paneId, props.path, props.remoteName, props.springLoad]);
  return <Droppable className="contents" zone={zone}>{props.children}</Droppable>;
}
...
export function createExplorerDropTargetSpec(options: {
  ...
  return {
    ...
    onDrop: (payload, modifiers) => dropOnTarget(kind, payload, options.path, options.remoteName, options.paneId, modifiers),
...
function acceptanceForTarget(kind: TargetKind, payload: ExplorerDragPayload, destination: string) {
  if (kind === "invalid") return { valid: false, label: "Not a drop destination", action: "invalid" as const };
  if (kind === "trash") return { valid: true, label: "Move to Trash", action: "trash" as const };
  if (kind === "library") {
    if (payload.items.length > 500) return { valid: false, label: "Library accepts up to 500 files", action: "invalid" as const };
    if (payload.items.some((item) => item.isDirectory)) return { valid: false, label: "Library accepts files only", action: "invalid" as const };
...
async function dropOnTarget(
  ...
  if (kind === "trash") {
    await explorerQueueDeleteItems({ paths: payload.items.map((item) => item.path), permanent: false });
    explorer.pushNotification(`Queued ${payload.items.length} item${payload.items.length === 1 ? "" : "s"} for Trash`, "success");
    refreshAllPanes();
    return;
  }
  if (kind === "library") {
  ...
  if (payload.origin === "external") {
    await explorer.dropExternalPaths(targetPaneId, payload.items.map((item) => item.path), destination);
    return;
  }
  const groups = groupItemsByOperation(payload.items, storageIdForPath(destination, remoteName), modifiers.copyRequested);
...
async function resolveLibraryFiles(payload: ExplorerDragPayload): Promise<string[]> {
  const directoryFlags = payload.origin === "external"
    ? await Promise.all(payload.items.map((item) => explorerPathIsDirectory(item.path)))
    : payload.items.map((item) => item.isDirectory);
  if (directoryFlags.some(Boolean)) throw new Error("Folders cannot be added to Library by dropping.");
  const remote = payload.items.filter((item) => item.location && item.location.kind !== "local");
  const local = payload.items.filter((item) => !item.location || item.location.kind === "local").map((item) => item.path);
  ...
  if (prepared.skipped.length) {
    useExplorerStore.getState().pushNotification(`Skipped ${prepared.skipped.length} remote Library item(s).`, "error");
  }
