...
import type { ExplorerDragItem, ExplorerDropZoneSpec } from "./types";

const mocks = vi.hoisted(() => ({
  cancelPreparation: vi.fn().mockResolvedValue(undefined),
  prepareItems: vi.fn().mockResolvedValue({ items: [], skipped: [] }),
  startDrag: vi.fn().mockImplementation(async (_options, callback) => callback({ result: "Dropped" })),
  unlistenDrag: vi.fn(),
  unlistenScale: vi.fn(),
...
vi.mock("../../../stores/useExplorerStore", () => ({
  useExplorerStore: { getState: () => ({ panes: {}, pushNotification: vi.fn(), refreshPane: vi.fn() }) },
}));
...
describe("ExplorerDragProvider", () => {
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(document, "elementsFromPoint", { configurable: true, value: () => elementAtPoint ? [elementAtPoint] : [] });
  ...
  it("starts after six pixels and dispatches a pointer drop", async () => {
    ...
    await act(async () => { window.dispatchEvent(pointer("pointerup", 7, 0)); await Promise.resolve(); });

    expect(onDrop).toHaveBeenCalledWith(expect.objectContaining({ origin: "internal", items: [item] }), expect.any(Object));
    expect(container.querySelector("[data-explorer-drag-interaction-shield]")).toBeNull();
  });
  ...
  it("preserves dragged items while spring-loading an invalid drop location for navigation", async () => {
    ...
    try {
      await act(async () => root.render(
        <ExplorerDragProvider><NavigationHarness onDrop={onDrop} invalidBeforeNavigation /></ExplorerDragProvider>,
      ));
      const source = container.querySelector<HTMLElement>("[data-source]")!;
      elementAtPoint = container.querySelector<HTMLElement>("[data-zone]");
      await act(async () => source.dispatchEvent(pointer("pointerdown", 0, 0)));
      await act(async () => window.dispatchEvent(pointer("pointermove", 8, 0)));
      expect(container.querySelector('[role="status"]')?.textContent).not.toContain("Cannot drop here");
      await act(async () => { await Promise.resolve(); await vi.advanceTimersByTimeAsync(701); });
      expect(container.querySelector("[data-source]")).toBeNull();

      await act(async () => window.dispatchEvent(pointer("lostpointercapture", 8, 0)));
      expect(container.textContent).toContain("photo.jpg");
      await act(async () => { window.dispatchEvent(pointer("pointerup", 8, 0)); await Promise.resolve(); });
      expect(onDrop).toHaveBeenCalledWith(expect.objectContaining({ items: [item] }), expect.any(Object));
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it("converts and dispatches Tauri Finder ingress and disposes listeners", async () => {
    ...
    await act(async () => mocks.dragListener?.({ payload: { type: "enter", paths: ["/tmp/in.txt"], position: { x: 400, y: 200 } } }));
    expect(container.querySelector<HTMLElement>('[role="status"]')?.style.left).toBe("214px");
    expect(container.querySelector("[data-explorer-drag-interaction-shield]")).not.toBeNull();
    await act(async () => { mocks.dragListener?.({ payload: { type: "drop", paths: ["/tmp/in.txt"], position: { x: 400, y: 200 } } }); await Promise.resolve(); });
    expect(onDrop).toHaveBeenCalledWith(expect.objectContaining({ origin: "external" }), expect.any(Object));
    expect(container.querySelector("[data-explorer-drag-interaction-shield]")).toBeNull();
    await act(async () => root.unmount());
  ...
  it("hands a held local drag to the native plugin on webview exit", async () => {
    ...
    await act(async () => { window.dispatchEvent(pointer("pointerout", 9, 0, { shiftKey: true })); await Promise.resolve(); });
    await vi.waitFor(() => expect(mocks.startDrag).toHaveBeenCalledOnce());
    expect(mocks.startDrag).toHaveBeenCalledWith(expect.objectContaining({ item: [item.path], mode: "move" }), expect.any(Function));
  });
  ...
  it("cancels remote preparation when the pointer is released before staging finishes", async () => {
    ...
    mocks.prepareItems.mockReturnValueOnce(new Promise((resolve) => { finishPreparation = resolve; }));
    const remoteItem: ExplorerDragItem = {
      ...item,
      path: "/mnt/misty/work/photo.jpg",
      storageId: "remote:work",
      location: { kind: "remote", providerType: "drive", remoteName: "work", remotePath: "/photo.jpg" },
    };
    ...
    await act(async () => { finishPreparation({ items: [{ localPath: "/tmp/photo.jpg" }], skipped: [] }); await Promise.resolve(); });
  ...
  async function renderHarness(onDrop: ExplorerDropZoneSpec["onDrop"], items = [item]) {
    ...
    await act(async () => root.render(<ExplorerDragProvider><Harness zone={zone} items={items} /></ExplorerDragProvider>));
  }
});

function Harness({ zone, items }: { zone: ExplorerDropZoneSpec; items: ExplorerDragItem[] }) {
  const source = useExplorerDragSource(items);
  return <><div data-source onPointerDown={source.onPointerDown}>Source</div><Droppable data-zone zone={zone}>Target</Droppable></>;
}

function NavigationHarness({ onDrop, invalidBeforeNavigation = false }: {
  ...
  const zone = useMemo<ExplorerDropZoneSpec>(() => ({
    ...
    priority: 10,
    accepts: () => invalidBeforeNavigation && sourceVisible
  ...
  return <>{sourceVisible ? <div data-source onPointerDown={source.onPointerDown}>Source</div> : null}<Droppable data-zone zone={zone}>Target</Droppable></>;
}
