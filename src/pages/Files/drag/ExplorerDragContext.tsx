...
import { explorerCancelDragPreparation, explorerPrepareDragItems } from "../../../api/misty";
import { getAppliedAppZoom } from "../../../shared/hooks/useAppZoom";
import { hasTauriInternals } from "../../../shared/tauri";
import { useExplorerStore } from "../../../stores/useExplorerStore";
import { dragDistance, edgeScrollDelta, physicalToClientPoint, selectDropCandidate } from "./geometry";
...
export function ExplorerDragProvider(props: { children: ReactNode }) {
  ...
  const modifiersRef = useRef<ExplorerDragModifiers>({ copyRequested: false, moveRequested: false });
  ...
  const cancel = useCallback((message?: string) => {
    ...
    if (prepared && !prepared.settled) void explorerCancelDragPreparation(prepared.sessionId).catch(() => undefined);
  ...
  const resolveTargetAt = useCallback((x: number, y: number) => {
    ...
    const selected = selectDropCandidate(candidates.map((candidate) => ({
      ...candidate,
  ...
  const registerZone = useCallback((element: HTMLElement, spec: ExplorerDropZoneSpec) => {
    ...
    return () => {
      ...
      if (element.dataset.explorerDropZoneId === spec.id) delete element.dataset.explorerDropZoneId;
      delete element.dataset.explorerDropActive;
    };
  }, [scheduleHitTest]);

  const beginPreparation = useCallback((payload: ExplorerDragPayload) => {
    const remoteItems = payload.items.filter((item) => item.location && item.location.kind !== "local");
    const localPaths = payload.items.filter((item) => !item.location || item.location.kind === "local").map((item) => item.path);
    ...
    if (remoteItems.length > 0) {
      preparation.promise = explorerPrepareDragItems({
        ...
        if (result.skipped.length > 0) {
          useExplorerStore.getState().pushNotification(`Skipped ${result.skipped.length} item(s) while preparing drag-out.`, "error");
        }
  ...
  const beginInternal = useCallback((armed: ArmedDrag, point: { x: number; y: number }) => {
  ...
  const beginNativeEgress = useCallback(async () => {
    ...
    try {
      ...
      const allLocal = current.payload.items.every((item) => !item.location || item.location.kind === "local");
      ...
      await startDrag({ item: paths, icon: dragPreviewDataUrl(current.payload.items), mode }, (event) => {
        if (event.result === "Dropped" && mode === "move") refreshExplorerPanes();
        cancel();
        window.setTimeout(() => { nativeEgressRef.current = false; }, 250);
      });
    } catch (error) {
  ...
  const armSource = useCallback((items: ExplorerDragItem[], event: ReactPointerEvent<HTMLElement>) => {
    if (event.button !== 0 || event.isPrimary === false || items.length === 0 || isInteractiveDragTarget(event.target)) return;
  ...
  useEffect(() => {
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (armed && stateRef.current.phase === "idle" && dragDistance(armed.start, point) >= DRAG_THRESHOLD) {
    ...
    const onPointerOut = (event: PointerEvent) => {
      if (event.relatedTarget == null && pointerHeldRef.current && stateRef.current.phase === "internal") void beginNativeEgress();
    };
    const onKeyDown = (event: KeyboardEvent) => {
      modifiersRef.current = modifiersFromEvent(event);
      if (event.key === "Escape" && stateRef.current.phase !== "idle") cancel("Drag canceled.");
    };
    const onKeyUp = (event: KeyboardEvent) => { modifiersRef.current = modifiersFromEvent(event); };
  ...
  useEffect(() => {
    ...
    void Promise.all([import("@tauri-apps/api/webview"), import("@tauri-apps/api/window")]).then(async ([webviewApi, windowApi]) => {
      if (disposed) return;
      const appWindow = windowApi.getCurrentWindow();
      scaleFactorRef.current = await appWindow.scaleFactor();
      unlisteners.push(await appWindow.onScaleChanged(({ payload }) => { scaleFactorRef.current = payload.scaleFactor; }));
      unlisteners.push(await webviewApi.getCurrentWebview().onDragDropEvent(({ payload }) => {
        if (nativeEgressRef.current || stateRef.current.phase === "native-egress") return;
        if (payload.type === "leave") return cancel();
        const point = physicalToClientPoint(payload.position, scaleFactorRef.current, getAppliedAppZoom());
        if (payload.type === "enter") {
          const dragPayload: ExplorerDragPayload = {
            sessionId: crypto.randomUUID(),
            origin: "external",
            items: payload.paths.map((path) => ({ path, name: fileName(path), isDirectory: false })),
          };
          stateRef.current = { ...initialState, phase: "external", payload: dragPayload, pointer: point };
          setState(stateRef.current);
          setWebviewDragActive(true);
        }
        scheduleHitTest(point.x, point.y);
  ...
  const value = useMemo<ExplorerDragContextValue>(() => ({ state, armSource, registerZone, cancel }), [armSource, cancel, registerZone, state]);
  return (
    <ExplorerDragContext.Provider value={value}>
      ...
      {state.payload && state.phase !== "native-egress" ? (
        <div
          aria-hidden="true"
          className="pointer-events-auto fixed inset-0 z-[2147483400] cursor-grabbing bg-transparent"
          data-explorer-drag-interaction-shield="true"
        />
      ) : null}
      <ExplorerDragPreview state={state} />
      <div className="sr-only" role="status" aria-live="polite">{dragAnnouncement(state)}</div>
...
export function useExplorerDragSource(items: ExplorerDragItem[]) {
  ...
  const dragging = Boolean(context?.state.payload?.origin === "internal"
    && items.some((item) => context.state.payload?.items.some((dragged) => dragged.entryId === item.entryId)));
  return useMemo(() => ({
    onPointerDown: context ? (event: ReactPointerEvent<HTMLElement>) => context.armSource(items, event) : undefined,
    dragging,
...
export function useExplorerDropZone(spec: ExplorerDropZoneSpec) {
  ...
  useLayoutEffect(() => element && registerZone ? registerZone(element, spec) : undefined, [element, registerZone, spec]);
...
export function Droppable(props: HTMLAttributes<HTMLDivElement> & { zone: ExplorerDropZoneSpec; children: ReactNode }) {
  const { zone, children, className, ...rest } = props;
  const drop = useExplorerDropZone(zone);
  return <div {...rest} ref={drop.ref} className={className}>{children}</div>;
}
...
function modifiersFromEvent(event: Pick<PointerEvent | KeyboardEvent | ReactPointerEvent, "altKey" | "ctrlKey" | "shiftKey">): ExplorerDragModifiers {
  return { copyRequested: event.altKey || event.ctrlKey, moveRequested: event.shiftKey };
}

function isInteractiveDragTarget(target: EventTarget | null): boolean {
  return target instanceof Element && Boolean(target.closest("button,input,textarea,select,a,[contenteditable=true],.inline-name-editor"));
}
...
function autoScrollAt(x: number, y: number): void {
  ...
  const container = document.elementsFromPoint(x, y)
...
function dragPreviewDataUrl(items: ExplorerDragItem[]): string {
  ...
  context.fillText(items.length > 1 ? `${items.length} items` : items[0]?.name ?? "Item", 54, 39, 62);
