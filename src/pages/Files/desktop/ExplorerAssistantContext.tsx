    ...
    <div className="grid gap-3">
      <ContextSection label="Status">
        <div className="grid grid-cols-[20px_minmax(0,1fr)] items-center gap-2">
          <span className={`size-2.5 rounded-full ${configured ? "bg-emerald-500" : "bg-muted-foreground"}`} aria-hidden="true" />
          <span className="min-w-0 truncate text-sm font-medium">
            {statusLabel}{statusMeta ? <small className="ml-2 font-normal text-muted-foreground">{statusMeta}</small> : null}
          </span>
        </div>
      </ContextSection>
  ...
  return (
    <div className="grid grid-cols-[20px_minmax(0,1fr)_32px] items-center gap-2">
      ...
      <span className="grid min-w-0 gap-0.5">
        <strong className="truncate text-sm font-medium" title={props.value}>{props.value}</strong>
        {props.detail ? <small className="truncate text-xs text-muted-foreground" title={props.detail}>{props.detail}</small> : null}
      </span>
      <Button
        variant="ghost"
        size="icon"
