...
export const ExplorerMikaPanel = memo(function ExplorerMikaPanel(props: ExplorerMikaPanelProps) {
  ...
  const {
    status, mode, messages, plans, toolApprovals, error,
    refreshStatus, setMode, sendPrompt, abortPrompt, approvePlan, approveToolRequest,
  } = useMikaSessionStore(useShallow((state) => ({
    status: state.status,
    mode: state.mode,
  ...
  return (
    <aside className={mikaPanelClass(props.surface)} aria-label="Mika Assistant">
      <header
        className={cx(assistantPanelStyles.header, assistantPanelStyles.mikaHeader, chatWindow && "cursor-grab active:cursor-grabbing [&_button]:cursor-pointer")}
        onPointerDown={(event) => {
      ...
      >
        <span className={cx(assistantPanelStyles.headerTitle, assistantPanelStyles.mikaHeaderTitle)}>
          Mika {running ? <small className={assistantPanelStyles.runningBadge}>Running</small> : null}
        </span>
        <div className={assistantPanelStyles.headerActions}>
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="icon" className={assistantPanelStyles.headerButton} type="button" aria-label="Mika context">
                <Info size={17} />
              </Button>
            </PopoverTrigger>
            <PopoverContent align="end" className="w-[min(340px,calc(100vw-32px))] p-3">
              <MikaContextContent status={status} workingDirectory={workingDirectory} selectedPaths={selectedPaths} />
            </PopoverContent>
          </Popover>
          <Button
            variant="ghost"
      ...
      <div className={assistantPanelStyles.mikaBody}>
        {error ? <div className={`${assistantPanelStyles.status} ${assistantPanelStyles.mikaStatus}`}><p className={assistantPanelStyles.errorText}>{error}</p></div> : null}
        <div ref={logRef} className={`${assistantPanelStyles.log} ${assistantPanelStyles.mikaLog}`} aria-live="polite">
          {messages.length === 0 ? (chatWindow ? null : <MikaEmptyState />) : messages.map((message) => (
            <AssistantMessage
              key={message.id}
              message={message}
              running={running}
        ...
        <form
          className={cx(assistantPanelStyles.composer, assistantPanelStyles.mikaComposer, chatWindow && "relative z-10 border-t-0 pt-0")}
          onSubmit={(event) => {
          ...
          <div className={chatWindow ? "relative z-10 min-w-0 rounded-xl bg-muted/60" : "contents"}>
            <Textarea
              className={cx(
                ...
                chatWindow && "w-full min-h-[60px] max-h-[200px] resize-none rounded-none border-0 bg-transparent px-4 pb-2 pt-3.5 shadow-none focus-visible:ring-0",
              )}
              ref={promptRef}
              value={prompt}
              rows={chatWindow ? 1 : 3}
            ...
            {chatWindow ? (
              ...
              <div className={`${assistantPanelStyles.composerActions} ${assistantPanelStyles.mikaComposerActions}`}>
                <Select value={mode === "full" ? "auto" : mode} onValueChange={(value) => setMode(value as Parameters<typeof setMode>[0])}>
                  <SelectTrigger className="h-9 w-32" aria-label="Mika mode"><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="ask">Ask first</SelectItem><SelectItem value="auto">Full access</SelectItem></SelectContent>
                </Select>
                {running ? (
                  <Button type="button" title="Cancel the active Mika gateway request." onClick={abortPrompt}>Stop</Button>
                ) : (
                  <Button type="submit" disabled={!configured || !prompt.trim()}><ArrowUp size={17} /> Send</Button>
                )}
              </div>
            )}
          </div>
        </form>
        {chatWindow ? null : <footer className={assistantPanelStyles.mikaFooter}><Sparkles size={15} />Mika can make mistakes. Review file plans before applying them.</footer>}
      </div>
    </aside>
  );
});
  ...
  return (
    <div className="relative z-10 flex h-[50px] min-w-0 items-center justify-between gap-3 px-3 pb-2">
      <div className="flex min-w-0 items-center gap-1">
        <Button variant="ghost" size="icon" className="size-9 rounded-full" disabled title="Attachments coming soon" type="button" aria-label="Add context"><Plus size={19} /></Button>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className={cx("h-9 min-w-[124px] justify-start gap-2 px-2.5", props.mode !== "ask" && "text-destructive")} type="button">
              <ShieldAlert size={17} />
              <span className="min-w-0 flex-1 truncate text-left">{props.mode === "ask" ? "Ask first" : "Full access"}</span>
              <ChevronDown size={15} />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" side="top" className="w-40">
            <DropdownMenuRadioGroup value={props.mode === "ask" ? "ask" : "auto"} onValueChange={(value) => props.setMode(value === "auto" ? "auto" : "ask")}>
              <DropdownMenuRadioItem value="ask">Ask first</DropdownMenuRadioItem>
              <DropdownMenuRadioItem value="auto" className="text-destructive focus:text-destructive">Full access</DropdownMenuRadioItem>
            </DropdownMenuRadioGroup>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
      <div className="flex shrink-0 items-center gap-1">
        <span className="max-w-28 truncate px-1 text-sm text-muted-foreground" title={props.modelName}>{props.modelName}</span>
        <Button variant="ghost" size="icon" className="size-9 rounded-full" disabled title="Voice input coming soon" type="button" aria-label="Voice input"><Mic size={17} /></Button>
        {props.running ? (
          <Button size="icon" className="size-9 rounded-full" type="button" aria-label="Stop Mika" title="Cancel the active Mika request" onClick={props.abortPrompt}><X size={17} /></Button>
        ) : (
          <Button size="icon" className="size-9 rounded-full" type="submit" aria-label="Send to Mika" disabled={!props.configured || !props.prompt.trim()}><ArrowUp size={19} /></Button>
        )}
      </div>
    </div>
  );
