const compareStyles = {
  ...
  diffGrid: "grid max-h-[min(420px,46vh)] grid-cols-[minmax(0,1fr)_minmax(0,1fr)] overflow-auto rounded-md border border-border bg-background text-xs max-[760px]:grid-cols-1",
  diffPane: "grid min-w-0 content-start border-r border-border last:border-r-0 max-[760px]:border-r-0 max-[760px]:border-b max-[760px]:last:border-b-0",
  diffPaneTitle: "sticky top-0 z-[1] border-b border-border bg-muted px-2.5 py-1.5 font-medium text-foreground",
  ...
  imageGrid: "grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3 rounded-lg bg-muted/30 p-2.5 max-[760px]:grid-cols-1",
  ...
  imageMeta: "min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-muted-foreground",
  rowList: "grid max-h-[min(340px,44vh)] overflow-auto rounded-lg bg-muted/30 p-1.5",
  row: "grid grid-cols-[minmax(0,1fr)_100px_100px_110px] items-center gap-2 rounded-md px-2 py-2 text-sm text-foreground hover:bg-muted max-[720px]:grid-cols-1",
  ...
  error: "rounded-lg border border-destructive/25 bg-destructive/10 px-3 py-2 text-sm text-destructive",
} as const;
