export const explorerShellStyles = {
  workspaceBase:
    "relative grid h-full min-h-0 overflow-hidden grid-cols-[minmax(0,1fr)] grid-rows-[minmax(0,1fr)] bg-[var(--misty-bg)] max-[980px]:grid-cols-1 max-[720px]:h-full",
  workspaceMikaOpen:
  ...
  bottomBar:
    "grid min-h-[22px] min-w-0 grid-cols-[auto_auto] items-center justify-between gap-2 border-t border-[var(--misty-border-soft)] bg-[var(--misty-bg-soft)] px-2 max-[720px]:hidden",
  bottomBarGroup:
    "grid grid-flow-col auto-cols-max items-center gap-1",
  bottomButton:
    "grid h-5 w-[22px] place-items-center rounded border-0 bg-transparent p-0 text-[var(--misty-text-subtle)] hover:bg-[var(--misty-surface-hover)] hover:text-[var(--misty-text)]",
  bottomButtonSelected: "bg-[var(--misty-surface-selected)] text-[var(--misty-text)]",
  paneActionButton:
  ...
  paneHeaderActionSection:
    "flex h-7 flex-none items-center gap-px overflow-hidden rounded-lg border border-[var(--misty-border)] bg-[var(--misty-surface-2)] p-0.5",
} as const;
