...
export const ExplorerChatOverlay = memo(function ExplorerChatOverlay() {
  ...
  const {
    ...
    refreshStatus, setMode, sendPrompt, abortPrompt, clearConversation, approvePlan, approveToolRequest,
  ...
  return (
    <section className={assistantPanelStyles.chatOverlay} aria-label="Explorer chat">
      <header className={assistantPanelStyles.header}>
        ...
        <Button variant="ghost" size="icon" className={assistantPanelStyles.headerButton} type="button" aria-label="Close chat" onClick={closeOverlay}>
          <X size={16} />
        </Button>
      </header>
      <div className={assistantPanelStyles.chatBody}>
        ...
        <div ref={logRef} className={`${assistantPanelStyles.log} ${assistantPanelStyles.chatLog}`} aria-live="polite">
          {messages.length === 0 ? (
        ...
        >
          ...
          <div className={assistantPanelStyles.composerActions}>
            <Select value={mode === "full" ? "auto" : mode} onValueChange={(value) => setMode(value as Parameters<typeof setMode>[0])}>
              <SelectTrigger className="h-8 w-28" aria-label="Mika mode"><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="ask">Ask first</SelectItem><SelectItem value="auto">Full access</SelectItem></SelectContent>
            </Select>
            <Button variant="outline" size="sm" type="button" onClick={openPanel}>Open Panel</Button>
            {running ? (
              <Button size="sm" type="button" title="Cancel the active Mika gateway request." onClick={abortPrompt}>Stop</Button>
            ) : (
              <Button size="sm" type="submit" disabled={!configured || !prompt.trim()}>Send</Button>
            )}
          </div>
        </form>
      </div>
