...
import type { ResolvedMediaAsset, ResolvedSmartLibraryAsset, SearchResult } from "../../../api/types";
import type { MediaSearchHit } from "../../../stores/mediaSearchServerApi";
import type { SemanticSearchHit } from "../../../stores/smartLibraryServerApi";
import { isPathWithin, mediaHitsToSearchResults, mergeHybridSearchResults, semanticHitsToSearchResults } from "./globalSearch";
...
describe("global semantic Explorer search", () => {
  ...
  it("shows specific generated entities before generic ranker reasons", () => {
    ...
    hit.metadata = { characters: ["Pikachu"], applications: ["file manager"], primarySubject: "Pikachu desktop" };
    const [result] = semanticHitsToSearchResults([hit], [resolved("asset", "/disk/pikachu.png", "local")]);
  ...
  it("honors exact current-folder boundaries and source scopes", () => {
    ...
    expect(semanticHitsToSearchResults([hit], [inFolder], { scope: "current", currentPath: "/disk/photos" })).toHaveLength(1);
    expect(semanticHitsToSearchResults([hit], [prefixCollision], { scope: "current", currentPath: "/disk/photos" })).toHaveLength(0);
  ...
  it("turns timestamped spoken and visual moments into separate actionable results",()=>{
    const location:ResolvedMediaAsset={assetId:"media_1",path:"/Users/test/Movies/movie.mp4",name:"movie.mp4",mediaType:"video",durationMs:120_000};
    const hits:MediaSearchHit[]=[{segmentId:"spoken_1",assetId:"media_1",mediaType:"video",kind:"spoken",content:"forty eight hours",transcript:"forty eight hours",visualDescription:"",startMs:180,endMs:1980,visibleText:[],score:.95,semanticScore:.9,lexicalScore:1},{segmentId:"visual_1",assetId:"media_1",mediaType:"video",kind:"visual",content:"red sports car",transcript:"",visualDescription:"A red sports car at night",startMs:50_000,endMs:60_000,visibleText:[],score:.9,semanticScore:.85,lexicalScore:1}];
    const results=mediaHitsToSearchResults(hits,[location]);
    expect(results).toHaveLength(2);expect(results[0].match).toMatchObject({mediaSegmentId:"spoken_1",mediaStartMs:180,mediaMatchKind:"spoken"});expect(results[1].match).toMatchObject({mediaSegmentId:"visual_1",mediaStartMs:50_000,mediaMatchKind:"visual"});
    expect(mergeHybridSearchResults([],results,10)).toHaveLength(2);
  });
});

function semanticHit(assetId: string): SemanticSearchHit {
  return { assetId, description: "match", tags: [], suggestedCollections: [], score: 0.9, semanticScore: 0.9 };
}

function resolved(assetId: string, path: string, sourceKind: "local" | "cloud"): ResolvedSmartLibraryAsset {
