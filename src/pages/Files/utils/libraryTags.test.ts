...
import { aggregateLibraryTags, DEFAULT_ASSET_TAG_LIMIT, DEFAULT_LIBRARY_TAG_LIMIT, tagsWithout, visibleAssetTags, visibleLibraryTags } from "./libraryTags";
...
describe("library tag disclosure", () => {
  ...
  it("searches hidden tags case-insensitively and keeps an active hidden tag visible", () => {
    const tags = Array.from({ length: 14 }, (_, index) => ({ name: `Tag ${index}`, count: 14 - index }));
    expect(visibleLibraryTags(tags, { query: "TAG 13", expanded: false, selectedTag: null }).map((tag) => tag.name)).toEqual(["Tag 13"]);
    expect(visibleLibraryTags(tags, { query: "", expanded: false, selectedTag: "Tag 13" }).slice(-1)[0]?.name).toBe("Tag 13");
  });
