...
export async function querySemanticExplorerSearch(
  ...
  if (nonWhitespaceLength(trimmed) < semanticQueryMinimumCharacters || options.includeFiles === false) return [];
  ...
  const request = (async () => {
    ...
    cacheable = libraryResponse.status === "fulfilled"
      && mediaResponse.status === "fulfilled"
      && (libraryHits.length === 0 || locations.length > 0)
      && (mediaHits.length === 0 || mediaLocations.length > 0);
    return [...semanticHitsToSearchResults(libraryHits, locations, options), ...mediaHitsToSearchResults(mediaHits, mediaLocations, options)]
  ...
  try {
    ...
    if (cacheable) {
      ...
      while (semanticCache.size > semanticCacheMaxEntries) semanticCache.delete(semanticCache.keys().next().value as string);
    }
    return results;
  } finally {
...
export function mergeHybridSearchResults(
  ...
  const records = new Map<string, { result: SearchResult; fused: number; indexed: boolean; semantic: boolean }>();
...
function addRankedResults(
  records: Map<string, { result: SearchResult; fused: number; indexed: boolean; semantic: boolean }>,
  ...
  results.forEach((candidate, index) => {
    ...
    if (candidate.match) existing.result = { ...existing.result, match: mergeMatch(existing.result.match, candidate.match) };
  });
}

async function resolveMediaLocations(hits: MediaSearchHit[]): Promise<ResolvedMediaAsset[]> {
  try { return await mediaSearchResolveAssets(uniqueStrings(hits.map((hit)=>hit.assetId)).slice(0,100)); }
  catch { return []; }
}

export function mediaHitsToSearchResults(hits:MediaSearchHit[],locations:ResolvedMediaAsset[],options:ExplorerSearchOptions={}):SearchResult[]{
  const byId=new Map(locations.map((item)=>[item.assetId,item]));const now=Date.now();
  return hits.flatMap((hit)=>{const resolved=byId.get(hit.assetId);if(!resolved||!pathAllowed(resolved.path,"local",options))return[];const extension=extensionOf(resolved.name);return [{entry:{id:resolved.path,name:resolved.name,path:resolved.path,extension,mimeType:hit.mediaType==="video"?"video/mp4":"audio/mpeg",remoteModified:null,kind:"file",sizeBytes:null,modifiedMs:null,createdMs:null,readonly:false,hidden:false,isDeleted:false,location:{kind:"local",providerType:null,remoteName:null,remotePath:null}},score:finiteScore(hit.score,hit.semanticScore),sourceKind:"local",indexedAtMs:now,match:{kind:(hit.lexicalScore??0)>0?"hybrid":"semantic",semanticScore:hit.semanticScore,lexicalScore:hit.lexicalScore,reasons:[formatTimestamp(hit.startMs),hit.kind==="spoken"?"Spoken audio":"Visual scene"],description:hit.kind==="spoken"?hit.transcript:hit.visualDescription,tags:hit.visibleText??[],assetKind:hit.mediaType,mediaSegmentId:hit.segmentId,mediaType:hit.mediaType,mediaStartMs:hit.startMs,mediaEndMs:hit.endMs,mediaMatchKind:hit.kind,transcript:hit.transcript,visualDescription:hit.visualDescription}} satisfies SearchResult];});
}

function formatTimestamp(ms:number):string{const seconds=Math.max(0,Math.floor(ms/1000));return `${Math.floor(seconds/60)}:${String(seconds%60).padStart(2,"0")}`;}

function mergeMatch(current: SearchResultMatch | undefined, incoming: SearchResultMatch): SearchResultMatch {
...
async function resolveSemanticLocations(hits: SemanticSearchHit[]): Promise<ResolvedSmartLibraryAsset[]> {
...
function resolvedFromLegacyAsset(rootPath: string, asset: SmartLibraryAsset): ResolvedSmartLibraryAsset {
  return {
...
function semanticMatch(hit: SemanticSearchHit, metadata: Record<string, unknown>): SearchResultMatch {
  ...
  const providerReasons = stringArray(hit.matchReasons).filter((reason) => !genericMatchReasons.has(reason.toLocaleLowerCase()));
  return {
...
function fileEntryFromSemanticLocation(resolved: ResolvedSmartLibraryAsset, hit: SemanticSearchHit, metadata: Record<string, unknown>): FileEntry {
  const sourceKind: SearchSourceKind = resolved.sourceKind === "cloud" ? "remote" : "local";
  const location: ExplorerLocation = sourceKind === "remote"
    ? { kind: "remote", providerType: stringValue(metadata.providerType), remoteName: stringValue(metadata.remoteName), remotePath: resolved.relativePath }
...
function pathAllowed(path: string, sourceKind: "local" | "cloud", options: ExplorerSearchOptions): boolean {
  ...
  if (options.scope === "current" && options.currentPath) return isPathWithin(path, options.currentPath);
  return true;
}
...
export function isPathWithin(path: string, root: string): boolean {
  ...
  return candidate === normalizedRoot || candidate.startsWith(normalizedRoot === "/" ? "/" : `${normalizedRoot}/`);
}
...
function stringArray(value: unknown): string[] {
  return Array.isArray(value) ? uniqueStrings(value.filter((item): item is string => typeof item === "string" && Boolean(item.trim()))) : [];
}
