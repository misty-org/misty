...
export type TransferTableColumn = "transfer" | "operation" | "status" | "time" | "remote" | "actions";
...
export const transferTableColumns: TransferTableColumn[] = ["transfer", "operation", "status", "time", "remote", "actions"];
...
export function filterAndSortTransfers(
  ...
  return [...filtered].sort((left, right) => direction * compareTransfers(left, right, filters.sortKey as TransferSortableKey));
}

export function buildTransferTreeRows(rows: TransferRecord[], expandedIds: Set<number>): TransferTreeRow[] {
...
export function includeTransferAncestors(filteredRows: TransferRecord[], allRows: TransferRecord[]): TransferRecord[] {
...
export function aggregateTransferProgress(row: TransferRecord, rows: TransferRecord[]): TransferProgressSnapshot {
  ...
  return descendants.reduce<TransferProgressSnapshot>((total, descendant) => {
    const transferred = Math.max(0, descendant.transferredBytes);
    return {
      transferredBytes: total.transferredBytes + transferred,
      totalBytes: total.totalBytes + (Math.max(0, descendant.totalBytes) || (isTerminalTransfer(descendant) ? transferred : 0)),
...
export function sortIndicator(activeKey: TransferSortKey, direction: TransferSortDirection, key: TransferSortableKey): string {
...
export function transferStatusTone(status: string): "neutral" | "info" | "success" | "warning" | "danger" {
...
export function isBinaryProgressTransfer(row: TransferRecord): boolean {
  return row.transferType === "create" || row.transferType === "archive" || row.transferType === "rename" || row.transferType === "delete";
}
