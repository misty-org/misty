...
export function AgentCenter({ spaceId, canRun, canViewStudio }: { spaceId: string; spaceName: string; canRun: boolean; canViewStudio: boolean }) {
  ...
  const renderTabs = () => (
    <Tabs value={tab} onValueChange={(value) => openTab(value as AgentCenterTab)}>
      <TabsList className="h-9 shrink-0 border-0 bg-transparent p-0" aria-label="Agent Center sections">
        {tabs.map((item) => (
  ...
  if (tab === "studio") return (
    <main className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden bg-background">
      <div className="flex min-h-12 items-center gap-3 overflow-x-auto border-b border-border/60 bg-card px-4" aria-label="Agent Center sections">
        {renderTabs()}
        <ToggleGroup
          type="single"
          variant="outline"
          ...
          onValueChange={(value) => {
            ...
            navigate(`/spaces/${encodeURIComponent(spaceId)}/agents/studio/${selection}${selectedId ? `?${queryKey}=${encodeURIComponent(selectedId)}` : ""}`);
          }}
          className="ml-auto rounded-lg bg-muted p-0.5"
          aria-label="Studio type"
        >
          <ToggleGroupItem value="agents" className="h-7 gap-1.5 px-2 text-xs"><Bot />Agents</ToggleGroupItem>
          <ToggleGroupItem value="workflows" className="h-7 gap-1.5 px-2 text-xs"><Workflow />Workflows</ToggleGroupItem>
        </ToggleGroup>
      </div>
  ...
  const runningCount = agents.reduce((total, item) => total + item.runs.filter((run) => ["queued", "running", "cooldown", "awaiting_approval"].includes(run.state)).length, 0);
  return (
    <main className="grid h-full min-h-0 grid-cols-[230px_minmax(0,1fr)] overflow-hidden bg-background">
      <aside className="min-h-0 overflow-auto border-r border-border/60 bg-sidebar p-3" aria-label="Agents">
        <div className="mb-2 flex min-h-9 items-center px-2">
          ...
          <IconButton className="ml-auto" size="sm" label="Refresh Agent Center" onClick={() => void load()}>
            <RefreshCcw className={loading ? "animate-spin" : ""} />
          </IconButton>
        </div>
        <SidebarNav label="Agents" className="gap-2">
          <SidebarNavSection>
            <AgentRailRow active={selectedAgentId === "all"} name="All Agents" detail={`${agents.length} available`} attention={attentionCount} running={runningCount} onClick={() => { setSelectedAgentId("all"); navigate(location.pathname, { replace: true }); }}/>
          </SidebarNavSection>
          <SidebarNavSection label="Published agents" className="border-t border-border/60 pt-3">
            ...
            {!loading && !agents.length ? <EmptyState compact icon={<Bot />} title="No published Agents" description="Published Agents will appear here when they become available in this Space." /> : null}
          </SidebarNavSection>
        </SidebarNav>
      </aside>
      <section className="grid min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] bg-background">
        <header className="flex min-h-12 items-center gap-3 border-b border-border/60 bg-card px-4">
          <strong className="truncate text-sm">{selectedAgentId === "all" ? "All Agents" : scopedAgents[0]?.catalog.agent_name ?? "Agent"}</strong>
          {attentionCount ? <StatusBadge status="warning" dot>{attentionCount} need attention</StatusBadge> : <StatusBadge status="success" dot>All clear</StatusBadge>}
          {selectedAgentId !== "all" && scopedAgents[0] ? <Button className="ml-auto" size="sm" variant="outline" type="button" onClick={() => navigate(`/spaces/${encodeURIComponent(spaceId)}/chat?agentId=${encodeURIComponent(selectedAgentId)}`)}><LockKeyhole />Chat</Button> : null}
        </header>
        <div className="flex min-h-12 items-center overflow-x-auto border-b border-border/60 bg-card px-4" aria-label="Agent Center sections">{renderTabs()}</div>
...
function AttentionView({ approvals, failed, updates, integrations, resources, calendars, details, busy, canRun, onOpen, onDecide, onRetry }: { approvals: Array<{ run: SpaceRun; agent: AgentRuntime }>; failed: Array<{ run: SpaceRun; agent: AgentRuntime }>; updates: AgentRuntime[]; integrations: SpaceIntegration[]; resources: ProviderSharedResource[]; calendars: SpaceCalendarSource[]; details: Record<string, RunDetail>; busy: string; canRun: boolean; onOpen: (id: string, trigger: HTMLElement) => void; onDecide: (id: string, approved: boolean) => void; onRetry: (id: string) => void }) {
  ...
  return <div className="mx-auto grid max-w-4xl gap-3">{empty ? <EmptyState icon={<Check />} title="Nothing needs your attention" description="Approvals, failed runs, and connection issues will show up here."/> : null}
    ...
    {failed.map(({ run, agent }) => <AttentionCard key={run.id} icon={AlertTriangle} tone="danger" title={`${agent.catalog.agent_name} run failed`} detail={`${run.error_message || run.error_code || "A workflow node exhausted its attempts."} · ${formatDate(run.updated_at)}`} action={<><Button size="sm" variant="outline" disabled={!canRun || busy === run.id} onClick={() => void onRetry(run.id)}><RefreshCcw />Retry</Button><IconButton size="sm" label="Inspect failed run" onClick={(event) => void onOpen(run.id, event.currentTarget)}><ChevronRight /></IconButton></>}/>)}
    {updates.map((item) => <AttentionCard key={item.catalog.agent_id} icon={Sparkles} tone="info" title={`${item.catalog.agent_name} has an update`} detail="Your pinned version will upgrade only while no run is active or approval-blocked."/>)}
    {integrations.map((item) => <AttentionCard key={item.id} icon={Unplug} tone="danger" title={`Reconnect ${providerById(item.provider)?.name ?? item.provider}`} detail={`${item.display_name} · ${item.status.replace(/_/g, " ")}`}/>)}
    {resources.map((item) => <AttentionCard key={item.id} icon={Unplug} tone="danger" title={`${providerById(item.provider)?.name ?? item.provider} source needs attention`} detail={`${item.display_name} · ${(item.last_error_code || item.status).replace(/_/g, " ")}`}/>)}
    {calendars.map((item) => <AttentionCard key={item.id} icon={CalendarClock} tone="danger" title="Google Calendar source needs attention" detail={`${item.display_name} · ${(item.last_error_code || item.status).replace(/_/g, " ")}`}/>)}
  </div>;
}
  ...
  return (
    <Card className="bg-[color-mix(in_srgb,var(--misty-warning)_6%,var(--card))] shadow-none ring-1 ring-[color-mix(in_srgb,var(--misty-warning)_24%,transparent)]">
      <CardContent className="flex items-start gap-3 p-4">
        <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-[color-mix(in_srgb,var(--misty-warning)_12%,transparent)] text-[var(--misty-warning)]"><ShieldCheck size={17}/></span>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold">{approval?.action_summary ?? action?.summary ?? "Approve external action"}</p>
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{agent.catalog.agent_name} · {run.workflow_identifier}@{run.workflow_version} · {info.provider || providerLabel(action?.details)}</p>
          {info.reason ? <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{info.reason}</p> : null}
          <dl className="mt-3 grid grid-cols-[max-content_minmax(0,1fr)] gap-x-3 gap-y-1 rounded-lg bg-muted p-3 text-xs">
            <dt className="text-muted-foreground">Bot</dt><dd className="truncate">{info.bot || "Misty"}</dd>
            <dt className="text-muted-foreground">Destination</dt><dd className="break-all">{info.destination || "Selected workflow destination"}</dd>
            <dt className="text-muted-foreground">Connection</dt><dd className="break-all">{info.connection || "Space installation"}</dd>
            <dt className="text-muted-foreground">Expires</dt><dd>{approval?.expires_at ? formatDate(approval.expires_at) : "Soon"}</dd>
          </dl>
          {info.content ? <div className="mt-2 max-h-28 overflow-auto whitespace-pre-wrap rounded-lg bg-muted/60 p-3 text-xs leading-relaxed text-muted-foreground">{info.content}</div> : null}
          {info.citations.length ? <div className="mt-2 flex flex-wrap gap-1" aria-label="Approval citations">{info.citations.slice(0, 4).map((citation, index) => <Badge variant="secondary" className="font-normal" key={`${citation}-${index}`}>{citation}</Badge>)}</div> : null}
          <p className="mt-2 text-xs text-muted-foreground">{info.reversibility || "The destination and permissions are checked again immediately before execution."}</p>
        </div>
        <div className="flex shrink-0 gap-2">
          <Button size="sm" variant="outline" disabled={!canRun || busy} onClick={() => onDecide(run.id, false)}>Reject</Button>
          <Button size="sm" disabled={!canRun || busy} onClick={() => onDecide(run.id, true)}>Approve</Button>
          <IconButton size="sm" label="Inspect approval" onClick={(event) => void onOpen(run.id, event.currentTarget)}><ChevronRight /></IconButton>
        </div>
      </CardContent>
    </Card>
  );
...
function RunList({ title, empty, items, busy, onOpen }: { title: string; empty: string; items: Array<{ run: SpaceRun; agent: AgentRuntime }>; busy: string; onOpen: (id: string, trigger: HTMLElement) => void }) {
  return <div className="mx-auto grid max-w-4xl gap-1"><h2 className="sr-only">{title}</h2>{!items.length ? <EmptyState icon={<Clock3 />} title={empty} description="This view will update automatically as your Agents work."/> : items.map(({ run, agent }) => <Button variant="ghost" className="grid h-auto min-h-16 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 bg-muted/30 px-3 py-2.5 text-left shadow-none hover:bg-muted/60" type="button" key={run.id} onClick={(event) => void onOpen(run.id, event.currentTarget)}><StatusBadge status={runStatus(run.state)} dot className="capitalize">{run.state.replace(/_/g, " ")}</StatusBadge><span className="min-w-0"><strong className="block truncate text-sm">{agent.catalog.agent_name} · {run.capability_id || run.trigger_kind}</strong><small className="mt-1 block truncate text-xs text-muted-foreground">{run.workflow_identifier}@{run.workflow_version} · {formatDate(run.created_at)}</small></span>{busy === run.id ? <LoaderCircle className="animate-spin" size={14}/> : <ChevronRight size={14}/>}</Button>)}</div>;
}
...
function HistoryView({ items, query, setQuery, busy, onOpen }: { items: Array<{ run: SpaceRun; agent: AgentRuntime }>; query: string; setQuery: (value: string) => void; busy: string; onOpen: (id: string, trigger: HTMLElement) => void }) {
  ...
  return <div className="mx-auto grid max-w-4xl gap-3"><label className="relative ml-auto block min-w-64"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"/><span className="sr-only">Filter runs</span><Input className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter runs"/></label><RunList title="Run history" empty="No matching runs." items={filtered} busy={busy} onOpen={onOpen}/></div>;
}
        ...
        <Card key={item.catalog.agent_id}>
          <CardHeader className="flex-row items-center justify-between space-y-0 p-4">
            <div>
              ...
              <CardDescription className="mt-1 text-xs">Pinned version {item.instance?.agent_version_id ?? "not initialized"} · {item.instance?.status ?? "idle"}</CardDescription>
            </div>
            {item.instance?.update_available ? <Button size="sm" variant="outline" disabled={busy === item.catalog.agent_id || item.instance.status !== "idle"} onClick={async () => { setBusy(item.catalog.agent_id); try { await agentArchitectureApi.updateAgentInstance(spaceId, item.catalog.agent_id); await onReload(); } catch (reason) { setError(errorText(reason)); } finally { setBusy(""); } }}><RefreshCcw />Update</Button> : null}
          </CardHeader>
          <CardContent className="grid gap-3 px-4 pb-4">
            {item.instance?.workflows.map((workflow) => <div className="flex items-center justify-between rounded-lg bg-muted/45 px-3 py-2" key={workflow.workflow_version_id}><span className="text-xs">Workflow {workflow.workflow_version_id}</span><StatusBadge status={workflow.enabled ? "success" : "neutral"} dot>{workflow.enabled ? "Enabled" : "Disabled"}</StatusBadge></div>)}
            {item.instance && integrations.some((connection) => connection.status === "active") ? <div className="grid gap-3 sm:grid-cols-2">{providerCatalog.filter((provider) => integrations.some((connection) => connection.provider === provider.id && connection.status === "active")).map((provider) => <label className="grid gap-1.5 text-xs font-medium text-foreground" key={provider.id}><span>{provider.name} account</span><Select disabled={busy === `${item.instance!.id}:${provider.id}`} value={item.instance!.connection_bindings[provider.id] || "unavailable"} onValueChange={(value) => void bindConnection(item.instance!, provider.id, value === "unavailable" ? "" : value)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="unavailable">Not available to this Agent</SelectItem>{integrations.filter((connection) => connection.provider === provider.id && connection.status === "active").map((connection) => <SelectItem key={connection.id} value={connection.id}>{connection.display_name}</SelectItem>)}</SelectContent></Select></label>)}</div> : null}
          </CardContent>
        </Card>
      ))}
      <section>
        <div className="mb-3">
          ...
          <p className="mt-1 text-xs text-muted-foreground">OAuth scopes are requested incrementally and tokens never appear in workflow definitions.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {providerCatalog.map((provider) => {
            const connection = integrations.find((item) => item.provider === provider.id);
            const unavailable = providers.find((item) => item.provider === provider.id)?.configured === false || unavailableProviders.has(provider.id);
            return <Card key={provider.id} className="shadow-none"><CardHeader className="flex-row items-start gap-3 space-y-0 p-4"><span className="grid size-9 shrink-0 place-items-center rounded-lg text-white" style={{ background: provider.color }}><Plug size={16}/></span><div className="min-w-0 flex-1"><CardTitle className="truncate text-sm">{provider.name}</CardTitle><CardDescription className="mt-1 line-clamp-2 text-xs leading-relaxed">{provider.description}</CardDescription></div></CardHeader><CardContent className="px-4 pb-4">{connection ? <><div className="flex items-center justify-between gap-2"><StatusBadge status={connection.status === "active" ? "success" : "warning"} dot className="min-w-0 truncate capitalize">{connection.display_name} · {connection.status.replace(/_/g, " ")}</StatusBadge><IconButton size="sm" label={`Disconnect ${provider.name}`} disabled={busy === connection.id} onClick={() => void disconnect(connection.id)}><Unplug /></IconButton></div>{connection.status === "active" && provider.id !== "google" ? <IntegrationResourceManager spaceId={spaceId} connection={connection} setError={setError}/> : provider.id === "google" && connection.status === "active" ? <p className="mt-3 text-xs text-muted-foreground">Publish calendars from Tasks &amp; Calendar.</p> : null}</> : <><Button variant="outline" size="sm" className="w-full" disabled={busy === provider.id || unavailable} onClick={() => void connect(provider.id)}>{busy === provider.id ? <LoaderCircle className="animate-spin" /> : unavailable ? <Unplug /> : <ExternalLink />} {unavailable ? "Unavailable" : "Connect"}</Button>{unavailable ? <p className="mt-2 text-xs text-[var(--misty-warning)]">Sign-in has not been configured on this Misty server.</p> : null}</>}</CardContent></Card>;
          })}
        </div>
      </section>
    </div>
...
function IntegrationResourceManager({ spaceId, connection, setError }: { spaceId: string; connection: SpaceIntegration; setError: (value: string) => void }) {
  ...
  return <div className="mt-3 border-t border-border/60 pt-3"><Button variant="outline" size="sm" className="w-full" type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)}><FileText />{open ? "Hide shared sources" : "Choose shared sources"}</Button>{open ? <div className="mt-2 grid max-h-48 gap-1 overflow-auto">{loading && !available.length ? <LoadingState compact title="Loading sources" /> : available.map((resource) => { const selected = published.some((item) => item.external_resource_id === resource.external_resource_id && item.resource_type === resource.resource_type); const key = `${resource.resource_type}:${resource.external_resource_id}`; return <label className="flex items-center gap-2 rounded-lg bg-muted/45 px-2.5 py-2 text-xs" key={key}><Checkbox checked={selected} disabled={busyResource === key} onCheckedChange={() => void toggle(resource)}/><span className="min-w-0 flex-1 truncate">{resource.display_name}</span><Badge variant="outline" className="text-[10px] uppercase">{resource.resource_type.replace("_", " ")}</Badge></label>; })}{!loading && !available.length ? <p className="py-3 text-center text-xs text-muted-foreground">No accessible sources were returned by this provider.</p> : null}</div> : null}</div>;
}
    ...
    >
      <SheetHeader className="flex-row items-start justify-between space-y-0 border-b border-border/60 px-5 py-4 text-left">
        <div className="min-w-0">
          ...
          <SheetDescription className="mt-1 text-xs">Execution details, actions, and output</SheetDescription>
        </div>
        <IconButton size="sm" label="Close run details" onClick={onClose} disabled={busy}><X /></IconButton>
      </SheetHeader>
      <div className="min-h-0 flex-1 overflow-y-auto p-5">
      {loading || !detail ? <LoadingState className="min-h-64" title="Loading run details" /> : <>
        <div className="mt-4 grid grid-cols-3 gap-2">{[["State", detail.run.state], ["Attempt", String(detail.run.attempt ?? 1)], ["Progress", `${Math.round(detail.run.progress || 0)}%`]].map(([label, value]) => <div className="rounded-lg bg-muted/50 p-3" key={label}><span className="block text-xs text-muted-foreground">{label}</span><strong className="mt-1 block text-xs capitalize">{value.replace(/_/g, " ")}</strong></div>)}</div>
        <section className="mt-5"><strong className="text-sm">Node progress</strong><div className="mt-3 grid gap-1">{detail.steps.map((step) => <Card className="bg-muted/40 shadow-none ring-0" key={step.ID}><CardContent className="p-3"><div className="flex items-center gap-2"><strong className="text-xs">{step.NodeID}</strong><StatusBadge className="ml-auto capitalize" status={runStatus(step.State)}>{step.State.replace(/_/g, " ")} · attempt {step.Attempt}</StatusBadge></div>{step.ErrorMessage ? <p className="mt-2 text-xs text-destructive">{step.ErrorCode}: {step.ErrorMessage}</p> : null}</CardContent></Card>)}</div></section>
        <section className="mt-5"><strong className="text-sm">Action timeline</strong><div className="mt-3 grid gap-1">{detail.actions.map((action) => <Card className="bg-muted/40 shadow-none ring-0" key={action.id}><CardContent className="p-3"><div className="flex items-center gap-2">{action.destructive ? <AlertTriangle className="text-[var(--misty-warning)]" size={13}/> : <ShieldCheck className="text-[var(--misty-success)]" size={13}/>}<strong className="text-xs">{action.summary}</strong><StatusBadge className="ml-auto capitalize" status={runStatus(action.state)}>{action.state}</StatusBadge></div>{Object.keys(action.details ?? {}).length ? <pre className="mt-2 max-h-36 overflow-auto whitespace-pre-wrap text-xs text-muted-foreground">{JSON.stringify(action.details, null, 2)}</pre> : null}</CardContent></Card>)}</div></section>
        <section className="mt-5"><strong className="text-sm">Output</strong><pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap rounded-lg bg-muted/50 p-3 text-xs">{JSON.stringify(detail.run.outputs, null, 2)}</pre></section>
        {approval ? <Alert className="mt-5 border-[color-mix(in_srgb,var(--misty-warning)_30%,var(--border))] bg-[color-mix(in_srgb,var(--misty-warning)_6%,var(--card))]"><ShieldCheck /><AlertTitle>{approval.action_summary}</AlertTitle><AlertDescription><pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded-lg bg-muted p-3 text-xs">{JSON.stringify(approval.proposed_actions, null, 2)}</pre><div className="mt-3 flex gap-2"><Button size="sm" variant="outline" disabled={!canRun || busy} onClick={() => onDecide(false)}>Reject</Button><Button size="sm" disabled={!canRun || busy} onClick={() => onDecide(true)}>Approve</Button></div></AlertDescription></Alert> : null}
        <footer className="mt-5 flex justify-end gap-2">{["queued", "running", "cooldown"].includes(detail.run.state) ? <Button size="sm" variant="outline" disabled={!canRun || busy} onClick={onCancel}><CircleStop />Cancel</Button> : null}{detail.run.state === "failed" ? <Button size="sm" variant="outline" disabled={!canRun || busy} onClick={onRetry}><RefreshCcw />Retry</Button> : null}</footer>
      </>}
      </div>
    </SheetContent>
  </Sheet>;
...
function AgentRailRow({ active, name, detail, attention, running, onClick }: { active: boolean; name: string; detail: string; attention: number; running: number; onClick: () => void }) { return <SidebarNavItem className="h-auto min-h-14 items-center px-2.5 py-2" active={active} icon={<Bot />} badge={<span className="flex gap-1">{attention ? <Badge variant="secondary" className="h-5 min-w-5 justify-center px-1.5 text-[10px]">{attention}</Badge> : null}{running ? <StatusBadge status="info" className="h-5 min-w-5 justify-center px-1.5 text-[10px]">{running}</StatusBadge> : null}</span>} onClick={onClick}><span className="block min-w-0"><strong className="block truncate text-xs text-foreground">{name}</strong><small className="mt-0.5 block truncate text-[11px] font-normal text-muted-foreground">{detail}</small></span></SidebarNavItem>; }
function AttentionCard({ icon: Icon, tone, title, detail, action }: { icon: typeof AlertTriangle; tone: "danger" | "info"; title: string; detail: string; action?: React.ReactNode }) { return <Card className="bg-muted/30 shadow-none ring-0"><CardContent className="flex items-center gap-3 p-4"><span className={`grid size-9 shrink-0 place-items-center rounded-lg ${tone === "danger" ? "bg-destructive/10 text-destructive" : "bg-[color-mix(in_srgb,var(--misty-info)_12%,transparent)] text-[var(--misty-info)]"}`}><Icon size={16}/></span><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{title}</strong><p className="mt-1 text-xs text-muted-foreground">{detail}</p></div>{action ? <div className="flex gap-2">{action}</div> : null}</CardContent></Card>; }
...
function runStatus(state: string): "neutral" | "info" | "success" | "warning" | "danger" { if (state === "completed") return "success"; if (state === "completed_with_errors" || state === "cooldown" || state === "awaiting_approval") return "warning"; if (state === "failed" || state === "rejected") return "danger"; if (state === "running" || state === "queued") return "info"; return "neutral"; }
