...
export default function SpaceStudioPage({
  ...
  useEffect(() => {
    if (canView) void loadStudio(spaceId, kind);
  }, [canView, kind, loadStudio, spaceId]);
  useEffect(() => {
    if (canView && kind === "agents") void loadStudio(spaceId, "workflows");
  }, [canView, kind, loadStudio, spaceId]);
    ...
    <div className="h-full min-h-0 bg-transparent">
      <main className="h-full min-h-0">
        {!canView ? (
          <PermissionState
            className="h-full"
            title="Studio access is turned off"
            description="Ask the Space owner to grant you permission to view Studio."
          />
        ) : kind === "workflows" ? (
          ...
          <SharedStudio
            ...
            resources={resources}
            workflows={workflowsBySpace[spaceId] ?? emptyStudioResources}
            initialResourceId={
              searchParams.get(kind === "agents" ? "agentId" : "workflowId") ?? undefined
            }
...
function SharedStudio(props: {
  ...
  const [workflowRunDetail, setWorkflowRunDetail] = useState<{
    run: SpaceRun;
    actions: RunAction[];
  ...
  const selected = creating
    ? null
    : (props.resources.find((item) => item.id === selectedId) ?? props.resources[0] ?? null);
  ...
  const openWorkflowRun = async (runId: string, returnFocus?: HTMLElement | null) => {
    ...
    setWorkflowHistoryError("");
    try {
      setWorkflowRunDetail(await agentArchitectureApi.runDetail(runId));
    } catch (reason) {
      setWorkflowHistoryError(errorText(reason));
    } finally {
      setWorkflowHistoryBusy(false);
  ...
  useEffect(() => {
    setCreating(false);
  }, [props.kind, props.spaceId]);
  useEffect(() => {
    if (
      props.initialResourceId &&
      appliedInitialResourceId.current !== props.initialResourceId &&
      props.resources.some((item) => item.id === props.initialResourceId)
    ) {
      appliedInitialResourceId.current = props.initialResourceId;
      setCreating(false);
  ...
  const workflowDirectRunAvailable =
    props.kind === "workflows" && workflowRuntime === "misty-cloud";
  const description =
    props.kind === "agents"
      ? agentRuntime === "device"
  ...
  const workflowCapabilities = useMemo(
    () => workflowCapabilityOptions(draft?.definition),
    [draft?.definition],
  );
  const workflowPromptRequired = useMemo(
    () => workflowCapabilityRequiresPrompt(draft?.definition, workflowCapability),
    [draft?.definition, workflowCapability],
  );

  useEffect(() => {
    setWorkflowCapability((current) =>
      workflowCapabilities.some((item) => item.id === current)
        ? current
        : (workflowCapabilities[0]?.id ?? ""),
    );
  }, [workflowCapabilities]);

  const createNew = () => {
    const fresh: Partial<SpaceStudioResource> =
      props.kind === "agents"
        ? {
            kind: "agent",
            name: "New Agent",
            description: "A capable teammate for this Space.",
            icon: "bot",
            instructions: "Help teammates in this Space.",
            access_policy: { mode: "space", allowedUserIds: [] },
            enabled: true,
            status: "available",
            runtime_kind: "cloud",
        ...
        : {
            kind: "workflow",
            name: "New Workflow",
            description: "A reusable workflow for this Space.",
            definition: starterWorkflowDefinition(),
            enabled: false,
            schedules_enabled: false,
            version: 0,
          };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const saved = await props.saveStudio(props.spaceId, props.kind, {
        ...draft,
        schedules_enabled: false,
      });
      ...
      /* server conflict is shown by the shared store */
    } finally {
      setSaving(false);
    }
  };
  ...
  const run = async () => {
    ...
    setWorkflowHistoryError("");
    try {
      const next = await props.runStudio(
        props.spaceId,
        props.kind,
        draft.id,
        workflowPrompt.trim(),
  ...
  const decideWorkflowRun = async (approved: boolean) => {
    ...
    try {
      ...
      await Promise.all([
        openWorkflowRun(workflowRunDetail.run.id),
        refreshWorkflowHistory(draft?.id),
      ]);
    } catch (reason) {
      setWorkflowHistoryError(errorText(reason));
    } finally {
  ...
  const remove = async () => {
    ...
    try {
      ...
      /* server error is shown by the shared store */
    }
  };
    ...
    <div className="grid h-full min-h-0 grid-cols-[250px_minmax(0,1fr)]">
      <aside className="min-h-0 overflow-auto border-r border-border bg-card p-3">
        ...
        <div className="grid gap-1">
          {props.resources.map((item) => (
            <Button
              className={`h-auto justify-start rounded-md px-3 py-2.5 text-left ${!creating && selected?.id === item.id ? "bg-accent text-accent-foreground" : "text-muted-foreground"}`}
              variant="ghost"
              key={item.id}
              onClick={() => {
                setCreating(false);
            ...
            >
              <span className="min-w-0">
                ...
                <span className="mt-1 block text-[10px] text-muted-foreground">
                  {props.kind === "workflows"
                    ? `${studioWorkflowRuntime(item) === "misty-device" ? "Device" : "Cloud"} · `
                    : `${item.runtime_kind === "device" ? "Device" : "Cloud"} · `}
                  v{item.active_workflow?.version ?? item.version} ·{" "}
          ...
          {props.canManage ? (
            ...
            >
              <Plus />
              <span>Add {props.kind === "agents" ? "agent" : "workflow"}</span>
            </Button>
          ) : null}
        </div>
      </aside>
      <section className="min-h-0 overflow-auto p-6">
        {!draft ? (
          <EmptyState
            className="h-full"
            icon={props.kind === "agents" ? <Bot /> : <Workflow />}
            title={`No ${props.kind} yet`}
            description={
              props.canManage
                ? `Create the first ${props.kind === "agents" ? "Agent" : "Workflow"} for this Space.`
                : "A Studio manager can add one here."
            }
            action={
              props.canManage ? (
          ...
          <div className="mx-auto grid max-w-3xl gap-5">
            <div>
              <h2 className="text-lg font-semibold">
                {draft.id
                  ? `Edit ${props.kind === "agents" ? "Agent" : "Workflow"}`
                  : `New ${props.kind === "agents" ? "Agent" : "Workflow"}`}
              </h2>
              <p className="mt-1 text-xs text-muted-foreground">{description}</p>
            </div>
            {props.error ? (
              <Alert variant="destructive">
                <AlertTitle>Studio couldn’t save this resource</AlertTitle>
            ...
            {!props.canManage ? (
              <Alert>
                ...
                <AlertDescription>
                  You can view this resource, but only members with Studio management permission can
                  change it.
                </AlertDescription>
              </Alert>
            ) : null}
            ...
            {props.kind === "agents" ? (
              <>
                ...
                <div className="grid grid-cols-[180px_minmax(0,1fr)] gap-3">
                  <label className={fieldLabelClass}>
                    ...
                    <Select
                      disabled={!props.canManage}
                      value={draft.access_policy?.mode ?? "space"}
                      onValueChange={(value) =>
                        setDraft({
                          ...draft,
                          access_policy: {
                            mode: value as "space" | "selected",
                            allowedUserIds:
                              value === "space" ? [] : (draft.access_policy?.allowedUserIds ?? []),
                          },
                        })
                      }
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="space">Everyone in Space</SelectItem>
                        <SelectItem value="selected">Selected members</SelectItem>
                      </SelectContent>
                    </Select>
                  </label>
                  {draft.access_policy?.mode === "selected" ? (
                    <label className={fieldLabelClass}>
                      Allowed member IDs
                      <Input
                        disabled={!props.canManage}
                        value={draft.access_policy.allowedUserIds.join(", ")}
                      ...
                      />
                      <span>Only these Space members and the creator can use this Agent.</span>
                    </label>
                  ) : (
                    <div className="self-end pb-2 text-[10px] text-muted-foreground">
                      Default: every Space member can use the Agent with isolated conversations and
                      run state.
                    </div>
                  )}
                </div>
              ...
              <>
                ...
                <label className={fieldLabelClass}>
                  Portable workflow package definition
                  <Textarea
                    className={`min-h-72 resize-y font-mono text-[11px] ${definitionError ? "border-destructive" : ""}`}
                    disabled={!props.canManage}
                    value={definitionSource}
                    aria-invalid={Boolean(definitionError)}
                    aria-describedby={definitionError ? "workflow-definition-error" : undefined}
                    onChange={(event) => {
                      ...
                      try {
                        ...
                        setDraft({ ...draft, definition: value as Record<string, unknown> });
                        setDefinitionError("");
                      } catch {
                        setDefinitionError("Enter a valid JSON object before saving.");
                      }
                    }}
                  />
                  <span>
                    Include structured capabilities, typed inputs and outputs, integrations,
                    permissions, runtime compatibility, and tags. Local-path and device-secret nodes
                    are rejected by the server.
                  </span>
                  {definitionError ? (
                    <span className="text-destructive" id="workflow-definition-error" role="alert">
                      {definitionError}
                    </span>
                  ) : null}
                </label>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <span className="text-[10px] text-muted-foreground">
                Runs charge the member who starts them.
              </span>
            </div>
            {props.kind === "workflows" && workflowRuntime === "misty-device" ? (
              <Alert>
                <AlertTitle>Device runtime required</AlertTitle>
                <AlertDescription>
                  This Workflow requires local device capabilities. Attach a folder to an Agent,
                  then pin this version to that Agent to test or run it.
                </AlertDescription>
              </Alert>
            ) : null}
            {props.kind === "agents" && draft.id ? (
              <AgentArchitecturePanel
                agent={draft as SpaceStudioResource}
                spaceName={props.spaceName}
                workflows={props.workflows}
                canManage={props.canManage}
                canRun={props.canRun}
                initialRunId={props.initialRunId}
                onAgentUpdated={(agent) => setDraft(agent)}
              />
            ) : null}
            {lastRun ? (
              <Button
                className="h-auto w-full justify-between rounded-md p-3 text-left text-xs shadow-none"
                variant="outline"
                onClick={(event) => void openWorkflowRun(lastRun.id, event.currentTarget)}
              >
                <StatusBadge status={runStatus(lastRun.state)} dot className="capitalize">
                  Latest run · {lastRun.state.replace(/_/g, " ")}
                </StatusBadge>
            ...
            {workflowHistoryError ? (
              <Alert variant="destructive">
                <AlertTitle>Workflow history error</AlertTitle>
                <AlertDescription>{workflowHistoryError}</AlertDescription>
              </Alert>
            ) : null}
            {props.kind === "workflows" && draft.id ? (
              <Card className="grid overflow-hidden shadow-none lg:grid-cols-2 lg:divide-x lg:divide-border">
                <section className="border-b border-border p-4 lg:border-b-0">
                  <div className="flex items-center justify-between gap-3">
                    <h3 className="text-xs font-semibold">Version history</h3>
                    <span className="text-[10px] text-muted-foreground">
                      {workflowVersions.length} pinned snapshot
                      {workflowVersions.length === 1 ? "" : "s"}
                    </span>
                  </div>
                  <div className="mt-3 grid max-h-48 gap-1 overflow-auto">
                    {workflowVersions.length ? (
                      workflowVersions.map((version) => (
                        <article className="rounded-md bg-muted/40 px-3 py-2" key={version.id}>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {workflowHistoryBusy ? "Loading versions…" : "No versions available."}
                      </p>
                    )}
                  </div>
                </section>
                <section className="p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      disabled={workflowHistoryBusy}
                      onClick={() => void refreshWorkflowHistory(draft.id)}
                    >
                      <History />
                  ...
                  <div className="mt-3 grid max-h-48 gap-1 overflow-auto">
                    {workflowRuns.length ? (
                      workflowRuns.map((item) => (
                        <Button
                          className="grid h-auto grid-cols-[auto_minmax(0,1fr)] justify-start gap-2 rounded-md bg-muted/30 px-3 py-2 text-left"
                          variant="ghost"
                          key={item.id}
                          onClick={(event) => void openWorkflowRun(item.id, event.currentTarget)}
                        >
                          <StatusBadge status={runStatus(item.state)} className="capitalize">
                            {item.state.replace(/_/g, " ")}
                          </StatusBadge>
                          <span className="truncate text-xs text-muted-foreground">
                            {item.capability_id} · {item.workflow_version}
                          </span>
                        </Button>
                      ))
                    ) : (
                      <p className="text-xs text-muted-foreground">
                        {workflowHistoryBusy ? "Loading runs…" : "No runs yet."}
                      </p>
                    )}
                  </div>
                </section>
            ...
            <div className="flex flex-wrap justify-between gap-3 border-t border-border pt-4">
              ...
              <div className="flex min-w-0 flex-1 flex-wrap justify-end gap-2">
                {workflowDirectRunAvailable &&
                props.canRun &&
                draft.id &&
                draft.enabled &&
                workflowCapabilities.length > 1 ? (
                  <Select value={workflowCapability} onValueChange={setWorkflowCapability}>
                    <SelectTrigger className="max-w-48" aria-label="Workflow test capability">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                {workflowDirectRunAvailable && props.canRun && draft.id && draft.enabled ? (
                  <Input
                    className="min-w-48 flex-1"
                    value={workflowPrompt}
                    onChange={(event) => setWorkflowPrompt(event.target.value)}
                    placeholder={workflowPromptRequired ? "Test input (required)…" : "Test input…"}
                    aria-label="Workflow test input"
                  />
                ) : null}
                {workflowDirectRunAvailable && props.canRun && draft.id && draft.enabled ? (
                  <Button
                    ref={workflowRunButtonRef}
                    variant="outline"
                    disabled={
                      running ||
                      (workflowCapabilities.length > 1 && !workflowCapability) ||
                      (workflowPromptRequired && !workflowPrompt.trim())
                    }
                    onClick={() => void run()}
                  >
                    <Play size={14} />
                ...
                {props.canManage ? (
                  <Button
                    disabled={saving || !draft.name?.trim() || Boolean(definitionError)}
                    onClick={() => void save()}
                  >
                    <Save size={14} />
                    {saving ? "Saving…" : "Save"}
        ...
        {workflowRunDetail ? (
          <RunDetailDialog
            ...
            resourceName={draft?.name ?? "Workflow"}
            spaceName={props.spaceName}
            busy={workflowHistoryBusy}
            canRetry={workflowDirectRunAvailable && Boolean(draft?.enabled) && props.canRun}
            operationError={workflowHistoryError}
            returnFocusRef={workflowRunReturnFocusRef}
            onClose={() => setWorkflowRunDetail(null)}
            onDecide={decideWorkflowRun}
            onCancel={cancelWorkflowRun}
            onRetry={retryWorkflowRun}
          />
        ) : null}
      </section>
    </div>
...
function workflowCapabilityOptions(
      ...
      ? [
          {
            id: candidate.id,
            name:
              typeof candidate.name === "string" && candidate.name ? candidate.name : candidate.id,
          },
        ]
      : [];
  });
...
function workflowCapabilityRequiresPrompt(
  definition: Record<string, unknown> | undefined,
  capabilityId: string,
): boolean {
  const metadata = definition?.metadata;
  ...
  const capability = capabilities.find(
    (item) =>
      ...
      typeof item === "object" &&
      !Array.isArray(item) &&
      (item as { id?: unknown }).id === capabilityId,
  );
  ...
  return (
    ...
    inputs.some(
      (item) =>
        ...
        typeof item === "object" &&
        !Array.isArray(item) &&
        (item as { name?: unknown; type?: unknown; required?: unknown }).name === "prompt" &&
        (item as { type?: unknown }).type === "string" &&
...
function workflowDraft(resource: SpaceStudioResource): SpaceStudioResource {
  const definition =
    resource.definition &&
    typeof resource.definition === "object" &&
    !Array.isArray(resource.definition)
      ? { ...resource.definition }
      : {};
  if (!("metadata" in definition) && resource.active_workflow?.metadata)
    definition.metadata = resource.active_workflow.metadata;
...
function starterWorkflowDefinition(): Record<string, unknown> {
  return {
    metadata: {
      capabilities: [
        {
          id: "default",
          name: "Run workflow",
          description: "Run this workflow with a text prompt.",
      ...
      requiredPermissions: [],
      runtime: { kind: "misty-cloud", compatibility: "1" },
      tags: ["general"],
    },
...
const fieldLabelClass =
  "grid gap-2 text-xs font-medium text-foreground [&>span]:text-[11px] [&>span]:font-normal [&>span]:text-muted-foreground";
