        ...
        <form className="flex min-h-0 flex-1 flex-col" onSubmit={onSave}>
          <SheetHeader className="border-b border-border/60 px-5 py-4 pr-14 text-left">
            <div className="flex items-center gap-3">
              <div className="min-w-0 flex-1">
                ...
                <SheetDescription>
                  {editing ? "Update the task details and assignment." : "Create a shared task for this Space."}
                </SheetDescription>
              </div>
              {onArchive ? (
                <DropdownMenu>
          ...
          <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_240px] overflow-auto max-sm:grid-cols-1">
            <div className="grid content-start gap-5 p-5 sm:p-6">
              ...
              {editing &&
              (editing.created_by_agent_id || editing.source_run_id || editing.source_refs.length) ? (
                <div className="rounded-lg bg-muted/30 p-3">
                  <details className="text-xs text-muted-foreground">
                    ...
                    <div className="mt-2 grid gap-1">
                      ...
                      <span>
                        {editing.source_refs.length} source{editing.source_refs.length === 1 ? "" : "s"}
                      </span>
                    </div>
                  </details>
                </div>
    ...
    <Sheet open onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="w-[min(460px,96vw)] bg-background sm:max-w-[460px]">
        <SheetHeader className="pr-8 text-left">
          <Badge className="mb-1 w-fit" variant="secondary">Google Calendar</Badge>
    ...
    <Sheet open onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="w-[min(480px,96vw)] bg-background p-0 sm:max-w-[480px]">
        <SheetHeader className="border-b border-border/60 px-5 py-4 pr-14 text-left">
          ...
          <SheetDescription>Choose which calendars are visible to members of this Space.</SheetDescription>
        </SheetHeader>
        <div className="min-h-0 overflow-auto p-5">
          ...
          {!activeIntegrations.length ? (
            ...
            <div className="mt-4 divide-y divide-border/60 overflow-hidden rounded-lg bg-muted/30">
              ...
              {!choices.length ? (
                <p className="text-xs text-muted-foreground">No calendars are available for this account.</p>
              ) : null}
            </div>
          ) : null}
          ...
          <div className="grid gap-1">
            {sources.map((source) => (
              <div className="flex min-h-11 items-center gap-2 rounded-md px-2 hover:bg-muted" key={source.id}>
            ...
            {!sources.length ? <p className="text-xs text-muted-foreground">No calendars published yet.</p> : null}
          </div>
        </div>
      </SheetContent>
    </Sheet>
