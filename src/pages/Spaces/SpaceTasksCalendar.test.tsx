...
const { createTask } = vi.hoisted(() => ({
  createTask: vi.fn().mockResolvedValue({ id: "task-1", task_key: "MST-1", title: "Write notes", notes: "", status: "todo", priority: "medium", rank: 1024, source_refs: [], due_timezone: "UTC", version: 1 }),
}));
...
describe("SpaceTasksCalendar", () => {
  ...
  it("does not send blank optional date and assignee fields when quick-adding a task", async () => {
    ...
    const createButton = [...container.querySelectorAll("button")].find((button) => button.textContent === "Drop or create");
    await act(async () => createButton?.click());
    const input = container.querySelector<HTMLInputElement>('input[placeholder="Task title"]');
    expect(input).not.toBeNull();
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set?.call(input, "Write notes");
    ...
    expect(payload).toMatchObject({ title: "Write notes", notes: "", status: "todo", priority: "medium" });
  ...
  it("includes notes when saving a task from the drawer", async () => {
    ...
    const createButton = [...container.querySelectorAll("button")].find((button) => button.textContent === "Create");
    await act(async () => createButton?.click());
    ...
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set?.call(title, "Write notes");
      title!.dispatchEvent(new Event("input", { bubbles: true }));
      Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, "value")?.set?.call(notes, "Keep the useful details.");
    ...
    expect(createTask).toHaveBeenCalledOnce();
    expect(createTask.mock.calls[0]?.[1]).toMatchObject({ title: "Write notes", notes: "Keep the useful details." });
  });
});
