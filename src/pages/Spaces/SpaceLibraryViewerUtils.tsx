...
export function LibraryMetadataRow({ label, value }: { label: string; value: string }) {
  return <div><dt className="text-[10px] capitalize text-muted-foreground">{label}</dt><dd className="m-0 mt-1 break-words text-muted-foreground">{value || "—"}</dd></div>;
}
export function LibraryEditRange({ label, value, min, max, step, onChange }: { label: string; value: number; min: number; max: number; step: number; onChange: (value: number) => void }) {
  return <label className="mt-4 grid grid-cols-[1fr_auto] items-center gap-x-3 gap-y-1 text-[10px] font-medium capitalize text-muted-foreground"><span>{label}</span><span>{value.toFixed(2)}</span><Slider className="col-span-2" value={[value]} min={min} max={max} step={step} onValueChange={([next]) => onChange(next ?? value)}/></label>;
}

export function LibraryAdvancedAdjustments({ draft, onChange }: { draft: LibraryEditDefinition; onChange: Dispatch<SetStateAction<LibraryEditDefinition>> }) {
  const update = (key: keyof LibraryEditDefinition, value: number) => onChange((current) => ({ ...current, [key]: value }));
  return <Collapsible className="mt-4 rounded-xl bg-muted/35 px-3 py-2">
    <CollapsibleTrigger asChild>
      <Button className="group h-7 w-full justify-between px-0 text-[10px] text-muted-foreground" size="sm" variant="ghost">
...
export function normalizeLibraryEdit(definition?: Partial<LibraryEditDefinition> | null): LibraryEditDefinition {
