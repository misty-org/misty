...
describe("libraryItemThumbnailEligible", () => {
  it("requests thumbnails for PDFs and common documents", () => {
    ...
    expect(libraryItemThumbnailEligible("application/octet-stream", "Research Brief.docx")).toBe(true);
  ...
  it("leaves unusual binary files on the generic-file fallback", () => {
    expect(libraryItemThumbnailEligible("application/octet-stream", "payload.weirdbin")).toBe(false);
  });
});
