...
export function LibraryMemoryPlayback({ spaceId, group, items, onClose }: { spaceId: string; group: LibraryDiscoveryGroup; items: SpaceLibraryItem[]; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}>
    <DialogContent className="flex h-[calc(100dvh-var(--misty-window-titlebar-inset))] max-h-[calc(100dvh-var(--misty-window-titlebar-inset))] w-screen max-w-none flex-col gap-0 rounded-none border-0 bg-black p-0 text-white sm:max-w-none [&>button]:right-5 [&>button]:top-5 [&>button]:text-white" onKeyDown={(event) => {
    ...
    <header className="px-5 py-4 pr-16"><DialogTitle className="truncate text-base text-white">{group.title}</DialogTitle><DialogDescription className="mt-1 truncate text-xs text-white/55">{group.subtitle}</DialogDescription></header>
