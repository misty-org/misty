...
export function LibraryItemThumbnail({ spaceId, item, reauthenticationToken = "" }: { spaceId: string; item: SpaceLibraryItem; reauthenticationToken?: string }) {
...
export type LibraryAssetStackInput = Pick<LibraryAssetStack, "kind" | "title" | "cover_item_id" | "motion_item_id" | "members">;

export function buildLibraryAssetStack(kind: LibraryAssetStack["kind"], items: SpaceLibraryItem[]): LibraryAssetStackInput | null {
  if (kind === "live_photo") {
...
export function AlbumCover({ spaceId, itemId }: { spaceId: string; itemId?: string }) {
  ...
  return <span className="grid aspect-[4/3] w-full place-items-center overflow-hidden bg-muted text-muted-foreground">{url ? <img className="size-full object-cover" src={url} alt=""/> : <LibraryIcon size={26}/>}</span>;
}

export function LibraryFacetGroup({ label, facets, onSelect }: { label: string; facets: LibrarySearchFacets["tags"]; onSelect: (facet: LibrarySearchFacets["tags"][number]) => void }) {
  return <div className="flex min-w-0 items-center gap-1.5"><span className="mr-0.5 text-[10px] font-semibold capitalize text-muted-foreground">{label}</span>{facets.slice(0, 6).map((facet) => <Button size="sm" variant="outline" type="button" key={`${facet.value}:${facet.label}`} onClick={() => onSelect(facet)}>{facet.label}<span className="text-muted-foreground">{facet.count}</span></Button>)}</div>;
}
...
export function LibraryCollectionCard({ icon: Icon, label, count, disabled = false, pinned = false, onClick, onTogglePin, onMoveEarlier, onMoveLater }: { icon: LucideIcon; label: string; count: number; disabled?: boolean; pinned?: boolean; onClick?: () => void; onTogglePin?: () => void; onMoveEarlier?: () => void; onMoveLater?: () => void }) {
  const canEdit = useContext(LibraryCanEditContext);
  return <article className="group relative overflow-hidden rounded-xl bg-card shadow-xs ring-1 ring-foreground/10"><Button className="block w-full border-0 bg-transparent p-4 text-left disabled:opacity-40" type="button" disabled={disabled} onClick={onClick}><Icon size={22}/><p className="mb-0 mt-3 truncate text-xs font-medium">{label}</p><p className="mb-0 mt-1 text-[10px] text-muted-foreground">{count} items</p></Button>{canEdit && onTogglePin && !disabled ? <Button className={`absolute right-2 top-2 grid size-7 place-items-center rounded-lg border-0 ${pinned ? "bg-accent text-foreground" : "bg-transparent text-muted-foreground opacity-0 group-hover:opacity-100 focus:opacity-100"}`} type="button" onClick={onTogglePin} title={pinned ? "Unpin" : "Pin collection"} aria-label={`${pinned ? "Unpin" : "Pin"} ${label}`}><Pin size={13} fill={pinned ? "currentColor" : "none"}/></Button> : null}{canEdit && (onMoveEarlier || onMoveLater) ? <span className="absolute bottom-2 right-2 flex gap-0.5 opacity-0 group-hover:opacity-100 focus-within:opacity-100">{onMoveEarlier ? <Button className="grid size-6 place-items-center rounded-md border-0 bg-muted text-muted-foreground" type="button" onClick={onMoveEarlier} title="Move earlier" aria-label={`Move ${label} earlier`}><ChevronLeft size={12}/></Button> : null}{onMoveLater ? <Button className="grid size-6 place-items-center rounded-md border-0 bg-muted text-muted-foreground" type="button" onClick={onMoveLater} title="Move later" aria-label={`Move ${label} later`}><ChevronRight size={12}/></Button> : null}</span> : null}</article>;
}

export function LibraryMapView({ points, onBack, onSelect }: { points: LibraryMapPoint[]; onBack: () => void; onSelect: (point: LibraryMapPoint) => void }) {
  return <div className="mb-6">
    <Button className="mb-4 border-0 bg-transparent p-0 text-xs text-muted-foreground" type="button" onClick={onBack}>← Collections</Button>
    ...
    <div className="mt-4 grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-3">{points.slice(0, 12).map((point) => <Button className="rounded-xl border-0 bg-muted/40 p-3 text-left shadow-none hover:bg-muted/65" type="button" key={point.id} onClick={() => onSelect(point)}><p className="m-0 truncate text-xs font-medium">{point.name}</p><p className="mb-0 mt-1 text-[10px] text-muted-foreground">{point.item_count} items · {point.latitude.toFixed(2)}, {point.longitude.toFixed(2)}</p></Button>)}</div>
  </div>;
}

export function LibraryDiscoveryCard({ spaceId, group, fallbackIcon: Icon, pinned = false, onTogglePin, onClick }: { spaceId: string; group: LibraryDiscoveryGroup; fallbackIcon: LucideIcon; pinned?: boolean; onTogglePin?: () => void; onClick: () => void }) {
  const canEdit = useContext(LibraryCanEditContext);
  return <article className="group relative overflow-hidden rounded-xl bg-card shadow-xs ring-1 ring-foreground/10"><Button className="block w-full border-0 bg-transparent p-0 text-left" type="button" onClick={onClick}><span className="relative block"><AlbumCover spaceId={spaceId} itemId={group.cover_item_id}/><span className="absolute left-3 top-3 grid size-8 place-items-center rounded-lg bg-black/55 text-white backdrop-blur"><Icon size={16}/></span></span><span className="block p-3"><span className="block truncate text-xs font-medium">{group.title}</span><span className="mt-1 block truncate text-[10px] text-muted-foreground">{group.subtitle}</span></span></Button>{canEdit && onTogglePin ? <Button className={`absolute right-3 top-3 grid size-8 place-items-center rounded-lg border-0 backdrop-blur ${pinned ? "bg-white text-black" : "bg-black/55 text-white opacity-0 group-hover:opacity-100 focus:opacity-100"}`} type="button" onClick={onTogglePin} title={pinned ? "Unpin" : "Pin collection"} aria-label={`${pinned ? "Unpin" : "Pin"} ${group.title}`}><Pin size={14} fill={pinned ? "currentColor" : "none"}/></Button> : null}</article>;
}

export function LibrarySelect({ value, options, onChange, label, className = "" }: { value: string; options: Array<[string,string]>; onChange: (value: string) => void; label: string; className?: string }) { const selectedValue = value || "__none__"; return <Select value={selectedValue} onValueChange={(next) => onChange(next === "__none__" ? "" : next)}><SelectTrigger className={className} aria-label={label}><SelectValue/></SelectTrigger><SelectContent>{options.map(([id,name]) => <SelectItem value={id || "__none__"} key={id || "__none__"}>{name}</SelectItem>)}</SelectContent></Select>; }
