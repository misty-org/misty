...
import { ErrorState, LoadingState, PermissionState } from "@/components/misty";
...
export function SpaceDetail() {
  ...
  if (!space && loading) {
    return <LoadingState className="h-full" title="Loading Space" description="Getting the latest Space details…" />;
  }

  if (!space) {
    return <ErrorState className="h-full" title="Space unavailable" description="This Space may have been removed, or you may no longer have access." />;
  }
    ...
    <div className="relative h-full min-h-0 overflow-hidden">
      ...
      {section === "library" || section === "files" ? (
        space.permissions?.["library.view"] === false ? (
          <SpacePermissionDenied title="Library access required" detail="You do not have permission to view this Space's Library." />
        ) : (
          <SpaceLibrary key={`library:${spaceId}`} spaceId={spaceId} />
        )
      ) : section === "studio" ? (
        <Navigate to={`/spaces/${encodeURIComponent(spaceId)}/agents/studio/${normalizeStudioKind(studioKind)}${query}`} replace />
      ) : section === "agents" ? (
        space.permissions?.["agents.run"] === false && space.permissions?.["studio.view"] === false ? (
          <SpacePermissionDenied title="Agent access required" detail="Ask a Space owner to grant Agent or Studio access." />
        ) : (
          <AgentCenter
        ...
        space.permissions?.["tasks.view"] === false ? (
          <SpacePermissionDenied title="Task access required" detail="Ask a Space owner to grant task access." />
        ) : (
          <SpaceTasksCalendar
            key={`tasks:${spaceId}`}
            spaceId={spaceId}
        ...
        <SpaceSettings key={`settings:${spaceId}:${studioKind}`} spaceId={spaceId} section={studioKind} />
      ) : space.permissions?.["messages.read"] === false ? (
        <SpacePermissionDenied title="Chat access required" detail="You do not have permission to read this Space's messages." />
      ) : (
