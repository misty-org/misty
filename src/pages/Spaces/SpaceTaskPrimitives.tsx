...
import { EmptyState, ErrorState } from "@/components/misty";
import type {
  SpaceMember,
  SpaceTask,
  SpaceTaskPriority,
...
export function TaskEmptyState({
  ...
  return (
    <EmptyState className="min-h-56 max-w-none rounded-lg bg-muted/30" compact icon={<ListTodo />} title={title} description={description} />
  );
}
...
export function TaskErrorState({ message, onDismiss }: { message: string; onDismiss: () => void }) {
  return (
    <ErrorState
      ...
      description={message}
      action={<Button size="sm" variant="outline" type="button" onClick={onDismiss}>Dismiss</Button>}
    />
  );
}
...
export function memberName(members: SpaceMember[], id?: string) {
  return id ? members.find((member) => member.user_id === id)?.name ?? "Former member" : "Unassigned";
}
