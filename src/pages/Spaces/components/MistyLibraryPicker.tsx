  ...
  const picker = (
    <div className="fixed bottom-0 left-[72px] right-0 top-7 z-[2147483100] grid place-items-center bg-black/60 p-6 backdrop-blur-md max-[800px]:left-0 max-[800px]:p-3 max-[560px]:p-0" role="presentation" onKeyDown={handleDialogKeyDown}>
      <section ref={dialogRef} tabIndex={-1} className="grid h-[min(680px,calc(100vh-88px))] w-[min(1100px,calc(100vw-140px))] grid-rows-[auto_auto_minmax(0,1fr)_auto] overflow-hidden rounded-2xl border border-[var(--misty-border)] bg-[var(--misty-app-modal-bg,var(--misty-surface))] text-[var(--misty-text)] shadow-2xl outline-none max-[800px]:size-full max-[560px]:rounded-none" role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId}>
        ...
        <div className="flex min-h-[60px] items-center gap-3 border-b border-[var(--misty-divider-subtle)] px-4">
          <label className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-[var(--misty-border-soft)] bg-[var(--misty-surface-2)] px-3"><Search className="shrink-0 text-[var(--misty-text-subtle)]" size={16}/><input className="h-10 min-w-0 flex-1 border-0 bg-transparent text-sm text-[var(--misty-text)] outline-none placeholder:text-[var(--misty-text-subtle)]" autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search names, filenames, and tags"/></label>
          <span className="shrink-0 text-xs text-[var(--misty-text-subtle)]">{filteredItems.length} item{filteredItems.length === 1 ? "" : "s"}</span>
        </div>
        ...
        <div className="grid min-h-0 grid-cols-[220px_minmax(0,1fr)] max-[760px]:grid-cols-1">
          <aside className="min-h-0 overflow-y-auto border-r border-[var(--misty-divider-subtle)] p-3 max-[760px]:hidden">
            <p className="mb-2 mt-1 px-2 text-[10px] font-semibold uppercase tracking-wide text-[var(--misty-text-subtle)]">Media</p>
          ...
          <main className="min-h-0 overflow-y-auto bg-[var(--misty-surface)] p-4">
