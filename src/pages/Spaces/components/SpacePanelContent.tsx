...
import { Bot, Briefcase, Check, ChevronDown, Folder, HardDrive, Heart, Images, Map as MapIcon, MessageSquare, MessagesSquare, Plus, Settings2, Sparkles, Trash2, Users } from "lucide-react";
...
export function SpacePanelContent(props: {
  ...
  const agentStudioActive = section === "studio" || (section === "agents" && routeParts[3] === "studio");
  const studioKindSegment = agentStudioActive && section === "agents" ? routeParts[4] : routeParts[3];
  ...
  useEffect(() => {
    ...
    navigate(spaceSectionPath(fallback.id, fallbackSection, studioKind, settingsSection, agentStudioActive), { replace: true });
  }, [activeSpace, agentStudioActive, navigate, props.loading, props.spaces, section, settingsSection, studioKind]);
  ...
  const switchSpace = (spaceId: string) => {
    ...
    navigate(spaceSectionPath(spaceId, nextSection, studioKind, settingsSection, agentStudioActive));
  };
  ...
  const sidebarContext = section === "chat" ? (
    <div className="grid gap-3">
      {enabledAgents.length ? <SpaceSidebarSection title="Agents">
        <nav className="grid gap-1" aria-label="Agent conversations">
          {enabledAgents.map((agent) => {
            ...
            return (
              <Link className={conversationLinkClass(search.get("agentId") === agent.id)} key={agent.id} to={`/spaces/${encodeURIComponent(activeSpaceId)}/chat?${params.toString()}`}>
                <span className="grid size-7 place-items-center rounded-md bg-sidebar-accent text-muted-foreground"><Bot size={14}/></span>
                <span className="min-w-0 truncate font-medium">{agent.name}</span>
              </Link>
            );
          })}
        </nav>
      </SpaceSidebarSection> : null}

      <SpaceSidebarSection title="Conversations">
        <nav className="grid gap-1" aria-label="Space conversations">
          <Link className={conversationLinkClass(!search.has("conversation") && !search.has("agentId"))} to={`/spaces/${encodeURIComponent(activeSpaceId)}/chat`}>
            <span className="grid size-7 place-items-center rounded-md bg-sidebar-accent text-muted-foreground"><Users size={14}/></span>
            <span className="min-w-0 truncate font-medium">Everyone</span>
          </Link>
          {groupConversations.map((conversation) => (
            <Link className={conversationLinkClass(search.get("conversation") === conversation.id)} key={conversation.id} to={`/spaces/${encodeURIComponent(activeSpaceId)}/chat?conversation=${encodeURIComponent(conversation.id)}`}>
              <span className="grid size-7 place-items-center rounded-md bg-sidebar-accent text-muted-foreground"><MessagesSquare size={14}/></span>
              <span className="min-w-0">
                <span className="block truncate font-medium">{conversation.title}</span>
                <span className="block truncate text-[9px] text-muted-foreground">{conversation.members.map((member) => member.user_id === user?.id ? "You" : member.name).join(", ")}</span>
              </span>
            </Link>
          ))}
          {members.some((member) => member.user_id !== user?.id) ? (
            <Button className="mt-1 h-9 w-full justify-start border-sidebar-border/60 bg-transparent px-2.5 text-xs text-sidebar-foreground shadow-none" variant="outline" type="button" onClick={() => setGroupDialogOpen(true)}>
              <Plus size={14}/><span>Add group chat</span>
            </Button>
          ) : null}
        </nav>
      </SpaceSidebarSection>
    ...
    <div className="grid gap-3">
      ...
      <section className="rounded-md bg-sidebar-accent/35 p-3" aria-label="Space storage quota">
        <div className="flex items-center gap-2.5">
          <span className="grid size-7 shrink-0 place-items-center rounded-md bg-sidebar-accent text-muted-foreground"><HardDrive size={14}/></span>
          <span className="min-w-0">
            <strong className="block text-[11px] font-medium text-sidebar-accent-foreground">Storage</strong>
            <span className="block text-[10px] text-muted-foreground">{libraryUsage ? `${formatStorageBytes(libraryUsage.remaining_bytes, libraryUsage.limit_bytes)} left` : "Checking..."}</span>
          </span>
        </div>
        <Progress className="mt-3 h-1.5" value={libraryUsage ? Math.max(0, Math.min(100, libraryUsage.used_bytes / Math.max(1, libraryUsage.limit_bytes) * 100)) : 0}/>
        {libraryUsage ? <p className="mb-0 mt-2 text-[9px] text-muted-foreground">{formatStorageBytes(libraryUsage.used_bytes)} of {formatStorageBytes(libraryUsage.limit_bytes)} used</p> : null}
      </section>
    </div>
  ) : section === "settings" ? (
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <Popover open={spacePickerOpen} onOpenChange={setSpacePickerOpen}>
      <PopoverTrigger asChild>
        <Button
          className="mb-3 h-10 w-full min-w-0 justify-start gap-2.5 border-sidebar-border/60 bg-sidebar-accent/35 px-2.5 text-left text-sidebar-accent-foreground shadow-none hover:bg-sidebar-accent"
          variant="outline"
          type="button"
        ...
        >
          <span className="grid size-6 shrink-0 place-items-center rounded-md bg-sidebar-accent text-muted-foreground"><Briefcase size={14}/></span>
          <span className="min-w-0 flex-1 truncate text-xs font-semibold">{activeSpace?.name ?? "Spaces"}</span>
          <ChevronDown className="shrink-0 text-muted-foreground" size={14}/>
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] min-w-[220px] p-1.5" align="start" sideOffset={6}>
        <p className="mb-1 px-2 py-1 text-[10px] font-semibold text-muted-foreground">Switch Space</p>
        <div className="misty-transient-scrollbar max-h-60 overflow-y-auto">
          {props.spaces.map((space) => (
            <div className={cn("group flex min-w-0 items-center rounded-md", space.id === activeSpaceId && "bg-accent")} key={space.id}>
              <Button
                className="h-auto min-h-9 min-w-0 flex-1 justify-start gap-2 bg-transparent px-2 py-1.5 text-left text-xs shadow-none hover:bg-accent"
                variant="ghost"
                type="button"
                aria-current={space.id === activeSpaceId ? "true" : undefined}
                onClick={() => { setSpacePickerOpen(false); switchSpace(space.id); }}
              >
                <span className="grid size-4 shrink-0 place-items-center text-primary">{space.id === activeSpaceId ? <Check size={13}/> : null}</span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate font-medium text-foreground">{space.name}</span>
                  <span className="block truncate text-[9px] font-normal text-muted-foreground">{space.is_personal ? "Personal Space" : space.is_shared ? "Shared Space" : "Private Space"}</span>
                </span>
              </Button>
              <Button
                ...
                onClick={() => { setSpacePickerOpen(false); navigate(`/spaces/${encodeURIComponent(space.id)}/settings/general`); }}
              >
                <Settings2 size={13}/>
              </Button>
            </div>
        ...
        <Button
          className="h-9 w-full justify-start px-2 text-xs shadow-none"
          variant="ghost"
          type="button"
          disabled={!canAddSpace}
          title={canAddSpace ? "New Space" : "You already own three Spaces"}
          onClick={() => { setSpacePickerOpen(false); props.onAddSpace(); }}
        >
    ...
    {props.loading && props.spaces.length === 0 ? <p className="mb-3 px-2 text-[10px] text-muted-foreground">Loading Spaces...</p> : null}
    {props.notices ? <div className="misty-transient-scrollbar mb-3 max-h-40 shrink-0 overflow-y-auto">{props.notices}</div> : null}
    {activeSpaceId ? <SpaceSectionNavigation spaceId={activeSpaceId} section={section} context={sidebarContext}/> : null}
      ...
      <DialogContent className="max-w-sm">
        <form onSubmit={(event) => void createGroup(event)}>
          <DialogHeader>
            ...
            <DialogDescription>Only selected members can see this conversation.</DialogDescription>
          </DialogHeader>
          <label className="mt-5 grid gap-2 text-xs font-medium text-muted-foreground">
            Group name
            <Input autoFocus maxLength={80} placeholder="Launch crew" value={groupTitle} onChange={(event) => setGroupTitle(event.target.value)}/>
          </label>
          <fieldset className="misty-transient-scrollbar mt-4 grid max-h-56 gap-1 overflow-y-auto border-0 p-0">
            ...
            {members.filter((member) => member.user_id !== user?.id).map((member) => <label className="flex min-h-10 items-center gap-3 rounded-md px-3 text-xs hover:bg-accent" key={member.user_id}><Checkbox checked={selectedMemberIds.includes(member.user_id)} onCheckedChange={(checked) => setSelectedMemberIds((current) => checked ? [...current, member.user_id] : current.filter((id) => id !== member.user_id))}/><span className="min-w-0"><span className="block truncate text-foreground">{member.name}</span><span className="block truncate text-[9px] text-muted-foreground">{member.email}</span></span></label>)}
          </fieldset>
          {groupError ? <p className="mb-0 mt-3 rounded-lg border border-destructive/25 bg-destructive/10 px-3 py-2 text-xs text-destructive" role="alert">{groupError}</p> : null}
          <DialogFooter className="mt-5">
            <Button variant="outline" type="button" disabled={groupSaving} onClick={closeGroupDialog}>Cancel</Button>
            <Button type="submit" disabled={groupSaving || !groupTitle.trim() || selectedMemberIds.length === 0}>{groupSaving ? "Creating..." : "Create group"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
function SpaceSidebarLink({ active, icon: Icon, label, to }: { active: boolean; icon: typeof Settings2; label: string; to: string }) {
  return <Link className={sidebarLinkClass(active)} to={to} aria-current={active ? "page" : undefined}><span className="grid size-5 place-items-center text-muted-foreground"><Icon size={14}/></span><span className="min-w-0 flex-1 truncate font-medium">{label}</span></Link>;
}

export function spaceSectionPath(spaceId: string, section: string, studioKind: string, settingsSection: string, agentStudioActive = false) {
  const destination = section === "studio" || agentStudioActive ? `agents/studio/${studioKind}` : section === "settings" ? `settings/${settingsSection}` : section;
  return `/spaces/${encodeURIComponent(spaceId)}/${destination}`;
}

function conversationLinkClass(isActive: boolean) { return cn("grid min-h-10 grid-cols-[28px_minmax(0,1fr)] items-center gap-2 rounded-md px-2 text-xs no-underline outline-none transition-colors focus-visible:ring-1 focus-visible:ring-sidebar-ring", isActive ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/65 hover:text-sidebar-accent-foreground"); }
function sidebarLinkClass(isActive: boolean) { return cn("flex h-9 min-w-0 items-center gap-2 rounded-md px-2.5 text-xs no-underline outline-none transition-colors focus-visible:ring-1 focus-visible:ring-sidebar-ring", isActive ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent/65 hover:text-sidebar-accent-foreground"); }
