...
import { spaceSectionPath } from "./SpacePanelContent";
...
describe("SpaceSectionNavigation", () => {
    ...
    await act(async () => {
      root.render(
        <MemoryRouter initialEntries={["/spaces/space-1/library"]}>
          <SpaceSectionNavigation spaceId="space-1" section="library" context={<p>Library context</p>}/>
        </MemoryRouter>,
      );
    });

    const links = [...container.querySelectorAll("a")];
    expect(links.map((link) => link.textContent?.trim())).toEqual(["Chat", "Agents", "Tasks", "Library", "Members", "Settings"]);
  ...
  it("hides inaccessible sections but keeps Studio-only Agent access visible", async () => {
    useSpacesStore.setState({ spaces: [spaceFixture({ permissions: {
      "messages.read": false,
      "agents.run": false,
      "studio.view": true,
      "tasks.view": false,
