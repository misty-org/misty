...
export function SpaceChat({ spaceId }: { spaceId: string }) {
  ...
  const {
    ...
    loadChatAgents,
    openNode,
  } = useSpacesStore(useShallow((state) => ({
  ...
  const activeConversation = groupConversations.find((conversation) => conversation.id === conversationId);
  ...
  const suggestions = useMemo<ChatComposerSuggestion[]>(() => {
    ...
    const library = canBrowseLibrary && pendingAttachments.length + selectedLibraryIds.length < 5
      ? libraryItems
    ...
    return [...peopleAndAgents, ...library]
      .filter((item) => !query || `${item.label} ${item.detail}`.toLocaleLowerCase().includes(query))
      .slice(0, 24);
  }, [agents, canBrowseLibrary, canRunAgents, libraryItems, members, pendingAttachments.length, selectedLibraryIds, suggestionQuery, user?.id]);
  ...
  useEffect(() => {
    ...
    setSuggestionsError("");
    void spacesApi.libraryItems(spaceId)
      .then((result) => setLibraryItems(result.items))
      .catch((error: unknown) => {
        setLibraryItems([]);
        setSuggestionsError(error instanceof Error ? error.message : "Library items could not be loaded.");
      })
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (conversationId) {
        ...
        setGroupMessages((current) => mergeSpaceMessages(current, [response.message, ...response.agent_replies]));
      } else {
        await sendMessage(
          spaceId,
          value,
      ...
      if (conversationId) setGroupChatError(reason instanceof Error ? reason.message : "The group message could not be sent.");
    }
  };
  ...
  const selectSuggestion = (suggestion: ChatComposerSuggestion) => {
    if (suggestion.kind === "library") {
      ...
      setSelectedLibraryIds((current) => current.includes(suggestion.id) ? current : [...current, suggestion.id]);
  ...
  const cancelEditing = (messageId: string) => {
    ...
    setEditingText("");
    window.setTimeout(() => {
      document.querySelector<HTMLButtonElement>(`#message-${CSS.escape(messageId)} button[aria-label="Message actions"]`)?.focus();
    }, 0);
  };
    ...
    return directAgent ? (
      ...
      <div className="grid h-full place-items-center p-8 text-center">
        <div className="max-w-sm">
          <span className="mx-auto grid size-12 place-items-center rounded-xl bg-muted/60"><Bot /></span>
          <h2 className="mt-4 text-base font-semibold">Agent chat unavailable</h2>
          <p className="mt-1 text-sm text-muted-foreground">This Space Agent is disabled, missing, or unavailable to your role.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="grid h-full min-h-0 grid-rows-[minmax(0,1fr)_auto] bg-background text-foreground">
      <SpaceChatMessages
        ...
        onSaveEdited={(event, message) => { void saveEditedMessage(event, message); }}
        ...
        onOpenNode={(nodeId) => { void openNode(spaceId, nodeId); }}
        onError={setGroupChatError}
        onLibraryItem={(item) => setLibraryItems((current) => [...current.filter((candidate) => candidate.id !== item.id), item])}
        onReload={() => { void loadMessages(spaceId); }}
      />

      {canWriteMessages ? (
        <form className="mx-[clamp(16px,4vw,56px)] mb-4 rounded-xl border border-border/80 bg-card p-2 shadow-xs" onSubmit={(event) => void submit(event)}>
          {replyToMessageId ? (
            <div className="mb-2 flex items-center justify-between rounded-md border-l-2 border-primary bg-muted px-3 py-1.5 text-xs text-muted-foreground">
              <span>Replying to {messages.find((item) => item.id === replyToMessageId)?.sender_name ?? "message"}</span>
              <Button variant="ghost" size="icon" className="size-6" type="button" onClick={() => setReplyToMessageId("")} aria-label="Cancel reply"><X /></Button>
            </div>
          ) : null}
          <Popover open={suggestionsOpen} onOpenChange={setSuggestionsOpen}>
            <PopoverAnchor asChild>
              <Textarea
                ...
                aria-activedescendant={suggestionsOpen && suggestions[activeSuggestionIndex] ? `${suggestionsListId}-option-${activeSuggestionIndex}` : undefined}
                role="combobox"
                maxLength={4000}
                ...
                onKeyDown={(event) => {
                  if (suggestionsOpen && event.key === "ArrowDown") { event.preventDefault(); setActiveSuggestionIndex((current) => Math.min(current + 1, Math.max(0, suggestions.length - 1))); return; }
                  if (suggestionsOpen && event.key === "ArrowUp") { event.preventDefault(); setActiveSuggestionIndex((current) => Math.max(0, current - 1)); return; }
                  if (suggestionsOpen && event.key === "Escape") { event.preventDefault(); setSuggestionsOpen(false); return; }
                  if (suggestionsOpen && event.key === "Enter" && !event.shiftKey) { event.preventDefault(); const selected = suggestions[activeSuggestionIndex]; if (selected) selectSuggestion(selected); return; }
                  if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); }
                }}
              />
            </PopoverAnchor>
            <PopoverContent side="top" align="start" sideOffset={10} className="w-[min(480px,calc(100vw-32px))] p-0" onOpenAutoFocus={(event) => event.preventDefault()}>
              <Command shouldFilter={false}>
                <CommandList id={suggestionsListId}>
                  <CommandGroup heading="Add to message">
                    {canBrowseLibrary ? <CommandItem onSelect={() => { setSuggestionsOpen(false); setLibraryBrowserOpen(true); }}><LibraryBig /><span><span className="block">Browse Library</span><span className="block text-xs text-muted-foreground">Select one or more shared items</span></span></CommandItem> : null}
                    {canUploadAttachments ? <CommandItem onSelect={() => { setSuggestionsOpen(false); setAttachmentPickerOpen(true); }}><Paperclip /><span><span className="block">Upload files</span><span className="block text-xs text-muted-foreground">Choose with Misty’s file picker</span></span></CommandItem> : null}
                  </CommandGroup>
                  <CommandSeparator />
                  <CommandGroup heading="Suggestions">
                    {suggestions.map((suggestion, index) => (
                      <CommandItem id={`${suggestionsListId}-option-${index}`} key={`${suggestion.kind}:${suggestion.id}`} aria-selected={index === activeSuggestionIndex} className={index === activeSuggestionIndex ? "bg-accent" : undefined} value={`${suggestion.kind}:${suggestion.id}`} onMouseEnter={() => setActiveSuggestionIndex(index)} onSelect={() => selectSuggestion(suggestion)}>
                        {suggestion.kind === "member" ? <Users /> : suggestion.kind === "agent" ? <Sparkles /> : <LibraryBig />}
                        <span className="min-w-0"><span className="block truncate">{suggestion.label}</span><span className="block truncate text-xs text-muted-foreground">{suggestion.detail}</span></span>
                      </CommandItem>
                    ))}
                  </CommandGroup>
                  {!suggestionsLoading && !suggestionsError && suggestions.length === 0 ? <CommandEmpty>No matching people, Agents, or Library items.</CommandEmpty> : null}
                  {suggestionsLoading ? <p className="p-3 text-sm text-muted-foreground">Loading Library…</p> : null}
                  {suggestionsError ? <p className="p-3 text-sm text-destructive">Library: {suggestionsError}</p> : null}
                </CommandList>
              </Command>
            </PopoverContent>
          </Popover>
          {pendingAttachments.length > 0 || selectedLibraryIds.length > 0 ? (
            <div className="flex flex-wrap gap-1.5 px-2 pb-2">
              {pendingAttachments.map((attachment) => <Button variant="secondary" size="sm" className="h-7" type="button" key={attachment.id} onClick={() => setPendingAttachments((current) => current.filter((item) => item.id !== attachment.id))}>{attachment.display_name}<X /></Button>)}
              {selectedLibraryIds.map((id) => <Button variant="secondary" size="sm" className="h-7" type="button" key={id} onClick={() => setSelectedLibraryIds((current) => current.filter((item) => item !== id))}><LibraryBig />{libraryItems.find((item) => item.id === id)?.display_name ?? "Library item"}<X /></Button>)}
            </div>
          ) : null}
          <div className="flex items-center justify-between gap-2 px-1">
            <div className="flex items-center gap-1">
              {canUploadAttachments ? <Button variant="ghost" size="icon" className="size-8" type="button" disabled={attachmentUploading || pendingAttachments.length + selectedLibraryIds.length >= 5} onClick={() => setAttachmentPickerOpen(true)} aria-label="Attach files"><Paperclip /></Button> : null}
              {canBrowseLibrary ? <Button variant="ghost" size="icon" className="size-8" type="button" onClick={() => setLibraryBrowserOpen(true)} aria-label="Browse Space Library"><LibraryBig /></Button> : null}
              <Button variant="ghost" size="sm" className="h-8 px-2 text-xs" type="button" onClick={() => { setText((current) => `${current}${current && !current.endsWith(" ") ? " " : ""}@`); openSuggestions(""); }}>@ Mention</Button>
            </div>
            <Button size="icon" className="size-8" disabled={sending || (!text.trim() && pendingAttachments.length === 0 && selectedLibraryIds.length === 0)} type="submit" aria-label="Send message"><Send /></Button>
          </div>
        </form>
      ) : (
        <div className="mx-[clamp(16px,4vw,56px)] mb-4 rounded-lg bg-muted/40 px-4 py-3 text-center text-sm text-muted-foreground" role="status">You can read this conversation, but you cannot send messages.</div>
      )}
