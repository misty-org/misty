...
export const MistyTaskDestinationSchema = z.strictObject({
  targetId: z.uuid(),
  containerReference: z.string().min(1).max(2048),
...
export const MistyTaskCreateResultSchema = z.strictObject({
  taskReference: z.string().min(1).max(2048),
  destination: MistyTaskDestinationSchema,
  title: z
    .string()
    .min(1)
...
export const mistyTaskCapabilities = [
  MistyCapabilityDefinitionSchema.parse({
    name: "tasks.create",
    version: 1,
    description:
      "Create one task in an explicitly resolved container and verify its content, destination, source and identity. " +
      "Unqualified requests visibly default to the originating Space Planner; missing Space or ambiguous external accounts/projects " +
      "require clarification. Reconcile an uncertain creation before any retry.",
    inputSchema: z.toJSONSchema(MistyTaskCreateInputSchema),
    outputSchema: z.toJSONSchema(MistyTaskCreateResultSchema),
