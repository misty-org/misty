...
import {
  ...
  providerUrlAllowed,
} from "../../features/browser/workspace/browserProviders";
import { browserProfileId } from "@/features/browser/workspace/browserProfileId";
...
it.each(["planner", "journal", "library"] as const)(
  ...
  async (app) => {
    ...
    for (const provider of integrationIds(app)) {
      ...
      expect(first.id).toBe(`default-${provider}`);
