...
export function ProviderWorkspace({
  ...
  }, [appId, misty, fail, report, attempt]);
  useEffect(() => {
    let cancelled = false;
    void misty.browser
      .availability()
  ...
  useEffect(() => {
    if (!ready) return;
    // API authorization is not a website session. Creating a browser profile
    // for each OAuth connection would bypass the shared provider sign-in.
