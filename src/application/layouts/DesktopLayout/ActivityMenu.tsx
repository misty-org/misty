      ...
      <PopoverContent side="top" align="start" sideOffset={10} className="w-[320px] p-0">
        <div className="flex items-center justify-between border-b border-charcoal-border/60 px-3 py-2">
          ...
          {unread.length > 0 ? (
            <button
              type="button"
              className="ml-auto rounded border-0 bg-transparent p-0 text-[11px] text-cream-muted hover:text-cream-bright"
        ...
        <div className="grid max-h-[380px] gap-0.5 overflow-y-auto p-1.5">
          {unread.length === 0 ? (
            <p className="m-0 px-2 py-6 text-center text-xs text-cream-muted">Nothing new.</p>
          ) : (
            unread.map((item) => (
              <button
                key={item.id}
              ...
              >
                ...
                {item.body ? (
                  <span className="truncate text-[11px] text-cream-muted">{item.body}</span>
                ) : null}
              </button>
            ))
          )}
