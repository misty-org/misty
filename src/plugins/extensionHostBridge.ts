...
const semanticTokenProperties: Record<SemanticThemeToken, string> = {
  background: "--misty-bg",
  surface: "--misty-surface",
  surfaceRaised: "--misty-surface-2",
  surfaceHover: "--misty-surface-hover",
  border: "--misty-border",
  borderStrong: "--misty-border-strong",
  text: "--misty-text",
  textMuted: "--misty-text-muted",
  textSubtle: "--misty-text-subtle",
  primary: "--misty-primary",
  primaryContrast: "--misty-primary-contrast",
  accent: "--misty-accent",
  focus: "--misty-interaction-focus",
  selection: "--misty-selection",
  success: "--misty-success",
  warning: "--misty-warning",
  danger: "--misty-danger",
...
let themeRevision = 1;
export function advanceThemeRevision() {
  themeRevision += 1;
}
...
export function themeSnapshot(): ThemeSnapshot {
  ...
  return {
    ...
    tokens: Object.fromEntries(
      Object.entries(semanticTokenProperties).map(([token, property]) => [
        token,
        styles.getPropertyValue(property).trim(),
      ]),
    ) as Record<SemanticThemeToken, string>,
  };
}
...
export function readEditableThemeTokens(): MistyCustomThemeTokens {
  ...
  return Object.fromEntries(
    Object.entries(customTokenProperties).map(([token, property]) => [
      token,
      styles.getPropertyValue(property).trim(),
    ]),
  ) as MistyCustomThemeTokens;
}
...
export function applyThemeTokenPreview(tokens: MistyCustomThemeTokens | null) {
  ...
  const derived: Record<keyof MistyCustomThemeTokens, Record<string, string>> = {
    background: {
      "--misty-page-bg": tokens?.background ?? "",
      "--misty-nav-bg": tokens?.background ?? "",
      "--misty-bg-soft": tokens?.background
        ? `color-mix(in srgb, ${tokens.background} 92%, var(--misty-text) 8%)`
        : "",
    },
    surface: {
      "--misty-surface-2": tokens?.surface
        ? `color-mix(in srgb, ${tokens.surface} 91%, var(--misty-text) 9%)`
        : "",
      "--misty-surface-3": tokens?.surface
        ? `color-mix(in srgb, ${tokens.surface} 84%, var(--misty-text) 16%)`
        : "",
      "--misty-surface-hover": tokens?.surface
        ? `color-mix(in srgb, ${tokens.surface} 82%, var(--misty-text) 18%)`
        : "",
      "--misty-dropdown-bg": tokens?.surface ?? "",
    },
    foreground: { "--misty-primary": tokens?.foreground ?? "" },
    muted: {
      "--misty-text-subtle": tokens?.muted
        ? `color-mix(in srgb, ${tokens.muted} 72%, var(--misty-bg) 28%)`
        : "",
    },
    accent: {
      "--misty-accent-strong": tokens?.accent ?? "",
      "--misty-interaction-focus": tokens?.accent
        ? `color-mix(in srgb, ${tokens.accent} 78%, transparent)`
        : "",
      "--misty-focus-ring": tokens?.accent
        ? `color-mix(in srgb, ${tokens.accent} 24%, transparent)`
        : "",
    },
    selection: {},
    success: {},
  ...
  Object.values(derived).forEach((properties) =>
    Object.entries(properties).forEach(([property, value]) =>
      value ? root.style.setProperty(property, value) : root.style.removeProperty(property),
    ),
  );
...
const isThemeId = (value: unknown): value is MistyThemeId =>
  value === "misty-dark" ||
  value === "misty-light" ||
  value === "graphite" ||
  value === "aurora" ||
...
export async function handleSharedExtensionCommand(
  command: string,
  ...
  if (command === "themes.snapshot")
    return {
      ...
      theme: themeSnapshot(),
      tokens: store.customTokens ?? readEditableThemeTokens(),
    };
  ...
  if (command === "themes.preview") {
    ...
    return {
      ...
      tokens,
      theme: themeSnapshot(),
      message: "Preview updated. Apply to keep these colors.",
    };
  }
  if (command === "themes.apply") {
    ...
    return {
      ...
      tokens,
      theme: themeSnapshot(),
      message: "Theme saved and applied across Misty.",
    };
  }
  if (command === "themes.revert") {
    ...
    return {
      ok: true,
      tokens: current.customTokens ?? readEditableThemeTokens(),
      theme: themeSnapshot(),
      message: "Reverted to the saved theme.",
    };
  }
  return undefined;
}
