...
const InputGroup = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, ...props }, ref) => (
    <div
      ...
      className={cn(
        "flex w-full flex-col overflow-hidden rounded-lg border border-charcoal-border bg-transparent shadow-xs transition-[color,box-shadow] bg-charcoal-card",
        className,
      )}
      {...props}
    />
  ...
  ({ className, ...props }, ref) => (
    <textarea
      ...
      className={cn(
        ...
        "text-base outline-none placeholder:text-cream-muted disabled:cursor-not-allowed",
        "disabled:opacity-50 md:text-sm bg-transparent",
        className,
      )}
      {...props}
    />
...
const InputGroupText = React.forwardRef<HTMLSpanElement, React.ComponentProps<"span">>(
  ({ className, ...props }, ref) => (
    <span
      ...
      className={cn("shrink-0 text-xs text-cream-muted", className)}
      {...props}
    />
  ),
);
