...
export function useDocumentSurfaceVariables(surfaceStyle: CSSProperties) {
  useEffect(() => {
    ...
    const previous = variables.map(([name]) => [name, rootStyle.getPropertyValue(name), rootStyle.getPropertyPriority(name)] as const);
    variables.forEach(([name, value]) => rootStyle.setProperty(name, String(value)));
    return () => previous.forEach(([name, value, priority]) => value
