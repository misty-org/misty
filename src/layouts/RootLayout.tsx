...
function AppZoomIndicator(props: { visible: boolean; percent: number }) {
  return (
    <div
      className={`pointer-events-none fixed left-1/2 top-[calc(var(--misty-window-titlebar-inset)+12px)] z-[2147483000] -translate-x-1/2 rounded-full border border-[#2f3338] bg-[#07090b] px-3 py-1.5 text-sm font-semibold text-[#f4f4f5] shadow-[0_12px_32px_rgba(0,0,0,0.52)] transition duration-150 ${props.visible ? "translate-y-0 opacity-100" : "-translate-y-2 opacity-0"}`}
      role="status"
      aria-live="polite"
