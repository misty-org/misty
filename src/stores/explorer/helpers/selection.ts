...
import type { ExplorerStore, PaneExplorerState, ExplorerBatchRenameItem, ExplorerDeleteMode, ExplorerDialogState, ExplorerInlineEditState, ExplorerNotificationType, ExplorerSortState } from "../types";
...
export function paneExplorerStatesEqual(left: PaneExplorerState, right: PaneExplorerState): boolean {
  return left === right || (
...
export function makeBatchRenameItem(entry: FileEntry, paneId: string, pane: PaneExplorerState): ExplorerBatchRenameItem {
...
export function syncInlineRenameSelection(
  ...
  const focusItem = items.find((item) => item.paneId === preferredPaneId && item.entryId === preferredEntryId)
    ...
    ?? items[0];
  const batchItems = H.validateBatchRenameItems(items.map((item) => ({ ...item, value: edit.value })));
...
export function withInlineEditValidation(
  ...
  if (edit.kind === "rename" && edit.batchItems && edit.batchItems.length > 1) {
    const batchItems = H.validateBatchRenameItems(edit.batchItems.map((item) =>
  ...
  const error = H.validateRenameValue(edit.value, edit.lockedExtension, edit.originalName, edit.entryId, pane);
  return { ...edit, error };
}
