...
import type { ExplorerStore, PaneExplorerState, ExplorerBatchRenameItem, ExplorerDialogState, ExplorerInlineEditState, ExplorerNotificationType, ExplorerSortState } from "../types";
import { explorerRuntime, getExplorerStore } from "../runtime";
import * as H from "./index";

export function explorerWorkspaceNeedsSave(
  state: ExplorerStore,
  previous: ExplorerStore,
): boolean {
  if (
...
export async function persistExplorerWorkspace(): Promise<void> {
  ...
  const workspace: NativeWorkspace = {
    ...
    tabs,
    explorer: tabs.find((tab) => tab.idx === H.tabIndex(activeTab.id, 0))?.explorer ?? tabs[0].explorer,
  };
...
export async function applyWorkspaceDocument(document: NativeWorkspaceDocument, homePath: string): Promise<void> {
  ...
  if (!multi.hydrate(restored.multiPanel)) {
    ...
    await getExplorerStore().getState().loadPane(useMultiPanelStore.getState().activePaneId || "explorer-pane-0", homePath, "replace");
    return;
  }
...
export async function saveWorkspaceDocument(document: NativeWorkspaceDocument): Promise<NativeWorkspaceDocument> {
...
export function consumeExplorerWorkspaceResetFlag(): boolean {
  ...
  try {
    const shouldReset = window.localStorage.getItem(explorerRuntime.explorerWorkspaceResetKey) === "1";
...
export function workspaceMetadata(document: NativeWorkspaceDocument): Pick<ExplorerStore, "workspaceEntries" | "activeWorkspaceId" | "activeWorkspaceTitle"> {
