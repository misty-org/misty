...
import type { ExplorerStore, ExplorerViewMode, PaneExplorerState, ExplorerBatchRenameItem, ExplorerDialogState, ExplorerInlineEditState, ExplorerNotificationType, ExplorerSortState } from "../types";
import { explorerRuntime, getExplorerStore } from "../runtime";
import * as H from "./index";

export function restoreNativeWorkspace(document: NativeWorkspaceDocument, homePath: string): {
  ...
  const tabs = nativeTabs.map((nativeTab, tabPosition): MultiPanelTab => {
    ...
    const restoredPanes = paneSnapshots.map((paneSnapshot, panePosition): MultiPanelPane => {
      ...
      return { id: paneId, path: restored.path, title: tabSnapshot?.title || H.titleFromPath(restored.path) };
    });
    const lanes = H.normalizeWorkspaceLanes(explorer.grid_pane_ids, restoredPanes.map((pane) => pane.id));
...
export function workspaceLanesForTab(tab: MultiPanelTab): string[][] {
  ...
  return H.normalizeWorkspaceLanes(tab.layout.lanes ?? H.lanesFromFlatWorkspaceLayout(tab), tab.panes.map((pane) => pane.id))
...
export function normalizeWorkspaceLanes(sourceLanes: string[][], fallbackPaneIds: string[]): string[][] {
...
export function parsePaneRestoreState(value: string | undefined, fallbackPath: string): {
  path: string;
  showHidden: boolean;
  gridView: boolean;
  sort: ExplorerSortState;
