...
import type { ExplorerNotification, ExplorerStore, NavigationMode, PaneExplorerState, ExplorerBatchRenameItem, ExplorerDialogState, ExplorerInlineEditState, ExplorerNotificationType, ExplorerSortState } from "../types";
...
export function shouldShowLoadingSkeleton(pane: PaneExplorerState, path: string): boolean {
  ...
  const mountRoot = H.resolveMountRoot(storageHome, advanced.mountPath || environment.mountPath || ".misty/mnt");
...
export function applyNavigationResult(
  ...
  if (mode === "push" && previousPath && !H.samePath(previousPath, listing.path)) {
    if (!H.samePath(backHistory[backHistory.length - 1] ?? "", previousPath)) backHistory.push(previousPath);
    forwardHistory = [];
  } else if (mode === "back" && previousPath) {
    backHistory = backHistory.slice(0, -1);
    if (!H.samePath(forwardHistory[forwardHistory.length - 1] ?? "", previousPath)) forwardHistory.push(previousPath);
...
export function scheduleExplorerWorkspaceSave(): void {
  ...
  if (explorerRuntime.workspaceSaveTimer !== null) window.clearTimeout(explorerRuntime.workspaceSaveTimer);
...
export function queuePaneRefresh(paneId: string, path: string, options: { immediate?: boolean } = {}): void {
  if (H.isExplorerInternalTabPath(path)) return;
  const key = `${paneId}\n${H.normalizedPath(path)}`;
  const pending = explorerRuntime.pendingPaneRefreshes.get(key) ?? { firstTimer: null, followupTimer: null };
...
export function transferTouchesDirectory(row: TransferRecord, directoryPath: string, mountRoot: string): boolean {
  const remote = H.remoteBrowseTargetForPath(directoryPath, mountRoot);
  if (remote) {
    return H.remoteTransferMatchesDirectory(remote, row.remoteSourceName, row.remoteSourcePath)
      || H.remoteTransferMatchesDirectory(remote, row.remoteDestName, row.remoteDestPath);
  }
  return H.localTransferMatchesDirectory(directoryPath, row.localSourcePath)
    || H.localTransferMatchesDirectory(directoryPath, row.localDestPath);
}

export function localTransferMatchesDirectory(directoryPath: string, candidatePath: string): boolean {
...
export function remoteBrowseTargetForPath(path: string, mountRoot: string): { remoteName: string; remotePath: string } | null {
