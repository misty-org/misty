...
import type { ExplorerClipboardState, ExplorerStore, PaneExplorerState, ExplorerBatchRenameItem, ExplorerDialogState, ExplorerInlineEditState, ExplorerNotificationType, ExplorerSortState } from "../types";
...
export async function setAssociationForPath(filePath: string, applicationPath: string): Promise<void> {
  await explorerSetOpenAssociation(filePath, applicationPath);
}

export async function pasteSystemClipboardTextIntoPane(paneId: string, directory: string): Promise<boolean> {
...
export async function pasteSystemClipboardImageIntoPane(paneId: string, directory: string): Promise<boolean> {
...
export async function writeNativeOrTextClipboardForSelection(
  ...
  if (remoteEntries.length > 0) {
    getExplorerStore().getState().pushNotification(
...
export function explorerClipboardFromPayload(payload: ClipboardPayload): ExplorerClipboardState | null {
...
export function selectedFileEntriesForPane(pane: PaneExplorerState | undefined): FileEntry[] {
  ...
  const selected = new Set(pane.selectedIds);
  return pane.listing.entries.filter((entry) => selected.has(entry.id) && H.isFileMasterEntry(entry));
}
...
export function pasteItemFromClipboardRef(fileRef: ClipboardPayload["file_refs"][number]): PasteItem | null {
  ...
  const mountPath = H.resolveMountRoot(environment.homeDir, settingsMountPath || environment.mountPath);
  return {
    path: H.remoteClipboardVirtualPath(mountPath, fileRef.provider_type, fileRef.remote_name, fileRef.remote_path),
