...
import type { ExplorerSortColumn, ExplorerSortDirection, ExplorerStore, ExplorerViewMode, PaneExplorerState, ExplorerBatchRenameItem, ExplorerDialogState, ExplorerInlineEditState, ExplorerNotificationType, ExplorerSortState } from "../types";
...
export function mergeDirectorySizeRecords(setState: ExplorerStoreSetter, records: DirectorySizeRecord[]): void {
...
export function markDirectorySizesCalculating(setState: ExplorerStoreSetter, paths: string[]): void {
  setState((state) => {
    const directorySizes = { ...state.directorySizes };
    let changed = false;
    for (const path of paths) {
...
export function markDirectorySizesFailed(setState: ExplorerStoreSetter, paths: string[], message: string): void {
  setState((state) => {
    const directorySizes = { ...state.directorySizes };
    for (const path of paths) {
      const key = H.normalizedPath(path);
...
export function directorySizeRecordsEqual(left: DirectorySizeRecord | undefined, right: DirectorySizeRecord): boolean {
  return Boolean(left)
...
export function compareEntries(
  ...
  if (column === "modified") {
    return H.compareNullableNumber(left.modifiedMs, right.modifiedMs)
      || H.compareText(left.remoteModified, right.remoteModified)
      || H.compareText(left.name, right.name);
  }
  if (column === "size") {
    return H.compareNullableNumber(H.entrySizeBytes(left, directorySizes), H.entrySizeBytes(right, directorySizes))
      || H.compareText(left.name, right.name);
  }
  if (column === "type") {
    return H.compareText(H.typeLabel(left), H.typeLabel(right)) || H.compareText(left.name, right.name);
  }
