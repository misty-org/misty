...
import type { ExplorerGet, ExplorerInlineEditState, ExplorerSet, ExplorerStore } from "../types";
...
export function createMutationsActions(set: ExplorerSet, get: ExplorerGet): Partial<ExplorerStore> {
  return {
  canCreateItem: (paneId, kind) => H.canCreateItemInPane(get().panes[paneId], kind, get().inlineEdit),
  ...
  confirmDialog: async () => {
    ...
    if (dialog.kind === "delete") {
      ...
      try {
        ...
        H.refreshTransferViews();
        get().pushNotification(H.deleteQueuedMessage(dialog.paths.length, dialog.permanent), "success");
