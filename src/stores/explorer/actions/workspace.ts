...
export function createWorkspaceActions(set: ExplorerSet, get: ExplorerGet): Partial<ExplorerStore> {
  ...
  setLibraryTags: async (entry, tags) => {
    try {
      set({ library: await explorerLibrarySetTags(H.libraryItemFromEntry(entry), tags) });
    } catch (error) {
  ...
  setLibraryMetadata: async (entry, tags, comments) => {
    try {
      set({ library: await explorerLibrarySetMetadata(H.libraryItemFromEntry(entry), tags, comments) });
    } catch (error) {
  ...
  initialize: async (homePath) => {
    ...
    const fallbackDocument = explorerRuntime.workspaceDocumentCache ?? H.defaultWorkspaceDocument();
  ...
  createWorkspace: async (title, homePath) => {
    const name = H.uniqueWorkspaceTitle(title.trim() || "Workspace", explorerRuntime.workspaceDocumentCache?.workspaces ?? get().workspaceEntries);
    try {
      await H.persistExplorerWorkspace();
