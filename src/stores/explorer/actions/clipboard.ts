...
export function createClipboardActions(set: ExplorerSet, get: ExplorerGet): Partial<ExplorerStore> {
  ...
  pasteIntoPane: async (paneId) => {
    ...
    try {
      ...
      if (clipboard?.operation === "move" && clipboard.items.length > 0) {
        ...
        get().pushNotification(`Queued move for ${H.itemCountLabel(clipboard.items.length)}`, "success");
        set({ clipboard: null });
      ...
      H.refreshTransferViews();
      get().pushNotification(`Queued copy for ${H.itemCountLabel(clipboard.items.length)}`, "success");
      H.queuePaneRefresh(paneId, directory);
    } catch (error) {
  ...
  dropItems: async (paneId, items, destination, operation) => {
    ...
    try {
      ...
      H.refreshTransferViews();
      get().pushNotification(`Queued ${operation === "move" ? "move" : "copy"} for ${H.itemCountLabel(items.length)}`, "success");
  ...
  pollTransferRefreshes: async (mountRoot) => {
    try {
      ...
      if (!explorerRuntime.transferRefreshObserverReady) {
        ...
        explorerRuntime.transferRefreshWatermarkMs = Math.max(0, ...completedRows.map((row) => row.completedAtMs));
        explorerRuntime.transferRefreshStatuses = Object.fromEntries(page.rows.map((row) => [row.id, row.status]));
        return;
      }
      ...
      const newlyCompleted = completedRows.filter((row) => {
        ...
        return previousStatus !== "completed" || row.completedAtMs > explorerRuntime.transferRefreshWatermarkMs;
      });
      if (newlyCompleted.length === 0) return;
      explorerRuntime.transferRefreshWatermarkMs = Math.max(explorerRuntime.transferRefreshWatermarkMs, ...newlyCompleted.map((row) => row.completedAtMs));

      const panes = get().panes;
      for (const [paneId, pane] of Object.entries(panes)) {
        const currentPath = pane.listing?.path;
        if (!currentPath) continue;
        if (newlyCompleted.some((row) => H.transferTouchesDirectory(row, currentPath, mountRoot))) {
