...
import { approveSmartLibrarySample, completeSemanticReindex, planSemanticReindex, registerSmartLibraryFolder, searchSemanticAssets } from "./smartLibraryServerApi";
...
describe("Smart Library managed API", () => {
  ...
  it("rejects analysis batches larger than eight before network usage", () => {
    ...
    const assets = Array.from({ length: 9 }, (_, index) => ({ assetId: `asset_${index}`, fingerprint: `fp_${index}`, mimeType: "image/jpeg", assetKind: "image", base64: "preview" }));
  ...
  it("submits bounded extracted text for non-visual files without inventing image bytes", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({
      folderId: "folder_1", phase: "sample_processing", successfulImages: 0, failedImages: 0, queuedImages: 1,
      batches: [], estimate: {}, nextResultSequence: 0,
    }), { status: 200, headers: { "Content-Type": "application/json" } }));

    await approveSmartLibrarySample("folder_1", [{
      assetId: "asset_doc",
    ...
    const body = JSON.parse(fetchMock.mock.calls[0]?.[1]?.body as string) as { previews: Array<Record<string, unknown>> };
    expect(body.previews[0]).not.toHaveProperty("base64");
    expect(body.previews[0]).toMatchObject({ assetKind: "document", extractedText: "A bounded report excerpt", truncated: true });
    expect(JSON.stringify(body)).not.toContain("/Users/");
  });

  it("searches the account catalog without sending device paths", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({
      hits: [], queryModel: "embedding", indexVersion: "2",
    }), { status: 200, headers: { "Content-Type": "application/json" } }));
  ...
  it("plans and explicitly completes bounded path-free reindex work", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(new Response(JSON.stringify({
        jobId: "job_1", status: "planned", targetVersion: 2, embeddingModel: "embedding", assets: [],
      }), { status: 200, headers: { "Content-Type": "application/json" } }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
        jobId: "job_1", status: "completed", completedAssets: 1, failedAssets: 0,
      }), { status: 200, headers: { "Content-Type": "application/json" } }));
    ...
    expect(JSON.parse(fetchMock.mock.calls[0]?.[1]?.body as string)).toEqual({ folderId: "folder_1", limit: 100 });
    const completion = JSON.parse(fetchMock.mock.calls[1]?.[1]?.body as string) as Record<string, unknown>;
  ...
  it("shows a safe server message instead of raw JSON for media failures", async () => {
    ...
    await expect(managedAiRequest("/ai/media-search/chunks", { method: "POST", body: "{}" }))
