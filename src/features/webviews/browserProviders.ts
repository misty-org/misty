...
export function providerUrlAllowed(providerId: BrowserProviderId, value: string) {
  try {
...
export function providerBelongsToApp(appId: string, providerId: BrowserProviderId) {
...
export function registerProviderBrowser(id: string, value: ProviderBrowserProfile) {
  views.set(id, value);
  return () => {
    views.delete(id);
  };
...
export const liveProviderBrowserProfiles = () =>
  [...views].map(([id, profile]) => ({ id, ...profile }));
const popupStorageKey = (tabId: string) => `misty:provider-browser-tab:${tabId}`;
export function popupBrowserProfile(tabId: string): ProviderBrowserProfile | undefined {
  const active = popups.get(tabId);
  if (active) return active;
  try {
    const value = JSON.parse(localStorage.getItem(popupStorageKey(tabId)) ?? "null");
    if (
      value &&
      /^[a-f0-9]{64}$/.test(value.profileId) &&
      typeof value.ownerAccountId === "string" &&
      typeof value.serverBase === "string" &&
      Object.prototype.hasOwnProperty.call(browserProviders, value.provider?.id) &&
      ...
      return {
        ...
        ownerAppId: typeof value.ownerAppId === "string" ? value.ownerAppId : undefined,
      };
    }
  } catch {
...
export function forgetProviderProfile(profileId: string) {
  ...
  try {
    for (const key of Object.keys(localStorage)) {
      ...
      if (JSON.parse(localStorage.getItem(key) ?? "null")?.profileId === profileId)
        localStorage.removeItem(key);
    }
  } catch {
    /* Ephemeral popup identities were already released. */
  }
}
