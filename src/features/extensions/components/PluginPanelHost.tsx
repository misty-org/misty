...
import { pluginPanelRender } from "@/native/filesystem";
...
import { pluginTabHostStyles } from "./pluginPanelStyles";
...
export function ExplorerNativePluginPanelHost(props: {
  ...
  if (!rendered && !renderError && props.mode === "app") {
        ...
        onRequest={async (message) => {
          ...
          if (command === "host.notify") {
            ...
            useActivityStore.getState().ingestLocal({
              appId: props.panel.pluginId,
              kind: level === "error" ? "failure" : level === "success" ? "completion" : "system",
              title: props.panel.title,
              body:
