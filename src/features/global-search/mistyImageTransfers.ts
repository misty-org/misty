...
export function createMistyImageTransfers(
  apiRequest: <T = void>(path: string, init?: RequestInit) => Promise<T>,
  transferLibraryObject: (
    transfer: Transfer,
    file: Blob,
    generation: number,
    options: { onProgress?: (value: number) => void },
  ) => Promise<void>,
  readApiSessionGeneration: () => number,
) {
  const accepted = new Set(["image/jpeg", "image/png", "image/webp"]);
  const maxMistyImageBytes = 10 * 1024 * 1024;
  ...
  function validateMistyImage(file: File) {
  ...
  async function uploadMistyImage(
    file: File,
    input: {
  ...
  async function deleteMistyImage(attachment: MistyImageAttachment) {
  ...
  function captureToFile(capture: { dataUrl: string; name: string; mimeType: string }) {
  ...
  async function modelRendition(
    ...
    return {
      file: new File([blob], `${filename}.model.jpg`, { type: "image/jpeg" }),
      width,
      height,
    };
  }
  ...
  return { uploadMistyImage, deleteMistyImage };
}
