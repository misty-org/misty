...
const fixture = vi.hoisted(() => ({ state: {} as any, local: vi.fn(), register: vi.fn(), bind: vi.fn() }));
vi.mock("./browserAskTargets", () => ({ bindBrowserAskTarget: fixture.bind }));
vi.mock("@/features/agents/store/useAgentsStore", () => ({ agentsDeviceSnapshot: fixture.local }));
vi.mock("@/features/agents/store/useAgentDeviceStore", () => ({ ensureServerAgentDevice: fixture.register }));
vi.mock("./useGlobalSearchStore", () => ({ useGlobalSearchStore: {
  getState: () => fixture.state,
  setState: (next: object) => Object.assign(fixture.state, next),
} }));
import { browserAskSource, openBrowserAsk, type BrowserAskSnapshot } from "./browserAskContext";
const snapshot = (): BrowserAskSnapshot => ({
  id: "view", scopeId: "host-scope", spaceId: "family", revision: "100:42", contentHash: "hash",
  page: { url: "https://mail.google.com/mail/u/0/#inbox/thread", title: "Pilot message", content: "Please reply", selection: true, editable: false, link: "", image: "", documentRevision: "100:42" },
});
beforeEach(() => {
  fixture.state = { accountId: "owner", working: false, context: [{ spaceId: "other" }], newConversation: vi.fn(async () => "new"), openPanel: vi.fn() };
...
it("captures the originating Space even if navigation changes while device registration is pending", async () => {
  fixture.register.mockImplementation(async () => { fixture.state.context = [{ spaceId: "work" }]; return { id: "device" }; });
  await openBrowserAsk(snapshot());
  expect(fixture.state.newConversation).toHaveBeenCalledWith("family");
  expect(fixture.state.browserRequest.context[0].spaceId).toBe("family");
  expect(fixture.state.browserRequest.deviceContexts[0]).toMatchObject({ opaqueRef: "host-scope", capabilities: ["browser.inspect"] });
  expect(fixture.state.query).toBe("");
  expect(fixture.state.openPanel).toHaveBeenCalledOnce();
});
it("keeps no-Space sources global without attaching automation or inheriting another conversation", async () => {
  ...
  expect(fixture.state.newConversation).toHaveBeenCalledWith(undefined);
...
it("abandons context if the Misty account changes during registration", async () => {
  fixture.register.mockImplementation(async () => { fixture.state.accountId = "someone-else"; return { id: "device" }; });
  await openBrowserAsk(snapshot());
...
it("rejects non-web and oversized snapshots before opening a conversation", async () => {
  const source = snapshot();
  expect(() => browserAskSource({ ...source, page: { ...source.page, url: "javascript:alert(1)" } })).toThrow();
  await expect(openBrowserAsk({ ...source, page: { ...source.page, content: "x".repeat(32001) } })).rejects.toThrow();
  expect(fixture.state.newConversation).not.toHaveBeenCalled();
});
it("attaches semantic browser actions only after target admission and retains the observed thread", async () => {
  fixture.bind.mockResolvedValue({ account: "pilot@example.com", target: { id: "target" }, capabilities: ["inbox.read", "inbox.draft", "inbox.send"], threadReference: snapshot().page.url });
  ...
  expect(request.context[0].metadata).toMatchObject({ targetId: "target", account: "pilot@example.com" });
...
it("pre-fills a native menu suggestion without submitting it", async () => {
  ...
  expect(fixture.state.query).toBe("Create a task from this email in this Space's Planner and prepare a reply for my review.");
