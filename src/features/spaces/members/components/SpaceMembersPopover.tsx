...
import type { Space, SpaceMember } from "@/api/spaces/dto/interfaces/types";
      ...
      <PopoverContent sideOffset={8} className="w-72 overflow-hidden border-charcoal-border/70 p-0">
        <div className="flex items-center justify-between border-b border-charcoal-border/60 px-2 py-2.5">
          <div className="min-w-0">
            ...
            <p className="mb-0 mt-0.5 text-[11px] text-cream-muted">
              {members.length || space.member_count} teammates in {space.name}
            </p>
          </div>
          {onlineCount > 0 ? (
        ...
        <div className="misty-transient-scrollbar max-h-[min(420px,65vh)] overflow-y-auto px-1 py-1.5">
          {loading && members.length === 0 ? (
            ...
            <TeamRows members={orderedMembers} />
          )}
        </div>

        {space.role === "owner" || space.permissions?.["space.invite"] === true ? (
...
function TeamRows({ members }: { members: PresentMember[] }) {
  return (
