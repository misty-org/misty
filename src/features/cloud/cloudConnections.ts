...
export function cloudConnectionsSnapshot(): Promise<CloudConnectionsSnapshot> {
  return spaceRequest("/cloud/connections");
}
...
export function beginCloudAuthorization(request: {
  ...
  return spaceRequest(`/cloud/connections/${encodeURIComponent(request.provider)}/authorize`, {
    method: "POST",
    body: JSON.stringify({
      name: request.name,
...
export function cloudConnectionToken(connectionId: string): Promise<CloudTokenLease> {
  return spaceRequest(`/cloud/connections/${encodeURIComponent(connectionId)}/token`, {
    method: "POST",
  });
}

export function deleteCloudConnection(connectionId: string): Promise<void> {
  return spaceRequest(`/cloud/connections/${encodeURIComponent(connectionId)}`, {
    method: "DELETE",
  });
}
