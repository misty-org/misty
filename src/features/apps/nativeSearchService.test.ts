...
beforeEach(() => {
  ...
  f.service.mockImplementation(
    async (_app: string, _space: string, run: (instance: string) => Promise<unknown>) => {
      try {
...
it("keeps personal authority until the background scan completes", async () => {
  f.invoke
    .mockResolvedValueOnce({ scanInProgress: true })
    .mockResolvedValueOnce({ scanInProgress: true })
    .mockResolvedValueOnce({ scanInProgress: false });
  expect(
    await invokeFilesSearch("search_start_scan", { request: { roots: ["/approved"] } }),
  ).toEqual({ scanInProgress: true });
  ...
  expect(f.service).toHaveBeenCalledWith("files", "", expect.any(Function), undefined, "search");
...
it("rejects startup failures and scopes queries through the installed app", async () => {
  ...
  expect(f.invoke).toHaveBeenLastCalledWith("search_query", {
    request: { query: "photo" },
    instance: "owned",
  });
});
