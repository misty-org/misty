...
// Keep no successful cache: the next operation still checks current account authority.
...
function readInstallations(spaceId: string) {
  ...
  const request = Promise.resolve()
    .then(() => {
      ...
      return appsApi.installations(spaceId);
    })
    .finally(() => {
...
export async function withNativeDocumentService<T>(
  ...
  try {
    assert();
    const installed = (await readInstallations(spaceId)).apps.find(
      (app) => app.app_id === appId && app.state === "installed" && !app.consent_required,
    );
    assert();
    if (!installed)
      throw new Error(
        `Install ${appId === "files" ? "Files" : "Storage"} for your account to use ${purpose}.`,
      );
    ...
    if (
      !app ||
      app.id !== appId ||
      app.version !== installed.installed_version ||
      !app.desktop.sha256
    )
    ...
    if (
      ...
      !!session.space_id ||
      capabilities.some(
        (capability) => !session.scopes.includes(capability) || !app.scopes.includes(capability),
      )
    )
    ...
    const matches = (entry: typeof installed | undefined) =>
      entry?.state === "installed" &&
      !entry.consent_required &&
      entry.authority_generation === installed.authority_generation &&
      entry.installed_version === installed.installed_version &&
      capabilities.every((capability) => entry.granted_scopes.includes(capability));
    stopStore = useAppsStore.subscribe((state) => {
      try {
        assert();
        if (state.ready && !matches(state.installations.find((app) => app.app_id === appId)))
          invalidate();
      } catch {
        invalidate();
      }
    ...
    if (!(await officialDesktopPackageReady(app, true))) {
      ...
      if (!(await officialDesktopPackageReady(app, true)))
        throw new Error("The native service package could not be verified.");
    }
    assert();
    const root = await invoke<string>("official_app_package_path", {
      pluginId: app.id,
    ...
    assert();
    instance = await invoke<string>("mini_widget_open", {
      request: {
        root,
        owner: {
