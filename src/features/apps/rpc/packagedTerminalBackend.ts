    ...
    async invoke<T>(command: string, args: Record<string, unknown> = {}): Promise<T> {
      ...
      if (command === "terminal_create") {
        try {
          ...
          return await backend.invoke<T>("terminal_service_create", { instance, request: args.request });
        } catch {
          const res = await backend.invoke<T>("terminal_create", args);
          if (typeof res === "string") {
            hostSessions.add(res);
          }
          return res;
        }
      }

      if (typeof args.sessionId === "string" && hostSessions.has(args.sessionId)) {
        if (command === "terminal_kill") {
          hostSessions.delete(args.sessionId);
        }
