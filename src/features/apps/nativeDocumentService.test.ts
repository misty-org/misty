...
const f = vi.hoisted(() => ({
  ...
  install: vi.fn(),
  account: vi.fn(),
  listener: undefined as undefined | ((s: any) => void),
  unsubscribe: vi.fn(),
}));
vi.mock("@tauri-apps/api/core", () => ({ invoke: f.invoke }));
vi.mock("@/api/apps", () => ({
  appsApi: { installations: f.installations, createSession: f.session },
}));
...
vi.mock("./desktopPackages", () => ({
  officialDesktopPackageReady: f.ready,
  installOfficialDesktopPackage: f.install,
}));
vi.mock("./useAppsStore", () => ({
  useAppsStore: {
    getState: () => ({ catalog: [] }),
...
beforeEach(() => {
  ...
  f.invoke.mockImplementation(async (method: string) =>
    method === "mini_widget_open"
      ? "instance"
      : method === "official_app_package_path"
        ? "/verified"
        : undefined,
  );
});
it("uses the installed release and closes the native session after work", async () => {
  ...
  expect(f.invoke).toHaveBeenCalledWith("mini_widget_open", {
    request: {
      root: "/verified",
      owner: { accountId: "member", deployment: "https://api.test/v1", authorityGeneration: 2 },
      scopeLimit: ["files.read"],
    },
  });
...
it("requires a personal installation and rejects Space-bound sessions", async () => {
  f.installations.mockResolvedValueOnce({ apps: [] });
  await expect(withNativeDocumentService("library", "space", vi.fn())).rejects.toThrow(
    "Install Library",
  );
...
it("discards results when server authority changed during processing", async () => {
  const run = async () => {
    f.installations.mockResolvedValueOnce({ apps: [{ ...installed, authority_generation: 3 }] });
    return "private text";
  };
  await expect(withNativeDocumentService("library", "space", run)).rejects.toThrow(
    "access changed",
  );
  expect(f.invoke).toHaveBeenCalledWith("mini_app_close", { instance: "instance" });
});
it("stops in-flight native work on removal and caller cancellation", async () => {
  await expect(
    withNativeDocumentService("library", "space", async () => {
      f.listener?.({ ready: true, installations: [] });
      return new Promise(() => {});
    }),
...
it("uses Files for background document work without an Agents installation", async () => {
  ...
  f.installations.mockResolvedValue({
    apps: [{ ...installed, app_id: "files", release_metadata: files }],
  });
  f.session.mockResolvedValue({
    app_id: "files",
    scopes: ["files.read"],
...
it("shares concurrent authority reads without caching the next operation", async () => {
  ...
  f.installations.mockImplementationOnce(
    () =>
      new Promise((done) => {
        resolve = done;
      }),
  ...
  const operations = Array.from({ length: 10 }, () =>
    withNativeDocumentService("library", "space", async () => "done"),
  );
