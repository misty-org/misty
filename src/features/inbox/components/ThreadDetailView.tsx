...
export function ThreadDetailView(props: {
  ...
  onSummarizeThread?: (thread: InboxThread) => void | Promise<void>;
  onBack: () => void;
}) {
  ...
  useEffect(() => {
    ...
    return () => {
      summaryGeneration.current = generation + 1;
    };
  }, [props.thread?.key]);
  ...
  const handleGenerateSummary = async () => {
    ...
    setSummaryError(null);
    try {
      await props.onSummarizeThread(thread);
    } catch (error) {
      if (summaryGeneration.current === generation) {
        setSummaryError("The summary could not be started. Try again.");
    ...
    <section className="flex h-full min-h-0 flex-col bg-charcoal-bg">
        ...
        <ToolbarGroup align="end">
          {props.onSummarizeThread ? (
            <Button
              ...
              disabled={summarizing || props.loading}
            >
              {summarizing ? <Spinner className="size-3.5" /> : <BookOpen className="size-3.5" />}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        >
          ...
          {summaryError ? (
            <p role="alert" className="mb-4 text-xs text-cream-muted">
              {summaryError}
            </p>
          ) : null}
