...
export function ComposeDialogView(props: {
  ...
  onSend: (draftId: string, connectionId: string, authoringSource?: "user" | "ai") => Promise<void>;
}) {
  ...
  const send = async () => {
    ...
    setValidationError("");
    try {
      await props.onSend(draft.provider_id, connectionId, props.authoringSource ?? "user");
