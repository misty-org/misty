...
import type { InboxCacheSnapshot } from "./inboxCache";

import { createInboxPrefetch, detailFreshMs, prefetchAttempts } from "./inboxPrefetch";
import {
  errorText,
  folderIdForAccount,
...
export interface InboxStoreRuntime {
  api: typeof mailApi;
  readCache(accountId: string): Promise<InboxCacheSnapshot | null>;
  persist(state: InboxStore): void;
  prefetchHtml(thread: InboxThread): void;
  errorMessage(code: string, fallback: string): string;
  signal?: AbortSignal;
}
const inboxStoreControls = new WeakMap<InboxStoreHook, { reset: () => void; close: () => void }>();

export function createInboxStoreWithRuntime(runtime: InboxStoreRuntime): InboxStoreHook {
  const mailApi = runtime.api;
  const { fetchThreadDetail, applyDetailedThread, clear } = createInboxPrefetch(
    mailApi,
    runtime.prefetchHtml,
  );
  const readInboxCache = runtime.readCache;
  const persistCurrentInbox = runtime.persist;
  const apiErrorMessage = runtime.errorMessage;
  ...
  const store = create<InboxStore>((set, get) => ({
    ...
    selectProvider: async (provider) => {
      ...
      await reloadThreads(runtime, set, get, {
    ...
    saveDraft: async (draft, draftId) => {
      ...
      if (generation !== accountGeneration) throw new Error("The Inbox account changed.");
      return result.draft;
    },
    sendDraft: async (draftId, connectionId, authoringSource = "user") => {
      const generation = accountGeneration;
      await mailApi.sendDraft(draftId, connectionId, authoringSource);
      if (generation !== accountGeneration) throw new Error("The Inbox account changed.");
      await reloadThreads(runtime, set, get, {
        accountGeneration: () => accountGeneration,
        contentGeneration: () => contentGeneration,
  ...
  const close = () => {
    reset();
    runtime.signal?.removeEventListener("abort", close);
  };
  inboxStoreControls.set(store, { reset, close });
  runtime.signal?.addEventListener("abort", close, { once: true });
  if (runtime.signal?.aborted) close();
  return store;
}

export function resetInboxStore(store: InboxStoreHook): void {
  inboxStoreControls.get(store)?.reset();
}
export function disposeInboxStore(store: InboxStoreHook): void {
  inboxStoreControls.get(store)?.close();
}

async function loadAccountContent(
  runtime: InboxStoreRuntime,
  accounts: MailAccount[],
  generation: number,
  set: InboxSet,
  isCurrent: () => boolean,
  ...
  await Promise.all(
    accounts.map(async (account) => {
      const [folders, threads] = await Promise.allSettled([
        runtime.api.folders(account.connection_id),
        runtime.api.threads({ connectionId: account.connection_id }),
      ]);
      if (!isCurrent()) return;
...
async function reloadThreads(
  ...
  await Promise.all(
    targets.map(async (account) => {
      ...
      try {
        const result = await runtime.api.threads({
          connectionId: account.connection_id,
