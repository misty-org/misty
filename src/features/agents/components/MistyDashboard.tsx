...
export function MistyDashboard({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (identity.current !== `${user?.id}:${spaceId}:${agentId}`) return;
      setEntries(result.entries);
      setError("");
    } catch (reason) {
      if (identity.current === `${user?.id}:${spaceId}:${agentId}`)
        setError(reason instanceof Error ? reason.message : String(reason));
    } finally {
      if (identity.current === `${user?.id}:${spaceId}:${agentId}`) setLoading(false);
    }
          ...
          <section key={entry.id} className="border-b border-charcoal-border py-4">
              ...
              <div className="min-w-0 flex-1">
                {entry.parent_run_id && (
                  <p className="text-xs text-cream-muted">
                    Delegated by {activityParent(entry, entries)?.title || "Misty"} · depth{" "}
                    {entry.delegation_depth}
                  </p>
                )}
                <h2 className="text-sm font-medium">{entry.title}</h2>
                <p className="mt-1 text-xs text-cream-muted">
                  {entry.state.replace(/_/g, " ")} · {new Date(entry.updated_at).toLocaleString()}
                </p>
              </div>
            ...
            {entry.result && entry.result !== "{}" && (
              <details className="mt-2 text-xs">
                ...
                <pre className="max-h-64 overflow-auto whitespace-pre-wrap">{entry.result}</pre>
              </details>
            )}
                ...
                <div key={approval.id} className="mt-3">
                  ...
                  {(["approve", "deny"] as const).map((decision) => (
                    <Button
                      ...
                      onClick={() =>
                        void action(() =>
                          agents.decideApproval(detail.summary.run_id, approval.id, decision),
                        )
                      }
                    >
