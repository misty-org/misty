...
export async function agentsPrepareScopedDocument(request: {
  ...
  if (!supportsBundledDocumentWorkers()) return invoke("agents_prepare_scoped_document", {request:{scopeId:request.scopeId,relativePath:request.relativePath}});
  const { withBuiltinService } = await import("@/features/builtin-services");
  return withBuiltinService("files", request.spaceId, instance =>
