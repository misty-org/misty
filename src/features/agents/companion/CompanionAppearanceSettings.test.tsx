...
it("offers immediate size and visibility controls in agent settings", () => {
  ...
  useCompanionState.setState({
    presentation: { ...initialCompanionPresentation, size: 150, showCompanion: false },
    control,
  });
  ...
  expect(control).toHaveBeenCalledWith({ kind: "size", size: 175 });
  fireEvent.click(screen.getByRole("button", { name: "Reset size" }));
  expect(control).toHaveBeenCalledWith({ kind: "size", size: 100 });
  fireEvent.click(screen.getByRole("switch", { name: "Show cursor companion" }));
  expect(control).toHaveBeenCalledWith({ kind: "visibility", visible: true });
});
