...
beforeEach(() => {
  ...
  state.invoke
    .mockReset()
    .mockResolvedValue(JSON.stringify({ publicKey: "original", privateKey: "private" }));
  state.generation = 1;
...
it("stops conflict retries for the same identity without modifying credentials, and isolates recovery scopes", async () => {
  const conflict = new ManagedAiRequestError(
  ...
  expect(state.invoke).toHaveBeenCalledWith("agents_device_identity_load", {
    localDeviceId: "identity-conflict-device",
  });

  state.register.mockResolvedValue({ id: "registered", name: "Mac" });
  await expect(
    ensureServerAgentDevice(local, { ...connected, endpointId: "endpoint-two" }),
  ).resolves.toMatchObject({ id: "registered" });
  state.generation++;
  await expect(ensureServerAgentDevice(local, connected)).resolves.toMatchObject({
    id: "registered",
  });
...
it("does not permanently cache transient registration errors", async () => {
  ...
  state.register
    ...
    .mockResolvedValueOnce({ id: "recovered", name: "Mac" });
