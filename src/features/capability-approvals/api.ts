...
// First-party Agent controls; only current browser actions are reviewable.
export const capabilityApprovalsApi = {
  async list(cursor = "", signal?: AbortSignal) {
    return pageSchema.parse(
      await apiRequest<unknown>(
        `/me/agent-approvals?limit=20${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`,
        { signal },
      ),
    );
  },
  async review(id: string, signal?: AbortSignal) {
    ...
    const value = reviewSchema.parse(
      await apiRequest<unknown>(`/me/agent-approvals/${encodeURIComponent(id)}`, { signal }),
    );
    const detail = value.review;
    const matches = detail.runId === value.approval.run_id;
  ...
  async decide(review: CapabilityApprovalReview, approved: boolean) {
    ...
    if (run_id.startsWith("invocation_")) {
      return apiRequest(
        `/me/agent-invocations/${encodeURIComponent(run_id.slice(11))}/approvals/${encodeURIComponent(id)}`,
