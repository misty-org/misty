...
export interface PickerBrowserState {
...
const runtime = {
  ...
  prewarmThumbnails: noAction,
  requestThumbnail: () => noAction,
  Error: ({ error }: { error: string }) => (
    <div role="alert" className="p-4 text-cream-muted">
      {error}
    </div>
  ),
};
...
export function PickerFileBrowser({
  state,
  onChange,
  initialPath,
  multiple,
  ...
  const pinnedPaths = useMemo(() => {
    try {
      const saved: unknown = JSON.parse(localStorage.getItem("misty.explorer.pinnedPaths") ?? "[]");
  ...
  const home = resolvePreferredRoot(
    selectGeneralPreferences(preferences).preferredWorkspaceRoot,
    app?.environment.homeDir ?? "/",
  );
  const mount = resolveMountRoot(
    home,
    selectAdvancedPreferences(preferences).mountPath || app?.environment.mountPath || ".misty/mnt",
  );
  ...
  const current = useRef({ state, onChange, history });
  current.current = { state, onChange, history };
  const load = useCallback(
    async (path: string | null, historyIndex?: number) => {
      ...
      try {
        ...
        setHistory((previous) =>
          historyIndex !== undefined
            ...
            : previous.paths[previous.index] === listing.path
              ? previous
              : {
                  paths: [...previous.paths.slice(0, previous.index + 1), listing.path],
                  index: previous.index + 1,
                },
        );
      } catch (error) {
        if (ticket === sequence.current) setError(String(error));
      } finally {
        if (ticket === sequence.current) {
          setLoading(false);
          current.current.onChange({ ...current.current.state, loading: false });
        }
      }
    },
    [hidden],
  ...
  const listing = useMemo(() => {
    ...
    return {
      ...state.listing,
      entries: [...state.listing.entries].sort((a, b) => {
        if (a.kind !== b.kind && (a.kind === "folder" || b.kind === "folder"))
  ...
  return (
    <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)]">
      <ExplorerPickerToolbar
        ...
        canGoForward={history.index + 1 < history.paths.length}
        canGoParent={Boolean(state.listing?.parentPath)}
        onBack={() => void load(history.paths[history.index - 1], history.index - 1)}
        onForward={() => void load(history.paths[history.index + 1], history.index + 1)}
        onParent={() => {
          if (state.listing?.parentPath) void load(state.listing.parentPath);
        }}
        onNavigate={(path) => void load(path)}
        onRefresh={() => void load(path, history.index)}
        onQueryChange={setQuery}
      />
      <div className="grid min-h-0 grid-cols-[180px_minmax(0,1fr)] max-[560px]:grid-cols-1">
        <aside className="min-h-0 overflow-auto border-r border-charcoal-border max-[560px]:hidden">
          <PickerPlaces
            homePath={home}
            activePath={path}
            mountRoot={mount}
        ...
        <FileBrowserRuntimeProvider value={runtime}>
          <FileBrowserView
            ...
            onSelect={(id, event, visible) => {
              ...
              if (
                ...
                const from = visible.indexOf(anchor.current),
                  to = visible.indexOf(id);
                selectedIds = visible.slice(Math.min(from, to), Math.max(from, to) + 1);
            ...
            onOpen={(entry) => {
              ...
              else onChange({ ...state, selectedIds: [entry.id] });
            }}
            onContextMenu={(event) => event.preventDefault()}
            onBackgroundContextMenu={(event) => event.preventDefault()}
            onDropItems={noAction}
            onInlineEditChange={noAction}
            onInlineEditCommit={noAction}
            onInlineEditCancel={noAction}
          />
