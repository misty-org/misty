...
export function librarySearchResults(
  ...
  if (parsed.terms.length === 0 && parsed.tags.length === 0 && parsed.comments.length === 0) return [];
...
function scoreLibraryItem(
  ...
  if (options.scope === "current" && options.currentPath && !normalizePath(item.path).startsWith(normalizePath(options.currentPath))) return null;
  ...
  if (!query.tags.every((tag) => (item.tags ?? []).some((candidate) => candidate.toLowerCase().includes(tag)))) return null;
  if (!query.comments.every((comment) => (item.comments ?? "").toLowerCase().includes(comment))) return null;
...
function fileEntryFromLibraryItem(item: ExplorerLibraryItem, sourceKind: SearchSourceKind): FileEntry {
