...
const GRID_ITEM_HEIGHT = 156;
const GRID_GAP = 18;
const GRID_PADDING = 3;
const GRID_SCROLLBAR_RESERVE = 14;
...
function GridThumbnail(props: { entry: FileEntry; enabled: boolean; iconSize: number }) {
  ...
  return (
    <span className={fileBrowserStyles.gridThumbIcon}>
      <FileIcon entry={props.entry} size={props.iconSize} />
    </span>
  );
}

function useGridThumbnailUrl(
  entry: FileEntry,
  enabled: boolean,
): {
  ...
  useEffect(() => {
    ...
    const cancelThumbnail = requestGridThumbnail(
      entry,
...
function normalizeItemScale(scale: number | undefined): number {
...
function passiveRenameDraftsFor(
...
function loadColumnWidths(): FileTableColumnWidths {
  try {
    ...
    return {
      name: validColumnWidth(
        parsed.name,
        defaultColumnWidths.name,
        minimumColumnWidths.name,
        maximumColumnWidths.name,
      ),
      modified: validColumnWidth(
        parsed.modified,
        defaultColumnWidths.modified,
        minimumColumnWidths.modified,
        maximumColumnWidths.modified,
      ),
      size: validColumnWidth(
        parsed.size,
        defaultColumnWidths.size,
        minimumColumnWidths.size,
        maximumColumnWidths.size,
      ),
      type: validColumnWidth(
        parsed.type,
        defaultColumnWidths.type,
        minimumColumnWidths.type,
        maximumColumnWidths.type,
      ),
    };
  } catch {
