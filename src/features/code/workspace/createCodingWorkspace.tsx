    ...
    const toggleTerminal = useCallback(
      async (direction: DockSplitDirection | "current" = "down") => {
        ...
        if (services.toggleTerminal) {
          try {
            await services.toggleTerminal(direction);
          } catch (error) {
    ...
    return (
      ...
      >
        {actionFailed ? (
          <div
            role="alert"
            className="absolute bottom-10 left-3 right-3 z-20 flex items-center gap-2 rounded-md border border-charcoal-border bg-charcoal-card p-3 text-sm text-cream"
          >
            <span>
              The action could not finish. Check your connection and try the command again.
            </span>
            <button
              type="button"
