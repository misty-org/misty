...
export function createOpenFolderCard(
  ...
  ProjectMenu?: React.ComponentType<{ onSelect(path: string): void }>,
) {
  return function OpenFolderCard({ onOpenRoot }: { onOpenRoot: (path: string) => void }) {
    ...
    return (
      <section className="code-theme-editor grid h-full place-items-center bg-charcoal-workspace p-8 text-cream">
        <div className="w-full max-w-lg rounded-2xl border border-charcoal-border bg-charcoal-card p-7 text-center shadow-xl">
          ...
          {ProjectMenu ? (
            <div className="mt-6 border-t border-charcoal-border pt-4">
              <ProjectMenu onSelect={onOpenRoot} />
            </div>
          ) : null}
        </div>
