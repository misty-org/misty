...
export function ExplorerToolbarSearchView(
  props: ExplorerToolbarSearchProps & { runtime: ExplorerToolbarSearchRuntime },
) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void props.runtime
        .query(query, props.path, lifetime.signal)
        .then((results) => {
  ...
  const runIndexedResult = (result: SearchResult) => {
    props.onNavigateSearchResult({
      result,
      path:
        result.entry.kind === "folder"
          ? result.entry.path
          : result.entry.path.slice(0, result.entry.path.lastIndexOf("/")),
      selectEntryId: result.entry.kind === "folder" ? null : result.entry.id,
    });
    props.onCommandQuery("");
    setSearchFocused(false);
  };
