...
import { deviceSignaturePayload, signedAgentDeviceRequest } from "./deviceApi";
...
describe("device request signing", () => {
  ...
  it("does not reopen a denied device-identity Keychain request during the same session", async () => {
    ...
    await expect(signedAgentDeviceRequest(deviceId, "/devices/test/heartbeat", { method: "POST" }))
