...
std::string plugin_sdk_version() {
    return MISTY_PLUGIN_SDK_VERSION;
}
...
PluginVerificationResult verify_plugin_manifest(nlohmann::json& manifest,
    ...
    if (!selected.plugin_api_version.empty() && selected.plugin_api_version != plugin_sdk_version()) {
...
bool sign_plugin_manifest(const fs::path& plugin_dir,
    ...
    if (!manifest.contains("plugin") || !manifest["plugin"].is_object()) {
        if (error) {
            *error = "Manifest is missing the plugin object.";
        }
        return false;
    }
    ...
    if (!plugin_json.contains("variants") || !plugin_json["variants"].is_array() ||
        ...
        if (error) {
            *error = "Manifest.plugin.variants must be a non-empty array.";
        }
        return false;
    }
    ...
    for (auto& variant : plugin_json["variants"]) {
        if (!variant.is_object()) {
            if (error) {
                *error = "Each entry in plugin.variants must be an object.";
            }
            return false;
        }
        ...
        if (library_rel.empty()) {
            if (error) {
                *error = "A variant is missing its library path.";
            }
            return false;
        }
        ...
        if (!fs::exists(library_path)) {
            if (error) {
                *error = "Variant library was not found: " + library_rel;
            }
            return false;
        }
        ...
        if (sha.empty()) {
            if (error) {
                *error = "Failed to compute SHA-256 for variant: " + library_rel;
            }
            return false;
        }
        variant["sha256"] = sha;
