...
import { MockTelemetryClient } from "./client";
import { ANALYTICS_SESSION_TIMEOUT_MS, AnalyticsLifecycleManager } from "./lifecycle";
import type { CommonClientProperties } from "./types";

const metadata: CommonClientProperties = {
  platform: "macos", app_version: "1.0.0", release_channel: "private_beta",
  distribution_channel: "direct", device_class: "desktop", environment: "production",
};
...
function harness() {
  ...
  let time = 1_000;
  let id = 0;
  const manager = new AnalyticsLifecycleManager(client, storage, async () => metadata, () => time, () => `uuid-${++id}`);
  return { client, manager, values, setTime: (value: number) => { time = value; } };
}
...
describe("analytics lifecycle", () => {
  ...
  it("deduplicates focus within 30 minutes and starts a session at the timeout", async () => {
    ...
    manager.initialize(); await Promise.resolve(); await Promise.resolve();
    setTime(2_000); manager.handleVisibility(false);
    await Promise.resolve();
    expect(client.events.filter((item) => item.event === "app_session_started")).toHaveLength(1);
    setTime(2_000 + ANALYTICS_SESSION_TIMEOUT_MS); manager.handleVisibility(false);
    await Promise.resolve(); await Promise.resolve();
  ...
  it("marks onboarding only once", async () => {
    const { client, manager } = harness();
    await manager.trackOnboardingCompleted(); await manager.trackOnboardingCompleted();
