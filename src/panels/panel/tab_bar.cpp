...
namespace misty::panel {
    namespace {
        constexpr float kTabButtonHeight = 26.0f;
        constexpr float kTabOuterPadX = 10.0f;
        constexpr float kTabTopPadY = 5.0f;
        constexpr float kTabGap = 6.0f;
        constexpr float kTabCorner = 5.0f;
        constexpr float kTabMinWidth = 140.0f;
        constexpr float kTabMaxWidth = 188.0f;
        constexpr float kPlusWidth = 32.0f;
        constexpr float kCloseIconSize = 12.0f;
        constexpr ImU32 kTabStripBg = IM_COL32(23, 23, 25, 255);
        constexpr ImU32 kTabStripBorder = IM_COL32(74, 69, 63, 155);
        constexpr ImU32 kTabIdle = IM_COL32(33, 33, 37, 255);
        constexpr ImU32 kTabHovered = IM_COL32(44, 42, 40, 255);
        constexpr ImU32 kTabActive = IM_COL32(72, 62, 53, 255);
        constexpr ImU32 kTabText = IM_COL32(239, 230, 216, 255);
        constexpr ImU32 kTabClose = IM_COL32(232, 224, 213, 230);
        ...
        bool draw_tab_button(const TabBarItem& item, float width, bool* close_requested) {
            ...
            dl->AddRectFilled(rect.Min, rect.Max, fill, kTabCorner);
            dl->AddRect(rect.Min, rect.Max, kTabStripBorder, kTabCorner, 0, 1.0f);
            ...
            dl->AddText(ImVec2(rect.Min.x + text_pad_x, text_y), kTabText, visible_title.c_str());

            if (item.closable) {
                const ImVec2 close_min(rect.Max.x - 22.0f, rect.Min.y + (size.y - kCloseIconSize) * 0.5f);
                const ImVec2 close_max(close_min.x + kCloseIconSize + 6.0f, close_min.y + kCloseIconSize);
                if (hovered && ImGui::IsMouseHoveringRect(close_min, close_max) && ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
                    if (close_requested) {
        ...
        bool draw_plus_button() {
            ...
            ImDrawList* dl = ImGui::GetWindowDrawList();
            dl->AddRectFilled(rect.Min, rect.Max, hovered ? kTabHovered : kTabIdle, kTabCorner);
            dl->AddRect(rect.Min, rect.Max, kTabStripBorder, kTabCorner, 0, 1.0f);
    ...
    TabBarResult render_tab_bar(const std::vector<TabBarItem>& items, bool show_plus) {
        ...
        dl->AddLine(ImVec2(strip_min.x, strip_max.y - 1.0f), ImVec2(strip_max.x, strip_max.y - 1.0f), kTabStripBorder, 1.0f);
