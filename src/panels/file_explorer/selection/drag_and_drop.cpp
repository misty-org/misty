...
std::vector<FileItem> selected_drag_items(const FileExplorerPanel::TransientUiState& ui, const FileListing& listing) {
    std::vector<FileItem> items;
    items.reserve(ui.selected_files.size());
    for (const auto& selected_id : ui.selected_files) {
        auto it = std::find_if(listing.files.begin(), listing.files.end(),
            [&](const FileItem& candidate) { return candidate.id == selected_id; });
        if (it != listing.files.end()) {
            items.push_back(*it);
        }
    }
    return items;
...
bool same_drag_items(const std::vector<FileItem>& lhs, const std::vector<FileItem>& rhs) {
...
bool active_drag_contains_path(const std::string& path) {
    return std::any_of(g_active_file_drag.items.begin(), g_active_file_drag.items.end(),
        [&](const FileItem& item) { return item.path == path; });
}
...
void draw_file_drag_preview(const FileExplorerState& state,
    ...
    const bool includes_dir = std::any_of(g_active_file_drag.items.begin(), g_active_file_drag.items.end(),
        [](const FileItem& item) { return item.is_dir; });
...
void FileExplorerPanel::begin_file_drag_source(FileExplorerState& state,
    ...
    if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID |
        ...
        std::vector<FileItem> drag_items = selected_drag_items(ui, listing);
        const bool new_drag =
            g_active_file_drag.drop_consumed ||
...
void FileExplorerPanel::handle_file_drop_target(FileExplorerState& state,
    ...
    std::vector<FileItem> items = g_active_file_drag.items;
    if (!items.empty()) {
