...
std::string icon_name_for_file(const FileListing& listing, const FileItem& file, bool open_directory) {
    if (listing.is_deleting(file.path)) return "trash-16";
...
std::string icon_name_for_file(const FileListing& listing, const FileItem& file) {
    return icon_name_for_file(listing, file, false);
}
...
void grid_item_icon(ImDrawList* draw_list,
    ...
    auto& icon = core::AssetManager::get().get_svg_texture(
        icon_name_for_file(listing, file, show_open_folder_icon),
    ...
    draw_list->AddImage(icon.id,
                        ...
                        ImVec2(0, 0),
                        ImVec2(1, 1),
                        grid_item_icon_color(listing, file));
    (void)state;
}
...
void grid_item_label(ImDrawList* draw_list,
    ...
    draw_list->AddText(ImGui::GetFont(),
                       ...
                       grid_item_text_color(listing, file, is_selected),
                       file.name.c_str(),
                       nullptr,
                       text_wrap_width);
    (void)state;
...
void sort_files(FileListing& listing, const ImGuiTableSortSpecs& sort_specs) {
    if (listing.files.size() < 2 || sort_specs.SpecsCount <= 0 || sort_specs.Specs == nullptr) {
        return;
    }

    auto compare_for_column = [&listing](const FileItem& lhs,
                                         const FileItem& rhs,
    ...
    std::stable_sort(listing.files.begin(), listing.files.end(),
        [&sort_specs, &compare_for_column](const FileItem& lhs, const FileItem& rhs) {
