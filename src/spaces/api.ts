...
function spaceErrorMessage(code: string | undefined, fallback: string): string {
  const messages: Record<string, string> = {
    ...
    space_ownership_limit_reached: "You already own three Spaces. Delete one permanently before creating another.",
    space_storage_quota_exceeded: "This upload would exceed this Space’s shared 1 GB storage limit.",
...
export const spacesApi = {
  ...
  libraryPins: (spaceId: string) => spaceRequest<{ pins: LibraryPinnedCollection[] }>(`/spaces/${encodeURIComponent(spaceId)}/library/pins`),
  setLibraryPins: (spaceId: string, targets: Array<{ kind: LibraryPinnedCollection["target_kind"]; id: string }>) => spaceRequest<{ pins: LibraryPinnedCollection[] }>(`/spaces/${encodeURIComponent(spaceId)}/library/pins`, { method: "PUT", body: JSON.stringify({ targets }) }),
  libraryImportHistory: (spaceId: string) => spaceRequest<{ imports: LibraryImportHistoryItem[] }>(`/spaces/${encodeURIComponent(spaceId)}/library/imports/history`),
  ...
  createAlbumFolder: (spaceId: string, name: string, parentFolderId = "") => spaceRequest<LibraryAlbumFolder>(`/spaces/${encodeURIComponent(spaceId)}/library/album-folders`, { method: "POST", body: JSON.stringify({ name, parent_folder_id: parentFolderId }) }),
  updateAlbumFolder: (spaceId: string, folder: LibraryAlbumFolder, patch: Partial<Pick<LibraryAlbumFolder, "name" | "parent_folder_id" | "position">>) => spaceRequest<LibraryAlbumFolder>(`/spaces/${encodeURIComponent(spaceId)}/library/album-folders/${encodeURIComponent(folder.id)}`, { method: "PATCH", body: JSON.stringify({ version: folder.version, name: patch.name ?? folder.name, parent_folder_id: patch.parent_folder_id ?? folder.parent_folder_id ?? "", position: patch.position ?? folder.position }) }),
  deleteAlbumFolder: (spaceId: string, folder: LibraryAlbumFolder) => spaceRequest(`/spaces/${encodeURIComponent(spaceId)}/library/album-folders/${encodeURIComponent(folder.id)}?version=${folder.version}`, { method: "DELETE" }),
  createAlbum: (spaceId: string, name: string, description = "") => spaceRequest<LibraryAlbum>(`/spaces/${encodeURIComponent(spaceId)}/library/albums`, { method: "POST", body: JSON.stringify({ name, description }) }),
  organizeAlbum: (spaceId: string, album: LibraryAlbum, patch: Partial<Pick<LibraryAlbum, "folder_id" | "view_mode" | "sort_mode" | "position">>) => spaceRequest<LibraryAlbum>(`/spaces/${encodeURIComponent(spaceId)}/library/albums/${encodeURIComponent(album.id)}/organization`, { method: "PUT", body: JSON.stringify({ version: album.version, folder_id: patch.folder_id ?? album.folder_id ?? "", view_mode: patch.view_mode ?? album.view_mode, sort_mode: patch.sort_mode ?? album.sort_mode, position: patch.position ?? album.position }) }),
  updateAlbum: (spaceId: string, album: LibraryAlbum, patch: Partial<Pick<LibraryAlbum, "name" | "description" | "cover_item_id">>) => spaceRequest<LibraryAlbum>(`/spaces/${encodeURIComponent(spaceId)}/library/albums/${encodeURIComponent(album.id)}`, { method: "PATCH", body: JSON.stringify({ version: album.version, name: patch.name ?? album.name, description: patch.description ?? album.description, cover_item_id: patch.cover_item_id ?? album.cover_item_id ?? "" }) }),
  deleteAlbum: (spaceId: string, album: LibraryAlbum) => spaceRequest(`/spaces/${encodeURIComponent(spaceId)}/library/albums/${encodeURIComponent(album.id)}?version=${album.version}`, { method: "DELETE" }),
  albumItems: (spaceId: string, albumId: string) => spaceRequest<{ items: SpaceLibraryItem[] }>(`/spaces/${encodeURIComponent(spaceId)}/library/albums/${encodeURIComponent(albumId)}/items`),
  addAlbumItems: (spaceId: string, albumId: string, itemIds: string[]) => spaceRequest(`/spaces/${encodeURIComponent(spaceId)}/library/albums/${encodeURIComponent(albumId)}/items`, { method: "POST", body: JSON.stringify({ item_ids: itemIds }) }),
  removeAlbumItem: (spaceId: string, albumId: string, itemId: string) => spaceRequest(`/spaces/${encodeURIComponent(spaceId)}/library/albums/${encodeURIComponent(albumId)}/items/${encodeURIComponent(itemId)}`, { method: "DELETE" }),
  reorderAlbumItems: (spaceId: string, album: LibraryAlbum, itemIds: string[]) => spaceRequest<LibraryAlbum>(`/spaces/${encodeURIComponent(spaceId)}/library/albums/${encodeURIComponent(album.id)}/order`, { method: "POST", body: JSON.stringify({ version: album.version, item_ids: itemIds }) }),
  groups: (spaceId: string) => spaceRequest<{ groups: LibraryGroup[] }>(`/spaces/${encodeURIComponent(spaceId)}/library/groups`),
  ...
  deletePerson: (spaceId: string, person: LibraryPerson) => spaceRequest(`/spaces/${encodeURIComponent(spaceId)}/library/people/${encodeURIComponent(person.id)}?version=${person.version}`, { method: "DELETE" }),
  personItems: (spaceId: string, personId: string) => spaceRequest<{ items: SpaceLibraryItem[] }>(`/spaces/${encodeURIComponent(spaceId)}/library/people/${encodeURIComponent(personId)}/items`),
...
async function fetchProtectedBlob(path: string, init?: RequestInit): Promise<Blob> {
  ...
  if (!response.ok) {
    const text = await response.text();
    let code: string | undefined;
    try { code = (JSON.parse(text) as { code?: string }).code; } catch { /* plain text */ }
    throw new SpaceRequestError(spaceErrorMessage(code, text), response.status, code);
  }
