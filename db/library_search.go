...
func (db *Database) LibraryFacets(ctx context.Context, userID, spaceID, prefix string) (*LibrarySearchFacets, error) {
	...
	err := db.spaceTx(ctx, func(tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, `SELECT count(*) FILTER(WHERE lifecycle_state='ready' AND hidden=FALSE),count(*) FILTER(WHERE lifecycle_state='ready' AND hidden=FALSE AND favorite=TRUE),count(*) FILTER(WHERE lifecycle_state='ready' AND hidden=TRUE),count(*) FILTER(WHERE lifecycle_state='trash') FROM space_library_items WHERE space_id=$1`, spaceID).Scan(&out.Total, &out.Favorites, &out.Hidden, &out.RecentlyDeleted); err != nil {
			return err
		}
		like := "%" + prefix + "%"
		queries := []struct {
			...
			{&out.Tags, `SELECT tag,tag,count(*) FROM space_library_items i CROSS JOIN LATERAL jsonb_array_elements_text(i.tags) tag WHERE i.space_id=$1 AND i.lifecycle_state='ready' AND i.hidden=FALSE AND ($2='' OR tag ILIKE $3) GROUP BY tag ORDER BY count(*) DESC,lower(tag) LIMIT 12`, []any{spaceID, prefix, like}},
			{&out.MediaTypes, `SELECT kind,initcap(kind),count(*) FROM (SELECT CASE WHEN b.server_detected_mime_type LIKE 'image/%' THEN 'image' WHEN b.server_detected_mime_type LIKE 'video/%' THEN 'video' WHEN b.server_detected_mime_type LIKE 'audio/%' THEN 'audio' ELSE 'document' END kind FROM space_library_items i JOIN library_files f ON f.id=i.file_id JOIN library_blobs b ON b.id=f.blob_id WHERE i.space_id=$1 AND i.lifecycle_state='ready' AND i.hidden=FALSE) media WHERE ($2='' OR kind ILIKE $3) GROUP BY kind ORDER BY count(*) DESC,kind`, []any{spaceID, prefix, like}},
			{&out.Years, `SELECT captured_year,captured_year,count(*) FROM (SELECT extract(year FROM COALESCE(i.date_override,f.intrinsic_capture_at,f.original_uploaded_at))::int::text captured_year FROM space_library_items i JOIN library_files f ON f.id=i.file_id WHERE i.space_id=$1 AND i.lifecycle_state='ready' AND i.hidden=FALSE) dates WHERE ($2='' OR captured_year ILIKE $3) GROUP BY captured_year ORDER BY captured_year DESC LIMIT 12`, []any{spaceID, prefix, like}},
			...
			{&out.Utilities, `SELECT utility,label,item_count FROM (
				...
				UNION ALL SELECT 'screenshots','Screenshots',count(*),5 FROM space_library_items i JOIN library_files f ON f.id=i.file_id WHERE i.space_id=$1 AND i.lifecycle_state='ready' AND i.hidden=FALSE AND (lower(f.original_filename) LIKE '%screenshot%' OR lower(f.intrinsic_metadata::text) LIKE '%screenshot%')
		...
		mediaSubtypes := []struct{ value, label string }{
			{"selfies", "Selfies"}, {"live-photos", "Live Photos"}, {"portraits", "Portraits"}, {"panoramas", "Panoramas"}, {"slo-mo", "Slo-mo"}, {"cinematic", "Cinematic"}, {"bursts", "Bursts"}, {"screenshots", "Screenshots"}, {"screen-recordings", "Screen Recordings"}, {"spatial", "Spatial"},
		}
		for _, subtype := range mediaSubtypes {
			var count int
			statement := `SELECT count(*) FROM space_library_items i JOIN library_files f ON f.id=i.file_id JOIN library_blobs b ON b.id=f.blob_id WHERE i.space_id=$1 AND i.lifecycle_state='ready' AND i.hidden=FALSE AND ` + libraryMediaSubtypeCondition(subtype.value)
