...
func TestSpaceTaskCursorRoundTrip(t *testing.T) {
	for _, offset := range []int{0, 1, 200, 1_000_000} {
		got, err := decodeTaskCursor(encodeTaskCursor(offset))
...
func TestSpaceTaskCursorRejectsInvalidOffsets(t *testing.T) {
	for _, cursor := range []string{"not-base64!", encodeTaskCursor(-1), encodeTaskCursor(1_000_001)} {
		if _, err := decodeTaskCursor(cursor); err == nil {
...
func TestValidateSpaceTaskRejectsUnknownPriority(t *testing.T) {
	...
	if err := validateSpaceTask(&item); !errors.Is(err, ErrSpaceInvalid) {
