...
fn development_packages_are_fresh(workspace: &Workspace, stamp: &Path) -> Result<bool> {
    ...
    let inputs = [
        ...
        workspace
            .misty
            .join("cli/tasks/build-official-app-packages.ts"),
        workspace.misty.join("package.json"),
        workspace.misty.join("package-lock.json"),
        workspace.extensions.clone(),
        workspace
            .misty
            .join("cli/tasks/apps/build-official-apps.ts"),
        workspace.misty.join("cli/tasks/apps/validate-catalog.ts"),
        workspace
            .misty
            .join("cli/tasks/apps/sync-server-official-apps.ts"),
        workspace.extensions.join("package.json"),
        workspace.misty.join("packages"),
    ];
    Ok(inputs
        .iter()
...
fn development_outputs_exist(workspace: &Workspace) -> Result<bool> {
    let catalog_path = workspace.extensions.join("catalog.json");
    ...
    for app in apps
        .iter()
        .filter(|app| app.pointer("/desktop/runtime").and_then(Value::as_str) == Some("downloaded"))
    {
