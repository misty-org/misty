 ...
 * Usage: MISTY_PLAYWRIGHT_MODULE=/path/to/playwright/index.ts node cli/tasks/sdk-journal-render-run.ts <package-output> [artifacts-directory]
 */
...
if (!process.argv[2])
  throw new Error("Pass the directory containing the built Journal app.js and app.css.");
const packageRoot = path.resolve(process.argv[2]);
const root = process.argv[3]
  ? path.resolve(process.argv[3])
  : await fs.mkdtemp(path.join(os.tmpdir(), "misty-journal-render-"));
await fs.mkdir(root, { recursive: true });
for (const [source, target] of [
  [path.join(packageRoot, "app.js"), "journal.js"],
  [path.join(packageRoot, "app.css"), "app.css"],
  [path.join(import.meta.dirname, "sdk-journal-render-probe.html"), "index.html"],
])
  await fs.copyFile(source, path.join(root, target));
const require = createRequire(import.meta.url);
const yjsRequire = createRequire(require.resolve("yjs"));
await build({
  entryPoints: [path.join(import.meta.dirname, "sdk-journal-render-probe.ts")],
  bundle: true,
  format: "esm",
  define: { "process.env.NODE_ENV": '"production"' },
  alias: { lib0: path.dirname(yjsRequire.resolve("lib0/package.json")) },
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (/^\/fonts\/[a-f0-9]{64}\.woff2$/.test(name)) {
      res.setHeader("Content-Type", "font/woff2");
      res.end(
        await fs.readFile(
          path.resolve(
            packageRoot,
            "../optional-assets/official-app-assets/journal",
            path.basename(name),
          ),
        ),
      );
    ...
    if (!["/", "/index.html", "/probe.js", "/journal.js", "/app.css"].includes(name))
...
try {
  ...
  await page.waitForFunction(() => journalProbe.snapshot().fonts >= 21);
  ...
  assert.equal(opened.surface, true);
  assert.equal(opened.elements, 2);

  assert.equal(
    await page.evaluate(
      () => journalProbe.calls.filter((c) => c.method === "network.fetch").length,
    ),
    0,
  );
  await page.evaluate(() => journalProbe.multilingual());
  try {
    await page.waitForFunction(
      () =>
        journalProbe.calls.some((c) => c.method === "network.fetch") && document.fonts.size > 21,
    );
  } catch (error) {
    console.log(
      await page.evaluate(() => ({
        snapshot: journalProbe.snapshot(),
        calls: journalProbe.calls.filter((c) => !c.method.startsWith("collaboration")),
        elements: journalProbe.drawingElements(),
        errors: journalProbe.errors,
      })),
    );
    throw error;
  }
  await page.evaluate(() => document.fonts.ready);
  await page.screenshot({ path: path.join(root, "multilingual.png") });
  await page.mouse.click(180, 160, { button: "right" });
  await page.getByRole("button", { name: /^Copy to clipboard as PNG/ }).click();
  await page.waitForFunction(() =>
    journalProbe.calls.some((call) => call.method === "clipboard.writeImage"),
  );
  await page.mouse.click(600, 300, { button: "right" });
  await page.getByRole("button", { name: "Copy to clipboard as SVG", exact: true }).click();
  await page.waitForFunction(() =>
    journalProbe.calls.some(
      (call) => call.method === "clipboard.writeText" && call.params?.text.includes("<svg"),
    ),
  );
  await page.mouse.click(600, 300, { button: "right" });
  await page.getByText("Paste", { exact: true }).click();
  await page.waitForFunction(() =>
    journalProbe.drawingElements().some((element) => element.text === "SDK clipboard text"),
  );
  ...
  await page.waitForFunction(() =>
    journalProbe.outputs.some((file) => file.name.endsWith(".excalidraw")),
  );
  ...
  await page.waitForFunction(
    () => journalProbe.calls.filter((call) => call.method === "clipboard.writeImage").length === 2,
  );
  ...
  const fontDownloads = await page.evaluate(
    () => journalProbe.calls.filter((c) => c.method === "network.fetch").length,
  );
  assert.ok(
    fontDownloads > 0 && fontDownloads < 20,
    `Only necessary CJK subsets should load: ${fontDownloads}`,
  );
  const svg = await page.evaluate(
    () =>
      journalProbe.calls.find(
        (c) => c.method === "clipboard.writeText" && c.params?.text.includes("<svg"),
      )?.params.text,
  );
  assert.ok(svg.includes("中文日本語") && svg.includes("data:font/woff2;base64,"));
  const exports = await page.evaluate(() => journalProbe.outputs);
  const methods = await page.evaluate(() => [
    ...new Set(journalProbe.calls.map((call) => call.method)),
  ]);
  ...
  assert.equal(closed.leases, 0);
  assert.equal(closed.subscriptions, 0);
  assert.equal(closed.surface, false);
