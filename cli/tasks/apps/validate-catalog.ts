...
const repo = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../apps");
const packageJson = await json("package.json");
const index = await json("catalog/index.json");
const interfaceManifest = await json("interface/manifest.json");
const officialCatalog = await json("catalog.json");
const ids = new Set();
...
for (const id of ids) {
  ...
  for (const [name, value] of [
    ...
    if (value.version !== catalog.version)
      fail(`${id} ${name} version must match its catalog (${catalog.version}).`);
  }
  if (!/^\d+\.\d+\.\d+(?:-beta\.[1-9]\d*)?$/.test(catalog.version ?? ""))
    fail(`${id} catalog must declare a valid independent version.`);
  ...
  for (const artifact of catalog.install.artifacts) {
    if (
      !artifact.url?.includes(
        `/releases/download/v${catalog.version}/${id}-${artifact.platform}.zip`,
      )
    )
      fail(
...
async function validateOfficialApps(catalog) {
  if (
    ...
    fail(
      "catalog.json must use official app schema version 1 and host protocol version 2.",
    );
  }
  const expected = [
    "chat",
