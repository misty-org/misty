...
} from "./official-app-development-paths.ts";
...
test("recovers old launcher paths and rejects other missing configured paths", () => {
  ...
  try {
    ...
    writeFileSync(join(root, "misty/apps/catalog.json"), "{}");
    for (const relative of ["public/official-apps", "catalog.json"]) {
...
test("embedded catalogs work without generated packages but still reject misspelled overrides", () => {
  ...
  try {
    ...
    const catalog = join(appsRoot, "catalog.json");
