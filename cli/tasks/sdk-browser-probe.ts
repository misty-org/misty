...
const tab=useWorkspaceStore.getState().openSurface({surfaceId:"official-app",groupKey:"app:browser",instanceKey:crypto.randomUUID(),title:"Browser",route:"/apps/browser",state:createBrowserTabState(location.origin+"/cli/tasks/sdk-browser-page.html")});
...
const ui=createAppUiRpc(scope,{settings:()=>({browser:{homeUrl:location.origin+"/cli/tasks/sdk-browser-page.html",searchEngineIndex:0}}),setTitle:value=>{title=value;},subscribeSettings:()=>()=>{},registerShortcut:()=>()=>{},reportError:message=>errors.push(message),openExternal:async()=>{throw new Error("Probe cannot open external links");}});
...
async function checkProfile(accountId:string, expected:string) {
 const profileTab=useWorkspaceStore.getState().openBrowserTab({url:location.origin+"/cli/tasks/sdk-browser-page.html"});
