...
const sdkRoot = resolve(root, './packages/sdk/src');
const catalogApps = JSON.parse(readFileSync(resolve(appsRoot, 'catalog.json'), 'utf8')).apps;
const appIds = catalogApps.map(app => app.id);
const entries = appIds.map(id => resolve(appsRoot, `${id}/index.tsx`));
...
const injectedSources = { journal: [resolve(appsRoot, 'journal/drawings/sdkDrawingInterop.ts')] };
...
const md = ['# App SDK dependency audit', '', result.analysis, '', 'Regenerate with `node cli/tasks/audit-app-sdk.ts`.', '', '| App | Catalog desktop runtime | Reachable source files | Native commands | Event call sites | Server/network call sites |', '| --- | --- | ---: | ---: | ---: | ---: |'];
