...
import { appSourceAliases, appSourceRoot } from "./app-source-paths.ts";
...
test("all migrated component aliases point at app-owned source and match TypeScript", () => {
  ...
  for (const id of ["journal", "planner", "library", "files", "browser", "code", "terminal"]) {
    const name = `@/features/apps/package/SDK${id[0].toUpperCase() + id.slice(1)}App`;
    const target = resolve(apps, id, "index");
