...
import { stagedAppBuild } from "./staged-app-build.ts";
import { resolve } from "node:path";

const root = resolve(import.meta.dirname, "../..");
const developmentEnv = loadAppEnv(root);
const appsRoot = resolve(root, "apps");
const catalogPath = developmentEnv.MISTY_OFFICIAL_APP_CATALOG_PATH || `${appsRoot}/catalog.json`;
...
const { nativeServicesForApp } = await import(pathToFileURL(resolve(root, "cli/tasks/apps/package-native-services.ts")).href);
...
if (process.platform === "darwin") {
  for (const service of nativeServices) {
    await new Promise((resolveRun, reject) => {
      const child = spawn(process.execPath, [resolve(root, "cli/tasks/apps/build-native-services.ts"), "", service], {cwd:appsRoot,env:developmentEnv,stdio:"inherit"});
