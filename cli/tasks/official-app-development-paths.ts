...
export function officialAppDevelopmentPath(
  ...
  const fallback = resolve(cwd, "./apps", relativePath);
  if (!configured?.trim()) return fallback;
  const selected = resolve(configured.trim());
  if (existsSync(selected)) return selected;
  const legacy = [resolve(cwd, "../misty-store", relativePath), resolve(cwd, "../misty-apps", relativePath)];
  ...
  if (legacy.includes(selected) && (existsSync(fallback) || optionalAssets)) {
