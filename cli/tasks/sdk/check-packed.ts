...
try {
  ...
  writeFileSync(resolve(directory, 'check.js'), `
  ...
  `);
  execFileSync(process.execPath, ['check.js'], { cwd: directory, stdio: 'pipe' });
  const sample = resolve(directory, 'habit-tracker');
  mkdirSync(sample);
  for (const file of readdirSync(resolve(root, 'examples/habit-tracker')).filter(file => file.endsWith('.ts'))) {
    copyFileSync(resolve(root, 'examples/habit-tracker', file), resolve(sample, file));
  }
  execFileSync(process.execPath, ['--test', 'adapter.node-test.ts'], { cwd: sample, stdio: 'pipe' });
