...
const sdkVersion = readJSON(resolve(root, 'packages/sdk/package.json')).version;
if (!/^\d+\.\d+\.\d+$/.test(sdkVersion)) throw new Error('Invalid pinned SDK version.');
const sdkTag = `sdk-v${sdkVersion}`;
...
if (command === 'prepare') {
  ...
  let sdkRelease;
  try { sdkRelease = JSON.parse(capture('gh',['release','view',sdkTag,'--repo','misty-org/misty','--json','isDraft,assets,targetCommitish'])); } catch {}
  if (!sdkRelease) run('gh',['release','create',sdkTag,'--repo','misty-org/misty','--target',manifest.source.host,'--draft','--title',`Misty SDK ${sdkVersion}`,'--notes','Typed contracts and SDK archives, verified in an isolated consumer. Install both archives together. npm publication is deferred.']);
  ...
  for (const name of [...sdkNames,'SDK-SHA256SUMS']) {
    ...
    if (asset) {
      ...
      run('gh',['release','download',sdkTag,'--repo','misty-org/misty','--pattern',name,'--dir',existing]);
  ...
  if (phase === 'assets') {
    run('gh',['release','edit',sdkTag,'--repo','misty-org/misty','--draft=false']);
