...
import { root, readJSON, writeJSON, run, capture, sha256, checksums, files } from "./lib.ts";
...
if (!id || !baselineFile || !destination)
  throw new Error(
    "Usage: node cli/tasks/release/prepare-app-update.ts <app-id> <baseline-catalog.json> <new-output-directory>",
  );
const apps = resolve(root, "apps");
if (capture("git", ["status", "--porcelain", "--untracked-files=no"], root))
  throw new Error("Prepare from clean committed source, using an isolated checkout if other work is in progress.");
...
const next = readJSON(resolve(apps, "catalog.json")).apps.find((app) => app.id === id);
...
run(process.execPath, [resolve(root, "cli/tasks/apps/build-official-apps.ts"), "--release", id], apps, env);
...
run(
  process.execPath,
  [resolve(root, "cli/tasks/apps/sync-server-official-apps.ts"), resolve(output, "catalog.go"), catalogPath],
  apps,
);
writeJSON(resolve(output, "release-manifest.json"), {
  ...
  sdk: ["contracts", "sdk"].map(name => ({ name: `@misty/${name}`, version: readJSON(resolve(root, "packages", name, "package.json")).version, source: capture("git", ["rev-parse", "HEAD"]) })),
