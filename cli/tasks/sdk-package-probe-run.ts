 ...
 * Usage: node cli/tasks/sdk-package-probe-run.ts browser /tmp/misty-sdk-browser-release-candidate
...
const catalogPath = path.join(candidateRoot, "catalog.json");
...
process.env.MISTY_APPS_ROOT ??= path.resolve(root, "./apps");
...
try {
  ...
  server = await createServer({
    ...
    plugins: [{ name: "sdk-browser-download-fixture", enforce: "pre", configureServer(server) {
      server.middlewares.use((request, response, next) => {
        if (request.url?.split("?", 1)[0] === "/official-catalog.json") {
        ...
        if (journalFixture && request.url === "/cli/tasks/sdk-journal-host-fixture.json") {
  ...
  probeOrigin = origin;
  const servedCatalog = await fetch(`${origin}/official-catalog.json`).then(response => response.json());
  ...
  const args = debug
    ...
    : [];
  const catalogUrl = new URL(`${origin}/official-catalog.json`);
