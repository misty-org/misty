...
```sh
cmake -S client -B build -DMISTY_PLUGIN_TRUST_PUBKEY=/path/to/plugin-public.pem
```

To require signatures for every plugin in that build:

```sh
cmake -S client -B build -DMISTY_PLUGIN_TRUST_PUBKEY=/path/to/plugin-public.pem -DMISTY_REQUIRE_SIGNED_PLUGINS=ON
```
