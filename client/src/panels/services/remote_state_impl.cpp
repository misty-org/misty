...
namespace misty::panel {
    ...
    void ServicesState::refetch_sync_items(const std::string& remote,
                                           const std::string& path,
                                           FilesCallback callback) {
        if (!worker_pool_) return;
        ...
        worker_pool_->add(
            [remote, path, callback]() {
                ...
                std::string url = base + "/api/sync/refetch";
                ...
                if (response.status_code >= 200 && response.status_code < 300) {
                    ...
                    std::string err = "HTTP " + std::to_string(response.status_code);
                    if (!response.body.empty()) err += ": " + response.body;
                    std::cerr << "refetch_sync_items(" << remote << ", " << path << "): " << err << std::endl;
                    callback(false, "", err);
                }
            },
            []() {},
    ...
    void ServicesState::fetch_sync_items(const std::string& remote,
        ...
        worker_pool_->add(
            [remote, path, callback]() {
                ...
                std::string url = base + "/api/sync/list?remote=" + core::url_encode(remote)
                    + "&path=" + core::url_encode(path);
                ...
                if (response.status_code >= 200 && response.status_code < 300) {
                    ...
                    std::string err = "HTTP " + std::to_string(response.status_code);
                    if (!response.body.empty()) err += ": " + response.body;
                    std::cerr << "fetch_sync_items(" << remote << ", " << path << "): " << err << std::endl;
                    callback(false, "", err);
                }
            },
            []() {},
