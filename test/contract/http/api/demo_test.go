...
func TestDemoServiceAdminAuthentication(t *testing.T) {
	...
	if service.TestingAuthorized(request) {
...
func TestDemoResetBodyPinsScenarioAndConfirmation(t *testing.T) {
	body := string(TestingDemoResetRequestBody())
	if !strings.Contains(body, TestingDemoScenarioVersion) || !strings.Contains(body, TestingDemoResetConfirm) {
