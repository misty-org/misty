...
- **Tool version.** The installed skill is older than the published one. `context.ts` reports that at boot as `UPDATE_AVAILABLE` and `npx impeccable update` fixes it. Not this command's job.
...
```
node .agents/skills/impeccable/cli/tasks/doctor.ts --json
```
...
- **`auto`** carries no decision. Run `node .agents/skills/impeccable/cli/tasks/doctor.ts --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
