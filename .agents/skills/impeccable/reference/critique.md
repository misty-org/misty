...
- A skipped detector is a failed critique run unless `detect.ts` is missing or crashes after a real attempt.
...
2. **Confirm the target slugs cleanly**:
   ```bash
   node .agents/skills/impeccable/cli/tasks/critique-storage.ts slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
node .agents/skills/impeccable/cli/tasks/detect.ts --json [target]
```
...
4. If mutation is available, start `node .agents/skills/impeccable/cli/tasks/live-server.ts --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
After Assessment B returns usable CLI findings, reuse them. Do not rerun `detect.ts` in the parent unless Assessment B failed, was truncated, or omitted count, rule names, or file locations.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"max_score":<n>,"na_heuristics":"<comma-separated numbers, or empty>","p0_count":<n>,"p1_count":<n>}' \
     node .agents/skills/impeccable/cli/tasks/critique-storage.ts write "<resolved target>" <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node .agents/skills/impeccable/cli/tasks/critique-storage.ts trend "<resolved target>" 5
   ```
