 ...
 *
 * Usage:
 *   node hook-admin.ts status                         # print current state
 *   node hook-admin.ts on                             # set enabled: true
 *   node hook-admin.ts off                            # set enabled: false
 *   node hook-admin.ts ignore-rule <rule-id>          # append to ignoreRules
 *   node hook-admin.ts ignore-rule overused-font --all-values
 *   node hook-admin.ts ignore-file <glob> [--shared|--local]   # append to ignoreFiles
 *   node hook-admin.ts ignore-value <rule> <value>    # append to shared ignoreValues
 *   node hook-admin.ts ignore-value <rule> <value> --local
 *   node hook-admin.ts ignore-value <rule> "*" --file <glob>   # rule off in <glob> only
 *   node hook-admin.ts ignore-value <rule> "*"                 # refused: scope it or use ignore-rule
 *   node hook-admin.ts reset                          # remove all config + cache
 *
...
import { IMPECCABLE_COMMAND } from './lib/provider.ts';
...
const IMPECCABLE_HOOK_COMMAND_MARKERS = [
  'skills/impeccable/cli/tasks/hook-probe.mjs',
  'skills/impeccable/cli/tasks/hook.ts',
  'skills/impeccable/cli/tasks/hook-before-edit.ts',
  'skills/impeccable/cli/tasks/hook-after-edit.mjs',
  'skills/impeccable/cli/tasks/hook-stop.mjs',
];
...
// cli/tasks/lib/transformers/hooks.js in the repo.
...
const HOOK_MANIFEST_TARGETS = [
  {
    ...
    manifest: () => ({
      ...
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/impeccable/cli/tasks/hook.ts"',
        ...
        Stop: [stopManifestEntry('node "${CLAUDE_PROJECT_DIR}/.claude/skills/impeccable/cli/tasks/hook.ts"')],
      },
    }),
  },
  {
    ...
    manifest: () => ({
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node ".agents/skills/impeccable/cli/tasks/hook.ts"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
        ...
        Stop: [stopManifestEntry('node ".agents/skills/impeccable/cli/tasks/hook.ts"')],
      },
    }),
  },
  {
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: 'node ".cursor/skills/impeccable/cli/tasks/hook-before-edit.ts"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
        ...
        postToolUse: [
          {
            ...
            bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/impeccable/cli/tasks/hook.ts"',
