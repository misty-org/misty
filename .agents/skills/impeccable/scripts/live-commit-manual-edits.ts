 ...
 *
 * Usage:
 *   node live-commit-manual-edits.ts
 *   node live-commit-manual-edits.ts --page-url=/
 *
 * Output JSON:
 *   { applied, failed, files, cleared, count, pageUrl }
 */

import { buildManualEditEvidence } from './live-manual-edit-evidence.ts';
import { readBuffer, readBufferStrict, writeBuffer, countByPage } from './live/manual-edits-buffer.ts';
import { isGeneratedFile } from './lib/is-generated.ts';
import {
  runCopyEditBatchAgent,
  runCopyEditPostApplyChecks,
} from './live-copy-edit-agent.ts';
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log('Usage: node live-commit-manual-edits.ts [--page-url=<url>] [--provider=auto|codex|claude|mock]');
    process.exit(0);
  }
...
if (process.argv[1]?.endsWith('live-commit-manual-edits.ts')) {
  main().catch((err) => {
