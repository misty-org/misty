...
import { getLiveDir } from '../lib/impeccable-paths.ts';
import { readBuffer as readManualEditsBuffer } from './manual-edits-buffer.ts';
...
function manualApplyResultShapeHint(eventId = 'EVENT_ID') {
  return `Use live-poll.ts --reply ${eventId} done --data '{"status":"done","appliedEntryIds":["ENTRY_ID"],"failed":[],"files":["src/page.html"],"notes":[]}'`;
}
