...
import { LIVE_CHROME_MOUNT_CONTRACT, LIVE_UI_SURFACES } from './ui-surfaces.ts';
...
export function assembleLiveBrowserScript({
  ...
  appRoot = null,
  parts,
  // Defaulted rather than threaded through live-server.ts: the browser bundle
  ...
  const prelude =
    ...
    // Canonical Live chrome inventory from live/ui-surfaces.ts. live-browser.js
