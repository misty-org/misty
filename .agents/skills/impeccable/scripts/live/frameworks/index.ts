 ...
 *
 * Before this existed, framework knowledge was smeared across live-inject.ts
 * (detection order, the Nuxt adapter, the Astro `is:inline` branch), the two
 * adapter modules, and live-wrap.ts (which extension gets component preview,
 ...
 *    live-inject.ts used to hard-code. static-html always matches, so
...
import { viteGeneric } from './vite-generic.ts';
import { staticHtml } from './static-html.ts';
import { TAG_PATCH_MARKERS, unpatchTagFile } from './tag-strategy.ts';
