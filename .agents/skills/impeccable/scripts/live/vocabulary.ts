 ...
 *   - skill/cli/tasks/live/event-validation.ts — re-exports VISUAL_ACTIONS.
 *   - skill/cli/tasks/live-browser.js — the real picker. It is served raw and
 *     injected as an IIFE, so it cannot import this at runtime; live-server.ts
 ...
 * Every one of these is emitted by `recordAgentPhase()` in live-server.ts;
