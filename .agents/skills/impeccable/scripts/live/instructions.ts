...
function pollCmd(scriptsPath) {
  return `node ${scriptsPath}/live-poll.ts`;
}
  ...
  switch (event.type) {
    ...
    case 'discard':
      return event?._completionAck?.ok === true
        ...
        : `Completion was not acknowledged: run node ${scriptsPath}/live-complete.ts --id ${event.id} --discarded, then poll again.`;
    ...
    case 'exit':
      return `Session over: kill any background poll, then node ${scriptsPath}/live-server.ts stop (removes the injected script tag). Sweep leftover impeccable-variants-start / impeccable-carbonize-start markers from source.`;
    default:
      return undefined;
  }
}
...
function acceptInstructions(event, scriptsPath) {
  ...
  if (result.mode === 'error') {
    if (result.error === 'source_locked') {
      return `${prefix}The source file is briefly locked by a publisher. Re-run the exact same live-accept.ts command (idempotent); do NOT hand-edit the file, and do not poll past this.`;
    }
    if (result.error === 'accept_receipt_conflict') {
      return `${prefix}This session already resolved as ${result.priorOperation || 'a prior operation'}; do not edit anything. Run node ${scriptsPath}/live-status.ts and tell the user what the session resolved to.`;
    }
    return `${prefix}Accept failed: ${result.error || 'unknown error'}. Source was not touched; do not hand-edit. Run node ${scriptsPath}/live-status.ts before continuing.`;
  }
...
/** Boot instructions attached to live.ts's success payload. */
