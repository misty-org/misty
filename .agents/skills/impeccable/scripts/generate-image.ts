 ...
 *
 * context.ts reports availability (it checks OPENAI_API_KEY); harness-native
 ...
 *
 *   node generate-image.ts --prompt "..." --out mock.png [--size 1536x1024] [--quality medium]
 *   node generate-image.ts --prompt-file prompt.txt --out mock.png
 *   node generate-image.ts --prompt "..." --out mock.png --ref screenshot.png [--ref more.png]
 *
...
// metadata via embed-prompt.ts) so intent survives copies across harnesses,
// plus a sidecar for anything that indexes rather than opens the image.
try {
  const { spawnSync } = await import('node:child_process');
  spawnSync(process.execPath, [new URL('./embed-prompt.ts', import.meta.url).pathname, out, '--prompt', prompt], { stdio: 'ignore' });
  fs.writeFileSync(`${out}.json`, JSON.stringify({ prompt, createdAt: new Date().toISOString(), tool: 'generate-image.ts', model: 'gpt-image-2', ...(refs.length ? { refs } : {}) }, null, 2));
