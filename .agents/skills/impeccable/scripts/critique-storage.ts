 ...
 *   node critique-storage.ts slug <resolved-target>
 *   node critique-storage.ts write <slug> <snapshot-body-file>
 *   node critique-storage.ts latest <slug>
 *   node critique-storage.ts trend <slug> [limit]
 *
...
import { getCritiqueDir } from './lib/impeccable-paths.ts';
import { slugFromTarget } from './lib/target-slug.ts';

export { slugFromTarget } from './lib/target-slug.ts';
...
function main(argv) {
  ...
  switch (cmd) {
    ...
    default:
      process.stderr.write('usage: critique-storage.ts <slug|write|latest|trend> [args]\n');
      process.exit(1);
  }
}
