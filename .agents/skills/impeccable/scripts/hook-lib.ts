 ...
 *
 * Pure-ish helpers split out from `hook.ts` so unit tests can exercise
 * config parsing, finding filtering, dedup, render, and cache logic without
 * spawning a subprocess. `hook.ts` itself is the thin stdin/stdout shim.
 *
 ...
 * - The detector loader looks for `detector/detect-antipatterns.ts` next to
 *   this file first (built skill layout) and falls back to the repo root's
 *   `cli/engine/detect-antipatterns.ts` (running from source).
 */
...
import { extractPlatform, loadContext } from './context.ts';
...
// markers. lib/template-extensions.ts owns the shape; re-exported here because
// hook-lib has been the import site for matchConfiguredExtension since #347.
import {
  matchConfiguredExtension,
  mergeExtensions,
} from './lib/template-extensions.ts';
...
// with the registry (cli/engine/registry/antipatterns.ts).
...
// The rule/value pair the footer's `hook-admin.ts ignore-value` command
...
// `hookEventName: "stop"`. hook.ts routes on the raw stdin, before any
...
const DETECTOR_CANDIDATES = [
  path.join(__dirname, 'detector', 'detect-antipatterns.ts'),
  path.join(__dirname, '..', '..', 'cli', 'engine', 'detect-antipatterns.ts'),
  path.join(__dirname, '..', '..', '..', 'cli', 'engine', 'detect-antipatterns.ts'),
];
...
const HOOK_ADMIN_COMMAND = `node ${quoteCommandArg(path.join(__dirname, 'hook-admin.ts'))}`;
...
//      hook-admin.ts invocation is runnable as-is and keeps agents out of
...
function directiveFooter(opts = {}) {
  if (opts.mode === 'short') {
    ...
    // runnable hook-admin.ts invocation, and restating ~70 chars of absolute
    // path on every repeat is the duplication this mode exists to cut.
    return 'Triage per the session policy: fix real problems; persist confident false-positive or sanctioned-exception ignores via `hook-admin.ts ignore-value` and disclose them in your reply; unsure, ask in one line.';
  }
  return [
