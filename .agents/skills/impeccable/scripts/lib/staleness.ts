 ...
 *      Owned by computeUpdateDirective in context.ts, not by this module.
 ...
 *   parses markdown context.ts has in memory, stats a bounded set of paths,
...
// readConfig), `updateCheck` (context.ts), `projectRoots` (context.ts's
// monorepo resolution), `buildPath` (context.ts's build-path directive), plus
...
// The only two values context.ts and new-work honor. A near miss reads as a
 ...
 * impeccable-paths (which resolves the project root) and context.ts (which
