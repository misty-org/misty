 ...
 * number is large.
 *
 * Same finding shape and severities as lib/staleness.ts.
 */
...
//   * bare:            node "${CLAUDE_PROJECT_DIR}/.../hook.ts"
//   * bundle-relative: node ".agents/.../hook.ts"
//   * legacy unquoted: node .claude/.../hook.ts
...
//   * github portable: node "$(git rev-parse --show-toplevel)/.../hook.ts"
 ...
 *
 * `candidates` comes from context.ts's discovery so the walk is not repeated.
 */
...
export async function loadKnownRuleIds(scriptsDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')) {
  // Same two locations detect.ts resolves: the bundled copy in an installed
  // skill, then the source-repo engine when running from a checkout.
  const candidates = [
    path.join(scriptsDir, 'detector', 'detect-antipatterns.ts'),
    path.join(scriptsDir, '..', '..', 'cli', 'engine', 'detect-antipatterns.ts'),
  ];
