 ...
 *
 * Usage:
 *   node live-inject.ts --port PORT [--token TOKEN]  # Insert the live script tag
 *   node live-inject.ts --remove                     # Remove the live script tag
 *   node live-inject.ts --check                      # Check whether live config exists
 *
...
import { resolveLiveConfigPath } from './lib/impeccable-paths.ts';
import {
  describeInjectArtifacts,
  frameworkIgnorePatterns,
  resolveFramework,
  resolveSourceTraits,
} from './live/frameworks/index.ts';
import {
  clearInjectJournal,
  healInjectJournal,
  recordInjection,
} from './live/frameworks/journal.ts';
import {
  buildTagBlock,
  insertTag,
  patchCspMeta,
  removeTag,
  revertCspMeta,
} from './live/frameworks/tag-strategy.ts';
import { buildLiveScriptSrc } from './live/frameworks/script-src.ts';
...
export async function injectCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: node live-inject.ts [options]
...
Output (JSON):
  ...
  // /live.js handler authorizes the browser fetch. `live.ts` always passes
...
if (_running?.endsWith('live-inject.ts') || _running?.endsWith('live-inject.ts/')) {
  enterLiveRoot();
  injectCli();
}

// Re-exported so long-standing importers (live.ts, the adapter modules, the
