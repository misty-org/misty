 ...
 *
 * Usage:
 *   node <scripts_path>/live-server.ts              # start
 *   node <scripts_path>/live-server.ts stop         # stop + remove injected live.js tag
 *   node <scripts_path>/live-server.ts stop --keep-inject   # stop only
 *   node <scripts_path>/live-server.ts --help
 */
...
import { parseDesignMd } from './lib/design-parser.ts';
...
} from './live/browser-script-parts.ts';
import { createLiveSessionStore, GENERATION_FENCED_PHASES } from './live/session-store.ts';
import { runGenerationPreflight } from './live/generation-preflight.ts';
import { validateEvent } from './live/event-validation.ts';
import { selectAvailablePendingEvent } from './live/poll-lanes.ts';
import { createManualEditRoutes } from './live/manual-edit-routes.ts';
import {
  LIVE_COMMANDS,
  VARIANT_PROGRESS_CHECKPOINT_REASONS as VARIANT_PROGRESS_CHECKPOINT_REASON_LIST,
} from './live/vocabulary.ts';
import {
  ...
  writeLiveServerInfo,
} from './lib/impeccable-paths.ts';
import { countByPage as countPendingByPage } from './live/manual-edits-buffer.ts';
import {
  createManualApplyController,
  summarizeManualApplyFailures,
} from './live/manual-apply.ts';
import {
...
function createRequestHandler({ detectScript, liveScriptParts }) {
  return (req, res) => {
    ...
    if (p === '/live.js') {
      ...
      // `?token=...` (see live-inject.ts). A missing/wrong token → 401.
...
if (args.includes('--help') || args.includes('-h')) {
  console.log(`Usage: node live-server.ts [options]
...
Options:
  ...
  --keep-inject Only with stop: skip live-inject.ts --remove
  --help        Show this help
...
if (args.includes('stop')) {
  ...
  if (!keepInject) {
    const injectPath = path.join(__dirname, 'live-inject.ts');
    try {
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log('Inject: managed by live-inject.ts; Astro source tags use is:inline automatically.');
