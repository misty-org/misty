...
import { verifyAcceptedFile } from './live/accept-verify.ts';
...
if (_running?.endsWith('live-complete.ts') || _running?.endsWith('live-complete.ts/')) {
  enterLiveRoot();
  completeCli();
}
