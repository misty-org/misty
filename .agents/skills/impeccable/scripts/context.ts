 ...
 * server-side scripts (live.ts, live-server.ts) that need the structured
...
import { parseTargetOptions } from './lib/target-args.ts';
import { IMPECCABLE_COMMAND, IMPECCABLE_PROVIDER_ID } from './lib/provider.ts';
import { resolveSurfaceBrief } from './lib/surface-briefs.ts';
import { collectBootFindings, designSidecarCandidatesFor } from './lib/staleness.ts';
import {
  buildStalenessDirective,
  filterFreshFindings,
  stalenessCheckDisabled,
} from './lib/staleness-notice.ts';
 ...
 * (this file lives at `<skill>/cli/tasks/context.ts`). Returns null when the
...
function valueHasHookMarker(value) {
  if (typeof value === 'string') {
    return value.includes('skills/impeccable/cli/tasks/hook.ts')
      || value.includes('skills/impeccable/cli/tasks/hook-before-edit.ts');
  }
...
function appendImageGenDirective(parts) {
  ...
  parts.push([
    ...
    `\`node ${scriptsPath}/generate-image.ts --prompt "..." --out <file>\` (gpt-image-2, billed to the user's key; say so before the first render, and never reach for it when a native tool exists).`,
...
function appendDetectorFallback(parts, ctx) {
  ...
  parts.push([
    ...
    `Once the changed web UI is finished, run the mechanical detector over it: \`node ${scriptsPath}/detect.ts --json <changed targets>\`.`,
...
function appendSurfaceBriefContext(parts, ctx) {
  ...
  const helper = path.join(path.dirname(fileURLToPath(import.meta.url)), 'surface-brief.ts');
  parts.push(
...
function buildMissingTargetDirective() {
  const script = process.argv[1] || 'context.ts';
  return (
    'MONOREPO_TARGET_REQUIRED: This is a monorepo and context.ts ran without --target. ' +
...
// scripts like `load-context.ts`, and realpath tolerates symlinked
