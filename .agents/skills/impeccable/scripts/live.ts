 ...
 *   - Enter the harness-native poll loop: `node live-poll.ts`
 *
 * Usage:
 *   node live.ts                   # Prepare everything, print JSON, exit
 *   node live.ts --help
 */
...
import { resolveTargetSelection } from './context.ts';
import { resolveFiles } from './live-inject.ts';
import { readLiveServerInfo } from './lib/impeccable-paths.ts';
import { resolveSurfaceBrief } from './lib/surface-briefs.ts';
import { resolveLiveTarget } from './live-target.ts';
import { bootInstructions } from './live/instructions.ts';
import { resolveRoots, writeRootsManifest } from './live/roots.ts';
...
async function liveCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: node live.ts
...
The agent should then:
  ...
  4. Enter the poll loop: node live-poll.ts`);
    process.exit(0);
  }
  ...
  const checkOut = runScript('live-inject.ts', ['--check'], { cwd: activeCwd });
...
/**
 * Same glob-to-regex mapping used by live-inject.ts. Kept inline here
 * to avoid a circular import (live-inject.ts already imports nothing
 * from live.ts). The two must stay in sync.
 */
...
function ensureServerRunning(cwd = process.cwd()) {
  ...
  // Start a new server
  const out = runScript('live-server.ts', ['--background'], { cwd });
...
if (_running?.endsWith('live.ts') || _running?.endsWith('live.ts/')) {
  liveCli();
}
