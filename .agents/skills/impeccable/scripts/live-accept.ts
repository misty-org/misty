 ...
 *
 * Usage:
 *   node live-accept.ts --id SESSION_ID --discard
 *   node live-accept.ts --id SESSION_ID --variant N
 *
...
import { isGeneratedFile } from './lib/is-generated.ts';
import { getLiveDir, safeSessionId } from './lib/impeccable-paths.ts';
import { resolveLiveTemplateExtensions } from './lib/template-extensions.ts';
import { readBuffer as readManualEditsBuffer, writeBuffer as writeManualEditsBuffer } from './live/manual-edits-buffer.ts';
import { NEVER_SOURCE_DIRS, findSourceFile } from './live/source-search.ts';
import { withSourceLockSync } from './live/source-lock.ts';
import {
...
// Mirrors VARIANT_ID_PATTERN in live/event-validation.ts, which gates the same
 ...
 *
 * live/completion.ts only classifies a result as `error` when it carries
 ...
 * without `mode`, so completion.ts classified them as agent_done and
...
export async function acceptCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: node live-accept.ts [options]
  ...
  if (priorReceipt) {
    ...
    console.log(JSON.stringify(sameOperation
      ...
      : {
          ...
          // handoff. Without it, live/completion.ts classifies the reply as
...
/**
 * De-indent content that was indented by live-wrap.ts.
...
if (_running?.endsWith('live-accept.ts') || _running?.endsWith('live-accept.ts/')) {
  enterLiveRoot();
  acceptCli();
}
