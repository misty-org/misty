 ...
 *
 * Usage:
 *   node <scripts_path>/live-poll.ts                         # Block until browser event, print JSON
 *   node <scripts_path>/live-poll.ts --stream                # Experimental: keep polling; one JSON line per event
 *   node <scripts_path>/live-poll.ts --timeout=600000        # Custom timeout (ms); default is long-poll friendly
 *   node <scripts_path>/live-poll.ts --reply <id> done       # Reply "done" to event <id>
 *   node <scripts_path>/live-poll.ts --reply <id> error "msg" # Reply with error
 */

import { execFileSync } from 'node:child_process';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { completionAckForAcceptResult, completionTypeForAcceptResult } from './live/completion.ts';
import { readLiveServerInfo } from './lib/impeccable-paths.ts';
import { enterLiveRoot } from './live/roots.ts';
import { instructionsForEvent } from './live/instructions.ts';
...
function readServerInfo() {
  ...
  if (!record) {
    console.error(`No running live server found. Start one with: ${scriptCmd('live.ts')}`);
    process.exit(1);
  }
  return record.info;
}
...
export function manualApplyPollBanner(event = {}) {
  ...
  return [
    `Manual Apply action required: edit source, then reply with \`live-poll.ts --reply ${id} done --data '<json>'\`.`,
    'The JSON data must include status, appliedEntryIds, failed, files, and notes; summary counters are only a recovery fallback.',
    'Do not run live-commit-manual-edits.ts for this leased event.',
...
function validateReplyArgs({ id, status }) {
  const usage = `Usage: ${scriptCmd('live-poll.ts')} --reply <id> <status> [--file path] [--data '<json>'] [message]`;
  if (!id || id.startsWith('--')) {
...
export async function augmentEventWithAcceptHandling(event, base, token) {
  ...
  const acceptScript = path.join(__dirname, 'live-accept.ts');
...
export function writeCarbonizeBanner(event) {
  ...
  if (event._acceptResult?.carbonize === true) {
    process.stderr.write('\n⚠ Carbonize cleanup REQUIRED before next poll. After cleanup, run live-complete.ts --id ' + event.id + '. See reference/live.md "Required after accept".\n\n');
  }
}
...
function handlePollError(err) {
  if (err.code === 'AUTH_FAILED') {
    ...
    console.error(`Try restarting: ${scriptCmd('live-server.ts')} stop && ${scriptCmd('live.ts')}`);
    process.exit(1);
  }
  if (err.cause?.code === 'ECONNREFUSED') {
    console.error(`Live server not running. Start one with: ${scriptCmd('live.ts')}`);
    process.exit(1);
  }
...
Harness note:
  ...
  // Reply mode: node <scripts_path>/live-poll.ts --reply <id> <status> [--file path] [--data '<json>'] [message]
...
if (_running?.endsWith('live-poll.ts') || _running?.endsWith('live-poll.ts/')) {
  enterLiveRoot();
  pollCli();
}
