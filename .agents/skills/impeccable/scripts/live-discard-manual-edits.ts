 ...
 *
 * Usage:
 *   node live-discard-manual-edits.ts              # discard all pending
 *   node live-discard-manual-edits.ts --page-url=/ # discard only entries for "/"
 *
 * Output JSON: { discarded: N, entries: [...discardedEntries], totalCount: N }
 */

import { readBuffer, removeEntries, truncateBuffer } from './live/manual-edits-buffer.ts';
...
if (args.includes('--help') || args.includes('-h')) {
  console.log('Usage: node live-discard-manual-edits.ts [--page-url=<url>]');
  process.exit(0);
}
