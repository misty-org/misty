 ...
 * live-commit-manual-edits.ts, which builds a page-scoped batch and uses this
...
export function describeNoProviderError({
  ...
  if (exists('claude')) {
    if (env.CLAUDE_CODE_OAUTH_TOKEN) {
      ...
      lines.push('      Headless fix: run `claude setup-token` once, then `export CLAUDE_CODE_OAUTH_TOKEN=<the printed sk-ant-oat01-… token>` before starting `live-server.ts`.');
