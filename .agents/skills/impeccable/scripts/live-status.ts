...
import { manualApplyResumeHint, mountFailureAction, renderSummary } from './live-resume.ts';
...
function recoveryHint({ server, manualApply, renderFailure }) {
  ...
  if (server) {
    return 'Run live-poll.ts to continue pending work, or live-complete.ts --id <session> after manual cleanup.';
  }
  return 'Start live-server.ts to requeue pending durable events, then run live-poll.ts.';
}
...
if (_running?.endsWith('live-status.ts') || _running?.endsWith('live-status.ts/')) {
  enterLiveRoot();
  statusCli();
}
