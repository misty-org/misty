 ...
 *
 * Usage:
 *   node live-insert.ts --id SESSION_ID --count N --position after \
...
export async function insertCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: node live-insert.ts [options]
...
Output (JSON):
  ...
  // See live-wrap.ts: preflight computes the scaffold but leaves source
...
if (_running?.endsWith('live-insert.ts') || _running?.endsWith('live-insert.ts/')) {
  enterLiveRoot();
  insertCli();
}
