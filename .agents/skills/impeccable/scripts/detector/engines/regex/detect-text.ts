import { GENERIC_FONTS, OVERUSED_FONTS, EM_DASH_FLOOR, EM_DASH_CHARS_PER_DASH } from '../../shared/constants.ts';
import { isNeutralColor } from '../../shared/color.ts';
import { extractGoogleFontFamilies } from '../../shared/fonts.ts';
import { checkSourceDesignSystem } from '../../design-system.ts';
import { scanCssTextForGlow, scanCssTextForGridBackground, scanCssTextForMarquee, scanCssTextForPseudoStripe, scanCssTextForRadialHalo } from '../../rules/checks.ts';
import { isFullPage } from '../../shared/page.ts';
import { applyInlineIgnores } from '../../shared/inline-ignores.ts';
import { finding } from '../../findings.ts';
import { profileFindings, profileStep } from '../../profile/profiler.ts';
 ...
 *
 * shared/color.ts's isNeutralColor only parses the computed function forms a
...
function isNeutralAuthoredColor(rawColor) {
  ...
  // Modern rgb() takes space-separated channels (`rgb(0 0 0)`). shared/color.ts
