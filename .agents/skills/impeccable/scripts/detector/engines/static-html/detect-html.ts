...
import { GENERIC_FONTS, OVERUSED_FONTS } from '../../shared/constants.ts';
import {
...
import { profileFindings, profileStep, profileStepAsync } from '../../profile/profiler.ts';
import {
  checkElementBorders,
...
import { detectText, runTextContentAnalyzers } from '../regex/detect-text.ts';
import {
  StaticDocument,
  buildStaticStyleMap,
  buildStaticWindow,
...
async function detectHtml(filePath, options = {}) {
  ...
  if (isFullPage(html)) {
    ...
    // rules/checks.ts): CSS-property regexes must not fire on prose ABOUT
