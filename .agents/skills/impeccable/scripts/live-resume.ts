...
import { createLiveSessionStore } from './live/session-store.ts';
...
function manualApplyReplyCommand(eventOrId = 'EVENT_ID') {
  ...
  return `live-poll.ts --reply ${id} done --data '<json>'`;
}
...
export function manualApplyResumeHint(event = {}) {
  ...
  return `Manual Apply pending${scope}. If you have not already leased it, run live-poll.ts. Apply the source edits from the manual_edit_apply batch, then reply with ${manualApplyReplyCommand(event.id)}. Polling only leases this work item; it does not commit source edits. Do not run live-commit-manual-edits.ts for this leased event. Do not poll again before replying.`;
}
...
export async function resumeCli() {
  ...
  const nextAction = pending?.type === 'manual_edit_apply'
    ...
    : mountAction || (pending
      ? `Run live-poll.ts, handle ${pending.type} ${pending.id}, then acknowledge with live-poll.ts --reply ${pending.id} done.`
      : snapshot.phase === 'carbonize_required'
        ? `Finish carbonize cleanup${snapshot.sourceFile ? ` in ${snapshot.sourceFile}` : ''}, then run live-complete.ts --id ${snapshot.id}.`
        : snapshot.phase === 'accept_requested'
          ? `Run live-complete.ts --id ${snapshot.id} after verifying the accepted variant is written.`
...
if (_running?.endsWith('live-resume.ts') || _running?.endsWith('live-resume.ts/')) {
  enterLiveRoot();
  resumeCli();
}
