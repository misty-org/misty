 ...
 *
 * Usage:
 *   node <scripts_path>/live-wrap.ts --id SESSION_ID --count N --query "hero-combined-left" [--file path]
 *
...
import { readBuffer as readManualEditsBuffer } from './live/manual-edits-buffer.ts';
import { findSourceFile } from './live/source-search.ts';
import { resolveSourceTraits } from './live/frameworks/index.ts';
import {
...
The agent should insert variant HTML at insertLine.`);
  ...
  // stranded the browser at 0/N (live-server.ts missed-completion note). It is
  ...
  if (useSvelteComponent) {
    ...
    // DOM, and live-accept.ts inlines the accepted variant back into the route.
    //
 ...
 * preview. See live/frameworks/index.ts for why extension and not project.
 */
...
// Auto-execute when run directly (node live-wrap.ts ...)
const _running = process.argv[1];
if (_running?.endsWith('live-wrap.ts') || _running?.endsWith('live-wrap.ts/')) {
  enterLiveRoot();
  wrapCli();
}
