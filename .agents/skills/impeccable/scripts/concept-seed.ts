 ...
 *
 * Usage:
 *   node cli/tasks/concept-seed.ts --scope direction --mode persuade
 *   node cli/tasks/concept-seed.ts --scope surface --mode operate --from <key>
 *   node cli/tasks/concept-seed.ts --scope surface --mode operate --grain flow
 *   node cli/tasks/concept-seed.ts --scope direction --candidate-count 6
 *   node cli/tasks/concept-seed.ts --scope direction --mode persuade --from <key> --reroll 1
 *   node cli/tasks/concept-seed.ts --scope direction --mode persuade --from <key> --reroll 1 --register bolder
 *   node cli/tasks/concept-seed.ts --chosen <challenger-id> --kind challenger --from <key> --scope direction
 *   node cli/tasks/concept-seed.ts --kind assigned --from <key> --scope direction
 *
...
import {
  ...
  WELL_TIERS,
} from './lib/concept-catalog.ts';
import { readCompositionCatalog } from './lib/composition-catalog.ts';
import {
  COMPOSITION_GRAINS,
...
// Selection itself lives in lib/roll-selection.ts so this script and the roll
...
(serve-question.ts, text-only card); the structured question tool remains
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    if (chosenIdx !== -1 || kindIdx !== -1) {
      ...
      const { loadContext } = await import('./context.ts');
