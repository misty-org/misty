...
function main(argv) {
  ...
  if (command === 'write') {
    if (!target || !bodyFile) throw new Error('usage: surface-brief.ts write <primary-target> <body-file>');
  ...
  throw new Error('usage: surface-brief.ts <path|list|read|write> [target] [body-file] [related-target ...]');
}
