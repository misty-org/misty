 ...
 *
 *   node doctor.ts                 # human-readable report
 *   node doctor.ts --json          # machine-readable, for the skill command
 *   node doctor.ts --fix           # apply the mechanical migrations only
 *   node doctor.ts --target <path> # pick a monorepo workspace
 *
 * The boot check in context.ts reports what a session can afford to measure.
...
import { loadContext, extractPlatform, resolveTargetSelection } from './context.ts';
...
function usage() {
  return [
    `Usage: node doctor.ts [--json] [--fix] [--target <path>]`,
    '',
...
// Read straight from disk rather than importing context.ts's private reader.
...
function renderText(report, fixes) {
  ...
  if (fixes) {
    ...
    lines.push(`Run \`node doctor.ts --fix\` to apply the automatic migrations, `
