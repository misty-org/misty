...
func libraryVideoFilters(definition db.LibraryEditDefinition) []string {
	...
	brightness := math.Max(-1, math.Min(1, definition.Brightness-1))
	if brightness != 0 || definition.Contrast != 1 || definition.Saturation != 1 {
		filters = append(filters, fmt.Sprintf("eq=brightness=%s:contrast=%s:saturation=%s", formatMediaNumber(brightness), formatMediaNumber(definition.Contrast), formatMediaNumber(definition.Saturation)))
	}
