...
func (proxy *Proxy) MountHandlers() {
	...
	proxy.APIRouter.Get("/session", api.CurrentSession(proxy.Database))
	proxy.APIRouter.Post("/session/refresh", api.RefreshSession(proxy.Database))
...
func mountRemoteRoutes(r chi.Router, base string) {
	...
	r.Get(base+"/status", remote.ListRemoteStatuses())
	r.Post(base+"/config/start", remote.ConfigStart())
	r.Post(base+"/config/continue", remote.ConfigContinue())
	r.Post(base+"/config/reconnect", remote.ConfigReconnect())
	r.Post(base+"/config/repair", remote.ConfigRepair())
