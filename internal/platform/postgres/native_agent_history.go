...
func (db *Database) NativeAgentConversationReceipts(ctx context.Context, userID, agentID, spaceID, conversationID, currentInvocation string) ([]NativeAgentActionReceipt, error) {
	items := []NativeAgentActionReceipt{}
	err := db.TestingWithRLSContext(ctx, userRLSSettings(userID), func(tx *sql.Tx) error {
		rows, err := tx.QueryContext(ctx, `SELECT j.tool_name,j.state,LEFT(j.request::text,2000),LEFT((j.result-'image')::text,3000),COALESCE(j.error_code,''),COALESCE((SELECT c.metadata->>'app_id' FROM ai_invocation_contexts c WHERE c.user_id=$1 AND c.invocation_id=i.id AND c.opaque_ref=j.request->>'scopeId' LIMIT 1),'') FROM agent_toolbox_action_journal j JOIN ai_invocations i ON i.id=j.run_id AND i.user_id=j.user_id WHERE j.user_id=$1 AND i.request_payload->>'agent_id'=$2 AND i.space_id=$3 AND i.conversation_id=$4 AND i.id<>$5 ORDER BY j.created_at DESC LIMIT 12`, userID, agentID, spaceID, conversationID, currentInvocation)
		if err != nil {
			return err
		}
		defer rows.Close()
