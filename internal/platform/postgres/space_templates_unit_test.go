...
func TestNormalizeSetupProvidersRejectsUnknownAndDeduplicates(t *testing.T) {
	got, err := normalizeSetupProviders([]string{"notion", "discord", "NOTION", "google"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := normalizeSetupProviders([]string{"slack"}); err == nil {
...
func TestBuiltInSpaceTemplateStarterContent(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.id, func(t *testing.T) {
			template, ok := templateByID(test.id)
			if !ok {
