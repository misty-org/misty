...
func (db *Database) BrowserSyncWorkspace(ctx context.Context, userID string) (*SyncWorkspace, error) {
	var w SyncWorkspace
...
func (db *Database) CreateBrowserSyncWorkspace(ctx context.Context, userID string, root []byte, keyEnvelope SyncKeyEnvelope, grant SyncDeviceGrant) error {
...
func (db *Database) EnrollBrowserSyncDevice(ctx context.Context, userID string, g SyncDeviceGrant) error {
...
func (db *Database) PublishBrowserSync(ctx context.Context, userID string, m SyncMutation) (*SyncReceipt, error) {
	...
	var public []byte
	var counter int64
	err = tx.QueryRowContext(ctx, `SELECT public_key,last_counter FROM browser_sync_devices WHERE workspace_id=$1 AND device_id=$2 AND revoked_at IS NULL`, m.WorkspaceID, m.DeviceID).Scan(&public, &counter)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrSyncForbidden
	}
	if err != nil {
...
func (db *Database) ReplayBrowserSync(ctx context.Context, userID, workspaceID, deviceID string, after int64, limit int) (*SyncReplay, error) {
