...
func (db *Database) ValidateSocialOutboundDelivery(ctx context.Context, id string) error {
	return db.TestingSpaceTx(ctx, func(tx *sql.Tx) error {
		var live bool
		if err := tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM social_outbound_commands c JOIN space_members m ON m.space_id=c.space_id AND m.user_id=c.requested_by_user_id WHERE c.id=$1 AND c.state='sending')`, id).Scan(&live); err != nil {
			return err
		}
		if !live {
