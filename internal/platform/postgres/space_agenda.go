...
func (db *Database) SpaceAgenda(ctx context.Context, userID, spaceID string, from, to time.Time) (SpaceAgendaSnapshot, error) {
	...
	err := db.TestingSpaceTx(ctx, func(tx *sql.Tx) error {
		...
		linkedEvents, err := loadSpaceAgendaTasksTx(ctx, tx, spaceID, from, to, &out)
		if err != nil {
			return err
		}
		...
		return loadSpaceAgendaRoadmapDatesTx(ctx, tx, spaceID, from, to, &out)
	})
...
func loadSpaceAgendaTasksTx(ctx context.Context, tx *sql.Tx, spaceID string, from, to time.Time, out *SpaceAgendaSnapshot) (map[string]bool, error) {
	...
	) ORDER BY COALESCE((schedule->>'starts_at')::timestamptz,due_at),id`, spaceID, from, to)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
