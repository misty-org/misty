...
func scanSpaceAgentMembership(row scanner, out *SpaceAgentMembership) error {
	return row.Scan(&out.ID, &out.SpaceID, &out.AgentID, &out.OwnerUserID, &out.Name, &out.Role, &out.Description, &out.Icon, &out.Avatar, &out.Instructions, &out.ModelID, &out.ReasoningEffort,
		&out.Enabled, &out.ApprovedVersionID, &out.ApprovedVersion, &out.LatestVersionID, &out.LatestVersion,
...
func (db *Database) AddSpaceAgentMembership(ctx context.Context, userID, spaceID string, input SpaceAgentMembershipInput) (*SpaceAgentMembership, error) {
	...
	err = db.TestingSpaceTx(ctx, func(tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, `SELECT a.owner_user_id,v.id,v.role FROM personal_agents a JOIN personal_agent_versions v ON v.agent_id=a.id AND v.version=a.version
			WHERE a.id=$1 AND a.enabled AND a.deleted_at IS NULL`, input.AgentID).Scan(&owner, &latestVersionID, &latestRole); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
		...
		_, err := tx.ExecContext(ctx, `INSERT INTO personal_agent_space_grants(
			id,agent_id,space_id,all_members,created_by_user_id,enabled,approved_version_id,space_role,space_instructions,permissions,managed_by_user_id,removed_at
		) VALUES($1,$2,$3,TRUE,$4,TRUE,$5,$6,$7,$8,$4,NULL)
		ON CONFLICT(agent_id,space_id) DO UPDATE SET all_members=TRUE,enabled=TRUE,approved_version_id=$5,
			space_role=$6,space_instructions=$7,permissions=$8,managed_by_user_id=$4,removed_at=NULL,version=personal_agent_space_grants.version+1,updated_at=NOW()`,
			id, input.AgentID, spaceID, userID, latestVersionID, spaceRole, input.SpaceInstructions, permissions)
		if err != nil {
			return err
		}
...
func (db *Database) UpdateSpaceAgentMembership(ctx context.Context, userID, spaceID, agentID string, input SpaceAgentMembershipInput) (*SpaceAgentMembership, error) {
	...
	err = db.TestingSpaceTx(ctx, func(tx *sql.Tx) error {
		...
		result, err := tx.ExecContext(ctx, `UPDATE personal_agent_space_grants SET enabled=$1,space_role=COALESCE($2,space_role),space_instructions=$3,permissions=$4,
			managed_by_user_id=$5,version=version+1,updated_at=NOW() WHERE space_id=$6 AND agent_id=$7 AND removed_at IS NULL AND version=$8`,
			enabled, spaceRole, input.SpaceInstructions, permissions, userID, spaceID, agentID, input.MembershipVersion)
		if err != nil {
			return err
		}
...
func (db *Database) EffectiveAgentSpacePermission(ctx context.Context, userID, spaceID, agentID, permission string) (bool, error) {
	...
	err := db.TestingSpaceTx(ctx, func(tx *sql.Tx) error {
		...
		allowed = agentMembershipPermission(membership.Permissions, permission)
		return nil
	})
	return allowed, err
}
...
func (db *Database) EffectivePersonalAgentToolPermissions(ctx context.Context, userID, spaceID, agentID string) (json.RawMessage, error) {
	...
	err := db.TestingSpaceTx(ctx, func(tx *sql.Tx) error {
		...
		return tx.QueryRowContext(ctx, `SELECT tool_permissions FROM personal_agents WHERE id=$1 AND enabled AND deleted_at IS NULL`, agentID).Scan(&raw)
	})
	return raw, err
}
...
func (db *Database) CreatePersonalAgentSpaceRun(ctx context.Context, userID, spaceID, agentID, sourceConversationID, sourceType, triggerKind string, input, envelope json.RawMessage) (*SpaceRun, error) {
	if sourceType != "group_mention" && sourceType != RunSourceAgentConsole || triggerKind != "mention" && triggerKind != RunSourceAgentConsole {
		return nil, ErrSpaceInvalid
	}
	...
	err := db.TestingSpaceTx(ctx, func(tx *sql.Tx) error {
		...
		) VALUES($1,$2,'agent',$3,$4,$4,$5,'running',$6,'{}'::jsonb,$4,NULLIF($7,''),$8,$3,'chat','{}'::jsonb,'[]'::jsonb,1,$9)
		RETURNING `+spaceRunColumns, out.ID, spaceID, agentID, userID, triggerKind, input, sourceConversationID, sourceType, envelope), out); err != nil {
			return err
		}
