...
func TestInspectLibraryContentVerifiesSafeContent(t *testing.T) {
	...
	detected, intrinsic, err := inspectLibraryContent(bytes.NewReader(data), int64(len(data)), "notes.txt", "text/plain")
	if err != nil {
...
func TestInspectLibraryContentRejectsDangerousAndMalwareContent(t *testing.T) {
	if _, _, err := inspectLibraryContent(strings.NewReader("#!/bin/sh"), 9, "run.sh", "text/plain"); libraryInspectionCode(err) != "dangerous_file_type" {
		t.Fatalf("script error = %v, want dangerous_file_type", err)
	}
	eicar := []byte("prefix EICAR-STANDARD-ANTIVIRUS-TEST-FILE suffix")
	if _, _, err := inspectLibraryContent(bytes.NewReader(eicar), int64(len(eicar)), "test.txt", "text/plain"); !errors.Is(err, errLibraryMalware) {
