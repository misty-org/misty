...
misty_test_container="misty-browser-test-$$"
cleanup() { docker rm -fv "$misty_test_container" >/dev/null 2>&1 || true; }
trap cleanup EXIT INT TERM
docker run -d --name "$misty_test_container" \
  -e POSTGRES_PASSWORD=misty-isolated-test -e POSTGRES_DB=misty_browser_test \
  --tmpfs /var/lib/postgresql/data:rw,size=1536m \
  -p 127.0.0.1::5432 pgvector/pgvector:pg16 >/dev/null
for attempt in $(seq 1 60); do
  if docker exec "$misty_test_container" psql -U postgres -d misty_browser_test -c "SELECT 1" >/dev/null 2>&1; then break; fi
...
export TEST_DB_NAME=misty_browser_test TEST_DB_SSLMODE=disable
export MISTY_BILLING_TEST_DSN="host=127.0.0.1 port=$misty_test_port user=postgres password=misty-isolated-test dbname=misty_browser_test sslmode=disable"
...
def execute(sql):
    subprocess.run(["docker", "exec", "-i", container, "psql", "-q", "-v", "ON_ERROR_STOP=1",
                    "-U", "postgres", "-d", "misty_browser_test"], input=sql, text=True, check=True)
