...
func (db *Store) ControlBrowserSyncDevice(ctx context.Context, user string, c SyncDeviceControl) (string, error) {
	if !validSyncID(c.DeviceID) || (c.DisplayName != nil && (!utf8.ValidString(*c.DisplayName) || len(*c.DisplayName) > 160 || strings.TrimSpace(*c.DisplayName) == "")) || (c.Platform != nil && len(*c.Platform) > 32) || (c.ControlVersion != nil && *c.ControlVersion != 1) {
	...
	var version int
	var full bool
	err = tx.QueryRowContext(ctx, `SELECT d.workspace_id,d.control_version,d.full_sync FROM browser_sync_devices d JOIN browser_sync_workspaces w USING(workspace_id) WHERE w.user_id=$1 AND d.device_id=$2 AND d.revoked_at IS NULL FOR UPDATE OF d`, user, c.DeviceID).Scan(&workspace, &version, &full)
