...
// An explicit legacy Space filter still requires current membership.
func (db *Database) MistyActivity(ctx context.Context, userID, spaceID string, agentIDs ...string) (json.RawMessage, error) {
   ...
   FROM ai_invocations i WHERE i.user_id=$1 AND COALESCE(i.space_id,'')=$2 AND ($3='' OR i.request_payload->>'agent_id'=$3)
   UNION ALL
   ...
   FROM space_runs r WHERE r.owner_user_id=$1 AND COALESCE(r.space_id,'')=$2 AND ($3='' OR r.agent_id=$3) AND (r.parent_run_id IS NOT NULL OR NULLIF(r.input->>'parent_invocation_id','') IS NOT NULL)
