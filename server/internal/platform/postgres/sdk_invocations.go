...
func (db *Database) AdmitSDKInvocation(ctx context.Context, userID string, request cap.Invocation) (*SDKInvocationRecord, error) {
	...
	err := db.TestingWithRLSContext(ctx, userRLSSettings(userID), func(tx *sql.Tx) error {
		...
		if a != nil {
			...
			if err := tx.QueryRowContext(ctx, `SELECT granted_scopes,authority_generation FROM user_app_installations WHERE user_id=$1 AND app_id=$2 AND state='installed' AND NOT consent_required FOR SHARE`, userID, a.AppID).Scan(&scopesRaw, &generation); err != nil {
				return err
			}
			var scopes []string
