...
// Estimates are customer-visible results from the configured adapter.
func (s *SmartLibraryService) hostedAIWeeklyRatio(userID string, images int64) float64 {
	service := s.database.BillingService()
	if !service.Adapter.Enabled() {
		return 0
	}
	decision, err := service.Adapter.Do(context.Background(), "check", billingadapter.Request{Version: 1, AccountID: userID, Operation: "library.images", OperationID: "preview", Key: "preview", Usage: billingadapter.Usage{Units: map[string]int64{"images": images}, Estimated: true}})
	if err != nil {
		return 0
	}
	var summary struct {
	...
	if json.Unmarshal(decision.Summary, &summary) != nil {
		return 0
	}
	return summary.EstimateRatio
}
