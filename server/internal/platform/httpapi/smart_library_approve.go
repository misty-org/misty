...
func (s *SmartLibraryService) Approve(kind string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		analyzer := s.analyzer.WithBilling(s.database.BillingService(), userID, "smart-library:"+batch.ID)
		analysis, analysisErr := analyzer.Analyze(r.Context(), assets)
		...
		for _, result := range analysis.Results {
			...
			embeddings, usage, embedErr := analyzer.EmbedAssets(r.Context(), []serveragent.SmartLibraryAsset{assetsByID[result.AssetID]}, map[string]serveragent.SmartLibraryMetadata{result.AssetID: result})
