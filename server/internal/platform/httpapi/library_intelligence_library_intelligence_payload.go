...
func (s *SpaceLibraryService) ProcessIntelligenceJobs(ctx context.Context, workerID string, limit int) (int, error) {
	...
	for processed < limit {
		...
		if err := s.processIntelligenceJob(ctx, job); err != nil {
			...
			switch {
			case errors.Is(err, billingadapter.ErrUnavailable):
				code = "billing_unavailable"
			case errors.Is(err, billingadapter.ErrDenied), errors.As(err, &insufficient):
...
func (s *SpaceLibraryService) processIntelligenceJob(ctx context.Context, job *db.LibraryIntelligenceJob) error {
	...
	analyzer := s.intelligence.WithBilling(s.database.BillingService(), job.BillingUserID, "space-library-intelligence:"+job.ID+":"+job.LeaseToken)
	analysis, err := analyzer.Analyze(ctx, []serveragent.SmartLibraryAsset{asset})
