...
func (s *SmartLibraryService) CompleteReindex() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		analyzer := s.analyzer.WithBilling(s.database.BillingService(), userID, requestKey)
		...
		for folderID, refreshInputs := range refreshByFolder {
			analysis, analyzeErr := analyzer.Analyze(r.Context(), refreshInputs)
		...
		for _, record := range records {
			...
			embeddings, usage, embedErr := analyzer.EmbedAssets(r.Context(), []serveragent.SmartLibraryAsset{input}, map[string]serveragent.SmartLibraryMetadata{input.AssetID: metadata})
...
func (s *SmartLibraryService) estimate(userID string, folder *db.SmartLibraryFolder) map[string]any {
	...
	return map[string]any{"eligibleImages": folder.EligibleImages, "includedImages": folder.IncludedImages, "billableImages": folder.BillableImages, "hostedAIWeeklyRatio": s.hostedAIWeeklyRatio(userID, int64(folder.BillableImages))}
}
