...
func (s *SmartLibraryService) PlanReindex() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, http.StatusCreated, map[string]any{"jobId": job.ID, "status": job.Status, "targetVersion": job.Version, "embeddingModel": job.Model, "nextCursor": job.Cursor, "assets": assets, "hostedAIWeeklyRatio": s.hostedAIWeeklyRatio(userID, int64(len(job.Assets)))})
	}
}
