...
func TestBrowserSyncNativeWorkerAgainstGo(t *testing.T) {
	...
	command.Env = append(os.Environ(), "MISTY_SYNC_FIXTURE_BASE="+server.URL, "MISTY_SYNC_FIXTURE_COOKIE="+TestingSessionCookieName+"="+cookie)
	...
	workspace, err := database.BrowserSyncWorkspace(context.Background(), "owner")
	...
	devices, err := database.BrowserSyncDevices(context.Background(), "owner", workspace.WorkspaceID)
