...
export async function runSpaceTaskAgent(input: SpaceTaskWorkflowInput) {
  ...
  if (context.routine_execution !== undefined) {
    throw new FatalError("Retired automation execution cannot be resumed.");
  }
      ...
      tool({
        ...
        toModelOutput: ({output}) => {
          if ((descriptor.name === "browser.visual" || descriptor.name === "browser.workspace.visual") && output && typeof output === "object") {
            const result = output as Record<string, unknown>;
            if (Array.isArray(result.displayCaptures)) {
              const {displayCaptures, ...observed} = result;
              const content: Array<{type:"text";text:string}|{type:"image-data";data:string;mediaType:string}> = [{type:"text",text:JSON.stringify(observed)}];
              for (const capture of displayCaptures as Array<{name:string;dataUrl:string;width:number;height:number}>) {
                const match = capture.dataUrl.match(/^data:(image\/(?:png|jpeg|webp));base64,([A-Za-z0-9+/=]+)$/);
                if (match?.[1] && match[2]) content.push({type:"text",text:`Fresh ${capture.name}: ${capture.width} x ${capture.height} screenshot pixels; top-left origin`},{type:"image-data",data:match[2],mediaType:match[1]});
              }
  ...
  try {
    const shared = {
      providerOptions: {
        gateway: { models: context.companion_mode ? [context.model_id] : fallbackModels(context.model_id) },
      },
                  ...
                  (image) =>
                    image.mime_type === "text/plain"
                      ...
                      : image.mime_type === "application/pdf"
                        ...
                        : [{type:"text" as const,text:`${image.name} (${image.width} x ${image.height} screenshot pixels; top-left origin)`}, {type:"image" as const,image:image.data_url,mediaType:image.mime_type}],
                ),
              ],
            },
          ]
