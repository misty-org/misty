...
test("signed controls bootstrap, advance ACL, and purge durable state", async (t) => {
  const internalSecret = "internal-secret-that-is-at-least-32-bytes"; // gitleaks:allow -- synthetic test fixture
  ...
  const child = spawn(process.execPath, ["index.ts"], {
    ...
    env: {
      ...
      JOURNAL_COLLAB_PROJECTION_SECRET: Buffer.alloc(32, 19).toString("base64"),
