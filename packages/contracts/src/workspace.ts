import { z } from "zod";
export type MistyViewState = null | boolean | number | string | MistyViewState[] | { [key: string]: MistyViewState };
...
export const MistyViewStateSchema = z.custom<MistyViewState>((root) => {
  const pending: Array<[unknown, number, boolean?]> = [[root, 0]], seen = new Set<object>();
  let nodes = 0, bytes = 0;
  while (pending.length) {
    ...
    if (value === null || typeof value === "boolean") bytes += 5;
    else if (typeof value === "number") { if (!Number.isFinite(value)) return false; bytes += 24; }
    else if (typeof value === "string") { if (value.length > 65536) return false; bytes += new TextEncoder().encode(JSON.stringify(value)).length; }
      ...
      if (keys.length > 8192) return false;
      if (Array.isArray(value) && keys.length !== value.length) return false;
      for (const key of keys) {
        if (key.length > 65536) return false;
