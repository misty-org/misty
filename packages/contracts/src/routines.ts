...
export type MistyRoutineValue =
  | { kind: "literal"; value: z.output<typeof MistyCapabilityValueSchema> }
  | MistyRoutineReference
  | { kind: "object"; fields: Record<string, MistyRoutineValue> }
  | { kind: "array"; items: MistyRoutineValue[] };
const value: z.ZodType<MistyRoutineValue> = z.lazy(() => z.discriminatedUnion("kind", [
...
export type MistyRoutineCondition =
  | { kind: "equals"; left: MistyRoutineValue; right: MistyRoutineValue }
  | { kind: "exists"; reference: MistyRoutineReference }
  | { kind: "all" | "any"; conditions: MistyRoutineCondition[] }
  | { kind: "not"; condition: MistyRoutineCondition };
const condition: z.ZodType<MistyRoutineCondition> = z.lazy(() => z.discriminatedUnion("kind", [
...
function boundedTree(input: unknown, ctx: z.RefinementCtx): unknown {
  ...
  const valid = (item: unknown, depth: number): boolean => {
    ...
    seen.delete(item); return okay;
  };
  if (!valid(input, 0)) { ctx.addIssue({ code: "custom", message: "Routine data is unsafe or too deeply nested." }); return z.NEVER; }
  try {
    if (new TextEncoder().encode(JSON.stringify(input)).length > 1024 * 1024) throw new Error();
  } catch { ctx.addIssue({ code: "custom", message: "Routine data must be bounded JSON." }); return z.NEVER; }
  return input;
}
...
export const MistyRoutineTriggerSchema = z.discriminatedUnion("kind", [
  ...
  z.strictObject({ kind: z.literal("schedule"), timezone: z.string().min(1).max(100).refine(zone => { try { new Intl.DateTimeFormat("en", { timeZone: zone }); return true; } catch { return false; } }),
...
export const MistyRoutineDefinitionSchema = z.preprocess(boundedTree, definition).superRefine((routine, ctx) => {
  const seen = new Set<string>(); let turns = 0;
  const references = (input: unknown, path: (string | number)[]) => {
    ...
    if (item.kind === "reference") {
      const ref = item as MistyRoutineReference;
      if (ref.source.kind === "step" && !seen.has(ref.source.stepId)) ctx.addIssue({ code: "custom", path, message: "A reference must name an earlier step." });
      return;
    }
    for (const [key, child] of Object.entries(item)) references(child, [...path, key]);
  };
  routine.steps.forEach((step, index) => {
    if (seen.has(step.id)) ctx.addIssue({ code: "custom", path: ["steps", index, "id"], message: "Step identities must be unique." });
    ...
    if (step.kind === "agent") turns += step.maxTurns;
    seen.add(step.id);
  });
  if (turns > routine.budget.modelTurns) ctx.addIssue({ code: "custom", path: ["budget", "modelTurns"], message: "Agent steps exceed the routine's total model-turn budget." });
});
