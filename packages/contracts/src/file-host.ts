...
export const mistyFileHostContracts = {
  ...
  "files.sources.resolveLocation": {capability: "files.read", params: z.strictObject({path: z.string().min(1).max(4096).refine(value => !value.includes("\0"))}), result: z.union([z.null(), z.strictObject({virtual: z.enum(["trash", "recent", "starred"])}), z.strictObject({unavailable: z.literal(true)}), z.strictObject({sourceId: handle, relative: z.array(z.string().min(1).max(1024).refine(name => name !== "." && name !== ".." && !/[\\/\0]/.test(name))).max(256)})])},
  ...
  "files.previewUrl": {capability: "files.read", params: z.strictObject({handle}), result: z.string().min(1).max(16384)},
...
export function isMistyFileHostMethod(method: string): method is MistyFileHostMethod {
