...
export const MISTY_CAPABILITY_PROTOCOL_VERSION = 1 as const;
...
export const MistyCapabilityDeviceIdSchema = z.string().refine(
  (value) => id.safeParse(value.startsWith("device_") ? value.slice(7) : value).success,
  "A trusted device identity is required.",
);
...
export const MistyCapabilityJsonSchema = z.preprocess((value, ctx) => {
  ...
  const visit = (value: unknown, depth: number): boolean => {
    ...
    return Object.entries(value).every(([key, child]) => {
      ...
      if (key === "$ref" && (typeof child !== "string" || !child.startsWith("#/$defs/"))) return false;
...
export const MistyCapabilityProviderSchema = z.strictObject({
  ...
  capabilities: z.array(MistyCapabilityDefinitionSchema).min(1).max(100).refine(
    (items) => unique(items.map((item) => item.name)), "Duplicate capability names.",
  ),
});
