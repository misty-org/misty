...
export const MistyBrowserProviderSchema = z.strictObject({
  id: z.enum(Object.keys(mistyBrowserProviders) as [keyof typeof mistyBrowserProviders, ...(keyof typeof mistyBrowserProviders)[]]),
...
export const mistyBrowserContracts = {
  ...
  "browser.interact": {
    ...contract(handle.extend({ documentId: z.string().uuid(), action: MistyBrowserInteractionSchema }),
      z.strictObject({ attempted: z.literal(true) })),
    capability: "browser.interact" as const,
  },
  "browser.type": {
    ...contract(handle.extend({
  ...
  "browser.request": {
    ...contract(handle.extend({ path: z.string().min(1).max(2048).regex(/^\/(?!\/)/) }),
      z.strictObject({ status: z.number().int(), body: z.string().max(262144), truncated: z.boolean() })),
    capability: "browser.inspect" as const,
  },
