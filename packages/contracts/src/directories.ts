...
export const mistyDirectoryContracts = {
  "files.openTrash": {
    capability: "files.write" as const,
...
export type MistyDirectoryListing = z.output<
  (typeof mistyDirectoryContracts)["files.listDirectory"]["result"]
>;
export type MistyOpenedEntry = z.output<
  (typeof mistyDirectoryContracts)["files.openEntry"]["result"]
>;
...
const newName = z
  ...
  .refine(
    (name) =>
      ...
      !name.includes("/") &&
      !name.includes("\0") &&
      encoder.encode(name).byteLength <= 255,
    "Expected a single filename of at most 255 UTF-8 bytes.",
  );
const mutation = <P extends z.ZodType, R extends z.ZodType>(
  params: P,
  result: R,
) => ({
  capability: "files.write" as const,
...
export const mistyDirectoryMutationContracts = {
  ...
  "files.renameEntry": mutation(
    z.strictObject({ directory: handle, entry, name: newName }),
    z.strictObject({ entry, name: newName }),
  ),
  "files.removeEntry": mutation(
    z.strictObject({
      directory: handle,
      entry,
